...
CREATE VIEW [dbo].[SafeGateIntegration_VGetYardDifference_Delete] -- SELECT * FROM SafeGateIntegration_VGetYardDifference_Delete
...
FROM		(SELECT			ROW_NUMBER() OVER (PARTITION BY CD.ContainerNo ORDER BY CD.ContainerNo, CD.createdDate DESC ) SL, CD.ActivityID,CD.YardName,CD.ContainerNo
			...
			LEFT JOIN		(SELECT		RouteKey, LegID, RT.LegNo,OrderDetailKey,SourceAddrKey TMSNextSourceAddrKey, LegID AS TMSNextLegID 
							FROm Routes RT
							INNER JOIN	Leg L On RT.LegKey = L.LegKey
							WHERE		L.FromLocation = 'Yard' AND L.ToLocation <> 'Yard') RL ON OD.OrderDetailKey = RL.OrderDetailKey AND RT.LegNo+1 = RL.LegNo
			LEFT JOIN		(SELECT		RouteKey, LegID, RT.LegNo,OrderDetailKey, DestinationAddrKey TMSPrevDestinationAddrKey, LegID AS TMSPrevLegID  
							FROm Routes RT
							INNER JOIN	Leg L On RT.LegKey = L.LegKey
			...
			WHERE			1 = 1  --  AND RT.RouteKey = 502813   AND ActivityId = 985643 --AND TMSYardName IS NOT NULL --  AND OD.ContainerNo = 'GCXU2115927'  
							...
							AND ISNULL(CASE WHEN Effect = 1 THEN YD.ShortName ELSE YS.ShortName  END,'') <> '' AND CD.CreatedDate > GETDATE()-10
							) A
