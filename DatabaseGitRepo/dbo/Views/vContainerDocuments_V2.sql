...
--select * from [vContainerDocuments_V2_Shiva] where orderdetailkey = 229488
CREATE View [dbo].[vContainerDocuments_V2]
as
	...
	ddata as (
		...
		--	D.CreateDate, DT.LinkTo, DT.StoragePath, OH.OrderNo,
		--	0 AS ROUTEKEY, OD.OrderKey, D.CreateUserKey as DocumentUserKey,
		--	D.FilePath + '\' + D.OriginalFileName as DocumentWithPath
		--	FROM dbo.Document D   WITH (NOLOCK) 
		--		INNER JOIN dbo.OrderHeaderDocuments ODD WITH (NOLOCK)  ON D.DocumentKey =ODD.DocumentKey 
		--		inner join dbo.OrderDetail OD  WITH (NOLOCK) on ODD.OrderKey = OD.OrderKey
		--		inner join dbo.OrderHeader OH  WITH (NOLOCK) on OD.OrderKey = OH.OrderKey
		--		INNER JOIN dbo.DocumenType DT  WITH (NOLOCK) ON DT.DocumentTypeKey=D.DocumentType
		--	WHERE D.IsDeleted=0
	) 
