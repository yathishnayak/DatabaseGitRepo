...
/*	73164, , 10035
	...
	DECLARE @InvoiceKey int =0, @InvoiceNo Varchar(50) = '100029',   @JsonOutput nvarchar(max) ='',
			@Status	bit = 0, @Reason varchar(500) = '', @debug bit = 0
	EXEC [Cost_Rerun_Flow] @InvoiceKey, @InvoiceNo, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT, @debug = 1

*/
CREATE PROC [dbo].[Cost_Rerun_Flow]
(
	@InvoiceKey				int	= 0,
...
BEGIN
	...
	BEGIN
		...
		End
		EXEC [Cost_OutputByInvoice] @InvoiceKey, @InvoiceNo, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT, @debug = 0
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				EXEC [Cost_BI_Fetch_DataByInvoice] @InvoiceKey, @InvoiceNo, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT, @debug = 0
				Select @JsonOutput as JsonResult
				--
				--
				--
