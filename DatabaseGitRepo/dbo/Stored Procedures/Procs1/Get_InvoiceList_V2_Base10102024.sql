...
DECLARE 
	@UserKey INT=714,
	@JSONString NVARCHAR(MAX)='{"Factored":2,"CustApproved":2,"PageNo":1,"PageSize":50,"Ascending":true,"IsAscending":true,"SearchText":"","CustomerKey":"","MarketLocationKey":"","InvoicerKey":"","CustCompanyKey":"","SortField":"TerminationDate","StatusKey":9}',
	@Status BIT=0,@IsDebug		BIT = 1,
	@Reason VARCHAR(100)=''
EXec Get_InvoiceList_V2_Base10102024 @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT
Select @Status, @Reason
**/


CREATE Procedure [dbo].Get_InvoiceList_V2_Base10102024	 -- Get_InvoiceList_V2 @StatusKey=2, @PageNo = 1,@SearchText='WHSU5296890'
(
	@UserKey		INT = 714,
...
BEGIN
	...
	--IF(@StatusKey in (1,2,3,0))
		SELECT IH.* INTO #OpenInvceHeader
		...
		BEGIN
			...
			FROM			#OpenInvceHeader  IH WITH (NOLOCK)
			...
			LEFT JOIN		CustomerCompany CC WITH (NOLOCK) ON ISNULL(CU.CustomerCompanyKey,0) = ISNULL(CC.CustomerCompanyKey,0)
			LEFT JOIN		CSR CSR WITH (NOLOCK) ON CSR.CsrKey=OH.CsrKey --14531
		END
