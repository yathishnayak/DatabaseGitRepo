CREATE Procedure [dbo].[Get_InvoiceList_JSON_Prev_20240514] -- Get_InvoiceList_JSON @StatusKey=2, @PageNo = 1,@SearchText='WHSU5296890'
(
	@StatusKey		INT= 0,
	@CustomerKey	INT= 0,
	@OrderKey		INT= 0,
...
BEGIN
	...
	Begin
		insert into #ToInvoice (OrderKey, OrderNo, ContainerNo, ActualArrival, CustId, CustName, OD.OrderDetailKey,
			...
			BookingNo, TerminationDate,  AddrName, MarketLocationKey, MarketLocation,CustApproved ,ReasonCodeKey,ReasonCodeName,CustCompanyKey,AgingDays,InvoicerKey)
		select OH.OrderKey,  OH.OrderNo, OD.ContainerNo as ContainerNo, MAX(RT.ActualArrival) AS ActualArrival, CU.CustId, CU.CustName,OD.OrderDetailKey,
			...
			DATEDIFF(DAY,ISNULL(OD.CompleteDate,MAX(RT.ActualArrival)),GETDATE()) AgingDays, 0
	...
	Begin
		insert into #ToInvoice (OrderKey, OrderNo, ContainerNo, ActualArrival, CustId, CustName, OrderDetailKey,
			...
			IsPaymentReceived, RevisionUserKey, InvoiceApprovedUserKey, CustomerNote, InternalNote,CustApproved,ReasoncodeKey,ReasonCodeName,CustCompanyKey,AgingDays,InvoicerKey)
		select  
			...
			CU.CustomerCompanyKey AS CustCompanyKey,DATEDIFF(DAY,IH.InvoiceDate,GETDATE()) AgingDays, IH.CreateUserKey
	...
	OrderNo,	
		...
		A.MarketLocationKey,A.MarketLocation,A.CustApproved,A.ReasoncodeKey,A.ReasonCodeName,A.CustCompanyKey,AgingDays,InvoicerKey
	...
	where B.InvoiceCount > 0

	SELECT A.statusKey, Description, case when InvoiceCount > 0 then InvoiceCount else LastCount end InvoiceCount, 'I' AS Level 
	into #StatusData
	FROM InvoiceCounts A
