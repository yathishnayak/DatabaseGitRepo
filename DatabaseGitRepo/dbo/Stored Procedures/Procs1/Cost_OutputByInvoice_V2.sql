/*
	DECLARE @InvoiceKey int =0, @InvoiceNo Varchar(50) = '121902',   @JsonOutput nvarchar(max) ='',@Status	bit = 0 , @Reason	varchar(500) = '', @IsDebug			bit = 1 
	EXEC [Cost_OutputByInvoice] @InvoiceKey, @InvoiceNo, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT, @IsDebug
...
Declare 
	...
	@IsDebug		BIT = 0,
	@JSONSTRING		NVARCHAR(Max) = '{"InvoiceNo":"130975"}'
	EXEC [Cost_OutputByInvoice_V2] @Userkey, @JSONSTRING, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT, @IsDebug
	SELECT @JsonOutput AS JSONOutput, @Status AS Status, @Reason AS Reason
*/
CREATE PROCEDURE [dbo].[Cost_OutputByInvoice_V2]
(
	@UserKey		INT = 1144,
...
BEGIN
	...
	(
		...
		LineItem1			varchar(100), -- Pre-Pull
		LineItem1_Value		decimal(18,3),
		LineItem2			varchar(100), -- Yard Shuttle
		LineItem2_Value		decimal(18,3),
		LineItem3			varchar(100), -- Stop Off
		LineItem3_Value		decimal(18,3),
		LineItem4			varchar(100), -- Dray base
		LineItem4_Value		decimal(18,3),
		LineItem5			varchar(100), -- Accessorial Costs
	...
	select convert(varchar(50),'') as ContainerNo, RecordSL, LineItem, Market, Terminal, TruckType, YardPort, [Zone], [Group], 
		...
		SplitPercent 
	into #AccRercs
	from COSTACC_FinalDataOutput WITH(NOLOCK) where 1=0
	...
	BEGIN
		...
		select top 1 @DrayBaseValue = Cost + fsf  , @DrayageValue = Cost  , @FSFValue = fsf 
			...
			order by convert(datetime, effectivedate) desc, CostOutputDataKey 
		...
		begin
			...
			where city = @city and State = @State and DriverType =@DriverType 
				and Terminal = @Terminal and Market = @Market  
				and (YardPortType = @YArdPortType OR @YArdPortType is null)
			order by convert(datetime, effectivedate) desc
		End
		...
		/* CHANGES FOR CENTURY AND ULS and Steam*/
		...
		Begin
			...
			set InternalCost = 0
			from #ItemInfo A
			Inner Join #BaseInfo1_Auto_ReturnBaseInfo B on A.InvoiceKey = B.InvoiceKey
			Inner Join Item I WITH(NOLOCK) on A.ItemKey = I.ItemKey
			...
			SET X.UnitCost = 0
			FROM #AccRercs X
			INNER JOIN #ItemInfo A ON A.Container = X.ContainerNo
			INNER JOIN #BaseInfo1_Auto_ReturnBaseInfo B ON A.InvoiceKey = B.InvoiceKey
			INNER JOIN Item I WITH(NOLOCK) ON A.ItemKey = I.ItemKey
			INNER JOIN Item M WITH(NOLOCK) ON I.MasterItemKey = M.ItemKey
			WHERE (A.ItemKey IN (68, 121, 294) AND B.YardId IN (14, 15, 17, 19)) OR (A.itemkey in (100) and B.ChassisCategoryKey IN (2, 3));
	...
	into #Terminal from PriceGrouping WITH (NOLOCK) 
	...
	Update A set InternalCost = S.LineItem3_Value, 
			ItemUnitCost = S.LineItem3_Value
	from #ItemInfo A
	left join #ContainerSummary S on A.Container = S.ContainerNo
	where A.DriverNonDriverCostDesc = 'Stop Off'

	Update A set InternalCost = Case when @IsDraybase = 1 then 
		case when S.IsFSFExists = 1 then LineItem4_Value else LineItem4_Value - isnull(LineItem8_Value,0)  end
		else 0 end
	from #ItemInfo A
	left join #ContainerSummary S on A.Container = S.ContainerNo
	...
	update A set InternalCost = R.totalCost
	--select * 
	from #ItemInfo A
	Inner join #AccRercs R on A.Description = R.LineItem
	where A.InternalCost = 0

	if(@IsDebug = 1)
	Begin
		select '#Summary--Last',* from #Summary
		Select '#ContainerSummary--Last',* from #ContainerSummary
		select '#ItemInfo- Last',* from #ItemInfo
	end
	...
	into #LegGroups
	from Cost_LegGroups WITH (NOLOCK) Order by LegGroupKey



	Select B.ContainerNo, B.OrderDetailKey, LegName, LegOrderBy, LT.LegTypeName
	into #LegTypeList
	from  Cost_LegTypes LT WITH (NOLOCK) 
					...
					LineItemDetails =  (
						Select ItemContainer = (Select distinct Container,
							ContainerItemList = (select 'Item Cost Breakup' as Heading,  A.ItemKey,
								...
								convert(decimal(18,3), isnull(A.InternalCost / (case when A.qty > 0 then A.qty else 1 end), 
									b.UnitCost)) * 
