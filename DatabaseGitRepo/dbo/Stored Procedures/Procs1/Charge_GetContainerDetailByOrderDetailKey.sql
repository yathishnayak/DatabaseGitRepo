...
set @JsonString = '{"OrderDetailKey":121226}'
exec Charge_GetContainerDetailByOrderDetailKey @UserKey, @JSONString, @Status output, @Reason output
select @Status, @Reason
*/
CREATE PROCEDURE [dbo].[Charge_GetContainerDetailByOrderDetailKey]    -- [Charge_GetContainerDetailByOrderDetailKey] 127919
(
	@UserKey		int,
...
BEGIN
	...
	SELECT
		...
		CASE 
			...
			isnull(CU.CustID,'') as CustID, ML.MarketLocationKey,MarketLocation,
			...
			ISNULL( OH.SalesPersonKey, CU.SalesPersonKey) SalesPersonKey, U1.UserName AS CreatedUser,
	...
	FROM  dbo.OrderDetail OD WITH (NOLOCK)			
		...
		LEFT JOIN [Address] SR					WITH (NOLOCK)	ON	SR.AddrKey=isnull(RS.SourceAddrKey, OH.SourceAddrKey)
		LEFT JOIN [Address] DT					WITH (NOLOCK)	ON	DT.AddrKey=isnull(RD.DestinationAddrKey, OH.DestinationAddrKey)
		...
		LEFT JOIN [user] u1					with (nolock) ON oh.CreateUserKey = u1.UserKey
	...
	GROUP BY A.OrderDetailKey
		...
		SELECT OrderKey,A.OrderDate,A.OrderDetailKey,A.OrderTypeKey,A.OrderNo,A.CsrName,
			...
			, IsEmpty, A.IsTMF, A.DriverNotes, A.SchedulerNotes ,ISNULL(H.IsHazmat,0) AS IsHazardous,
			 isTransLoad
			, A.CustID, A.CustName, A.BrokerRefNo, A.VesselETA,A.MarketLocationKey,A.MarketLocation,
			A.ActualPickup, A.ActualDelivery, A.ActualReturn, A.SalesPersonKey, a.SalesPersonName,
			A.CreatedUser, isnull(A.isCSChargeConfirmed,0) as IsCSChargeConfirmed,LinkedContainerno
			FROM #ContainerListData A 
