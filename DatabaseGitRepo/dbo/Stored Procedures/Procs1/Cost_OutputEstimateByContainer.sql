...
/*
	DECLARE @ContainerNo		varchar(50) = '', @JsonOutput nvarchar(max) ='',@Status	bit = 0 , @Reason	varchar(100) = '' 
	SET @ContainerNo		 = 'WHSU5465619'
	EXEC Cost_OutputEstimateByContainer @ContainerNo, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT
	SELECT @JsonOutput, @Status, @Reason
*/

CREATE proc [dbo].[Cost_OutputEstimateByContainer]
(
...
BEGIN
	...
	BEGIN
		set @Status = 0
		set @Reason = 'Container No. not received'
		return
	END
	SEt @Status = 1
	Declare
	...
	Select @OrderDetailKey = OrderDetailKey, @OrderKey = OrderKey
	...
	INTO #BaseInfo
	from OrderDetail OD 
	Left join InvoiceHeader Ih on ih.OrderKey = OD.OrderKey
	...
	select ih.InvoiceKey, IH.InvoiceNo, ID.InvoicelineKey,ID.Container, ID.ItemKey,I.ItemID, I.Description, 
		...
		sum(OE.UnitCost) as ItemUnitCost, sum(I.InternalCost) InternalCost,sum( ID.qty) qty
	into #ItemInfo
	from OrderExpense  OE
	inner join Routes RT on OE.RouteKey = RT.RouteKey
	LEft join InvoiceDetail ID on Rt.OrderDetailKey = ID.OrderDetailKey and OE.Itemkey = id.ItemKey
		...
		select PriceGroupingKey, PriceGrouping, MarketLocationKey,
			...
			LineItemDetails =  (
				select 'Accessorial Item Details' as Heading, 
						isnull(LineItem, A.Description + ' (Not in Accessorial Item)') as LineItem, B.Per, isnull(b.UnitCost,A.ItemUnitCost) UnitCost , 
						convert(int, isnull(A.qty,0)) as qty,
						convert(decimal(18,3), isnull(b.UnitCost,A.ItemUnitCost)) * isnull(A.qty,0) as TotalCost
				from #ItemInfo A
				Left join #AccRercs B on A.Description = B.LineItem
				for JSON PATH
			),
