...
DECLARE 
	@UserKey INT=714,
	@JSONString NVARCHAR(MAX)='{"CustomerKeys":"","CSRKeys":"","CreatedUserKeys":"","MarketLocationKeys":"","PropertyKeys":"","EntryType":"","OrderTypeKey":"","StatusKey":0,"SearchText":"fLEXPORT","PageNo":1,"PageSize":50,"IsAscending":false,"SortField":"CreateDate","outputType":""}',
	@Status BIT=0,@IsDebug		BIT = 0,
	@Reason VARCHAR(100)=''
EXec [Get_OrderList_V2] @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @IsDebug
Select @Status, @Reason
**/
create PRocedure [dbo].[Get_OrderList_V2_temp] -- [Get_OrderList_V2] @StatusKey=2, @PageNo = 1,@SearchText='WHSU5296890'
(
	@UserKey		INT = 714,
...
*/
  ...
  --          ), 1, 1, '') AS Properties
    ...
    FROM orderheader oh with (NOLOCK)
		...
		LEFT JOIN [User] U	 with ( NOLOCK)				ON OH.CreateUserKey = U.UserKey
		LEFT JOIN MarketLocation ML WITH (NOLOCK) ON OH.MarketLocationKey =  ML.MarketLocationKey
	where ((isnull(@StatusKey,0) = 0 and  OH.Status NOT IN (6,14) AND ISNULL(@SearchText,'')<> '') 
