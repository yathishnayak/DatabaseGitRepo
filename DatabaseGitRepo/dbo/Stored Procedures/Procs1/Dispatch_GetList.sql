...
--	[Dispatch_GetList] @Status='4:', @Weekday ='FUT', @Customer='', @OrderNo = '', @ContainerNo='', @LegType = '',  @ContainerType='',@PickUpDateFrom = '2019-12-31', @PickUpDateTo = '2049-12-31' --  MSKU123457
--	[Dispatch_GetList] @ContainerNo = 'SHIV1111120'
--	[Dispatch_GetList] @Status = '1:'
--	[Dispatch_GetList] @Weekday ='', @Customer='', @OrderNo = '', @ContainerNo='', @LegType = '', @Status='', @ContainerType='',@PickUpDateFrom = '2022-08-01', @PickUpDateTo = '2022-10-07'


CREATE PROCEDURE [dbo].[Dispatch_GetList] 
		@Weekday			CHAR(3)='',
		@Customer			VARCHAR(50)='',
		@OrderNo			VARCHAR(20)='',
...
BEGIN	
	...
	SELECT			Value as StatusKey 
	INTO			#Status 
	FROM			dbo.Fn_SplitParamCol(@status)

	--SELECT *		From #Status

	SELECT			ShortComment,orderdetailkey,Comment 
	INTO			#ContTypes
	FROM			(SELECT			OC.orderdetailkey,[value] as 'Comment',LEFT([value],3) AS ShortComment
					...
					WHERE			OC.OrderDetailKey IN 	( SELECT OrderDetailKey FROM OrderDetail WHERE STATUS NOT IN (1,11) )) A 
	...
	SELECT			WeekNum =  CASE WHEN RT.PickupDateFrom BETWEEN @StartDate AND @EndDate THEN  datepart(WEEKDAY, RT.PickupDateFrom ) 
					...
					ROW_NUMBER() over (Order by ScheduledPickupDate , ContainerNo) as RowNum
	INTO			#TMP
	...
	INNER JOIN		#OrderDetailStatus ODS				WITH (NOLOCK) ON ODS.StatusKey = OD.Status
	lEFT jOIN		[USER] u2							WITH (NOLOCK) ON OD.StreetTurnSetUser = U2.UserKey
	leFT jOIN		(SELECT		DISTINCT orderdetailkey, 1 AS IsHazmat 
					FROM		#ContTypes WHERE Comment='Hazard' ) HZ ON od.OrderDetailKey = HZ.OrderDetailKey
