...
/*
	DECLARE @InvoiceKey int =0, @InvoiceNo Varchar(50) = '10278',   @JsonOutput nvarchar(max) ='',
	...
	SELECT @JsonOutput, @Status, @Reason
*/
CREATE PRoc [dbo].[Cost_OutputByInvoice_Shravana]
(
	@InvoiceKey		int	= 0,
...
BEGIN
	...
	BEGIN
		...
		WHERE Container = @_ContainerNo and DriverNonDriverCostDesc in ( 'Accessorial','Dry Run')
		...
		Begin
			set @DryRunReason = 'Records not found in Cost Database for DRY RUN and the combination of City:' + isnull(@CityDryRun,'') + ', State:' + isnull(@StateDryRun,'') 
				+ ', Market:' + isnull(@Market,'')
				+ ', Terminal:' + isnull(@Terminal,'') + ', DriverType:' + isnull(@DriverType,'')
			print '@DryRunReason: ' + @DryRunReason
		End
