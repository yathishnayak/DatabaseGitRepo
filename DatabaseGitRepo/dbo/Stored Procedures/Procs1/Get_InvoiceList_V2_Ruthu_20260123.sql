...
DECLARE 
	@UserKey INT=714,
	@JSONString NVARCHAR(MAX)='{"Factored":2,"CustApproved":2,"PageNo":1,"PageSize":50,"Ascending":true,"IsAscending":true,"CustomerKey":"","MarketLocationKey":"","InvoicerKey":"","CustCompanyKey":"","ChargeConfirmed":2,"SortField":"TerminationDate","StatusKey":2,"WarehouseStatusKeys":"","PaymentStatus":0}',
	@Status BIT=0,@IsDebug		BIT = 1,
	@Reason VARCHAR(100)=''
EXec [Get_InvoiceList_V2_Ruthu_20260123] @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @IsDebug
Select @Status, @Reason
**/


CREATE Procedure [dbo].[Get_InvoiceList_V2_Ruthu_20260123] -- Get_InvoiceList_V2_Ruthu @StatusKey=2, @PageNo = 1,@SearchText='WHSU5296890'
(
	@UserKey		INT = 714,
...
BEGIN
	...
	SELECT			@StatusKey			= StatusKey,			@CustomerKey		= CustomerKey,		@OrderKey			= OrderKey,			
					...
					@WarehouseStatusKeys=WarehouseStatusKey,    @InvoiceDateFrom =isnull(InvoiceDateFrom ,NULL), 
					@InvoiceDateTo = isnull(InvoiceDateTo ,NULL),		@ReasonCode         = ReasonCode,		@DoorToDoor         = DoorToDoor
	...
	If(@StatusKey in (0,9))
		BEGIN
			INSERT INTO		#ToInvoice (OrderKey, OrderNo, ContainerNo, ActualArrival, CustId, CustName, OD.OrderDetailKey,
							...
							AllowInvoicing, IsCSChargesApproved, CSChargesApproveDate, ExpCount,DoorToDoor, ContainerCount)
			SELECT			OH.OrderKey,  OH.OrderNo, OD.ContainerNo AS ContainerNo, 
							...
							ISNULL(OD.bookingNo, OH.BookingNo)BookingNo, ISNULL(OD.CompleteDate,'') AS TerminationDate, 
			...
			GROUP BY		OH.OrderKey,oh.OrderNo,od.ContainerNo,CU.CustName, AD.City,OH.DestinationAddrKey, OD.OrderDetailKey,
							...
							OH.CustKey, OH.OrderDate, OH.BillOfLading, OT.OrderTypeKey, OrderType, ISNULL(OD.bookingNo, OH.BookingNo), OD.CompleteDate, 
	...
	IF(@StatusKey in (1,2,3,0))
		BEGIN
			INSERT INTO		#ToInvoice (OrderKey, OrderNo, ContainerNo, ActualArrival, CustId, CustName, OrderDetailKey,
							...
							AgingDays,InvoicerKey,AprovedReasonCodeKey,ApprovedReasonCode,CustCompanyName,RouteKey,
							LegID,IsDataSelected,IsSelectedStatusKey,CSR,WarehouseStatus,WarehouseStatusKey, AllowInvoicing,ExpCount,DoorToDoor,ContainerCount)
	...
	*/
	...
	WHERE				(  ISNULL(@OrderKey,0) =0  OR OrderKey=@OrderKey )	
					...
					-- AND (@CustomerKey IS NULL OR EXISTS(SELECT 1 FROM #CustomerKeys C WHERE C.CustomerKey = CustKey))
	...
	DECLARE			@STRSQL VARCHAR(MAX)DECLARE @RecFrom INT, @RecTo INT;

	-- Calculate the row range based on page number and page size
	SET @RecFrom = ((@PageNo - 1) * @PageSize) + 1;
	SET @RecTo   = @PageNo * @PageSize;

	-- Build dynamic SQL for pagination
	SET @STRSQL = N'
		SELECT * FROM (
			SELECT TOP 1000000 *,
				   ' + CONVERT(VARCHAR, @cnt) + ' AS RecCount,
				   ROW_NUMBER() OVER(
					   ORDER BY ' + @SortField + ' ' + CASE @IsAscending WHEN 0 THEN 'DESC' ELSE 'ASC' END + ', ContainerNo'
				   + ') AS RowNum
			FROM #InvoiceListData
			WHERE IsSelectedStatusKey = 1
		) a
		WHERE RowNum BETWEEN ' + CONVERT(VARCHAR, @RecFrom) + ' AND ' + CONVERT(VARCHAR, @RecTo) + '
		ORDER BY RowNum;
	';
	...
	--VALUes (@UserKey,'Get_InvoiceList_V2','Procedure Execution end','',GETDATE())
		...
		DROP TABLE IF EXISTS #ToInvoice;
		DROP TABLE IF EXISTS #InvoiceListData;
		DROP TABLE IF EXISTS #ContainerCount;
		DROP TABLE IF EXISTS #CustomerKeys;
		DROP TABLE IF EXISTS #CustCompanyKeys;
		DROP TABLE IF EXISTS #MarketLocationKeys;
		DROP TABLE IF EXISTS #InvoicerKeys;
		DROP TABLE IF EXISTS #WarehouseStatusKeys;
