...
/*
	DECLARE @InvoiceKey int =0, @InvoiceNo Varchar(50) = '19192',   @JsonOutput nvarchar(max) ='',@Status	bit = 0 , @Reason	varchar(500) = '' 
...
/*
	DECLARE @InvoiceKey int =0, @InvoiceNo Varchar(50) = '85117',   @JsonOutput nvarchar(max) ='',
			@Status	bit = 0 , @Reason	varchar(500) = '', @debug			bit = 1
	EXEC [Cost_OutputByInvoice_Roshan] @InvoiceKey, @InvoiceNo, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT, @debug
	SELECT @JsonOutput, @Status, @Reason
*/
CREATE PRoc [dbo].[Cost_OutputByInvoice_Roshan]
(
	@InvoiceKey		int	= 0,
...
BEGIN
	...
	IH.Invoicekey, IH.InvoiceNo, IH.InvoiceDate,OD.OrderDetailKey,
	...
	if(@debug = 1)
	Begin
		Select '#BaseInfo1',* from #BaseInfo1
	end
	
	select @IsBobtail = case when  count(1) > 0 then 1 else 0 end from #BaseInfo1 where IsBobtail = 1
	select @IsDryRun = case when  count(1) > 0 then 1 else 0 end from #BaseInfo1 where IsDryRun  = 1
	select @DryRunType = LegTypeName from #BaseInfo1 where IsDryRun = 1
	...
	select distinct OrderdetailKey, ContainerNo into #ContainerBase from  #BaseInfo1
	...
	BEGIN
		...
		from #BaseInfo1 where ORDERDETAILKEY = @_OrderdetailKey AND ContainerNo = @_ContainerNo AND TruckType is not null and 
		...
		From #BaseInfo1 Where ORDERDETAILKEY = @_OrderdetailKey AND ContainerNo = @_ContainerNo 
		...
		select distinct Yardid as value into #YardShuttleKeys from #BaseInfo1 
		...
		if((Select count(1) from #BaseInfo1 where ORDERDETAILKEY = @_OrderdetailKey AND ContainerNo = @_ContainerNo AND LegCostType = '1')>0)
		Begin
			SEt @isPrePull = 1
			SElect @PrePullYardPortType = YardType, @PrePullLocationKey = YardId, @PrePullLocation = ShortName 
				from #BaseInfo1 where ORDERDETAILKEY = @_OrderdetailKey AND ContainerNo = @_ContainerNo AND LegCostType = '1'
		...
		Begin
			...
			SElect @PrePullYardPortType = 'Port', @PrePullLocationKey = YardId, @PrePullLocation = ShortName 
				from #BaseInfo1 where ORDERDETAILKEY = @_OrderdetailKey AND ContainerNo = @_ContainerNo AND IsDryRun = 1
			print '@PrePullYardPortType'
			print @PrePullYardPortType
		End

		if((Select count(1) from #BaseInfo1 where ORDERDETAILKEY = @_OrderdetailKey AND ContainerNo = @_ContainerNo AND LegCostType = '4a')>0)
		Begin
			SEt @isStopOff  = 1
			SElect @StopOffYardPortType = YardType, @StopOffLocationKey = YardId, @StopOffLocation = ShortName 
			from #BaseInfo1 where ORDERDETAILKEY = @_OrderdetailKey AND ContainerNo = @_ContainerNo AND LegCostType = '4a'
		...
		if(@debug = 1)
		Begin
			Select 'Access', @MarketKey as marketKey,
		...
		BEGIN
			...
			from #BaseInfo1 where FromLocation in ('Customer','Consignee','Shipper') and ToLocation in ('Customer','Consignee','Shipper')
			...
			from #BaseInfo1 Where FromLocation = 'Port' and 
			...
			from #BaseInfo1 where LegNo = @ConsToConsLegNo - 1
		...
		update #ContainerSummary set LineItem4_Value = Case when @IsDraybase = 1 then  case when IsFSFExists = 1 then  
			isnull(@DrayBaseValue ,0) else isnull(@DrayageValue ,0) end else 0 end
			where ContainerNo = @_ContainerNo
		update #ContainerSummary set LineItem5_Value = isnull(@AddedAccessorialsTotalCost,0)
			where ContainerNo = @_ContainerNo
		update #ContainerSummary set LineItem6_Value = Case when @IsCenturyULS = 1 then 0 else  
			case when @DryRunType = 'PrePull' then 
				(select top 1 PrePullCost from #Prepull) 
				else  isnull(@DrayBaseValueDryRun,@DrayBaseValue) end end
	...
	Update A set InternalCost = Case when @IsDraybase = 1 then 
		case when S.IsFSFExists = 1 then @DrayBaseValue else  @DrayageValue end
		else 0 end
	from #ItemInfo A
	left join #ContainerSummary S on A.Container = S.ContainerNo
	...
	inner join #BaseInfo1 B on LT.LegTypeID = B.LegCostType
	order by  LegOrderBy
		...
		select PriceGroupingKey, PriceGrouping, MarketLocationKey,
					...
					ContainerDetails = (
						...
						LegDetails = (select LegId, 
									...
									IsBobtail
									from #BaseInfo1 BI where ContainerNo = B.ContainerNo
									for JSON PATH)
						From #BaseInfo1 B
						for JSON PATH
					),
