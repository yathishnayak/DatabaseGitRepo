...
CREATE PROCEDURE [dbo].[Get_OrderHeaderSearchWithAddress_Delete]
/*
Order Screen List
*/
@CustomerKey		INT=0,
...
BEGIN
    ...
    SELECT 
		...
		OH.Consignee
	--into OrderListData
    FROM (SELECT * FROM dbo.OrderHeader ) OH   			
    ...
    WHERE   OH.[Status]=14  
		AND ( @OrderDateTo	IS NULL OR @OrderDateFrom IS NULL OR OH.OrderDate IS NULL OR OH.OrderDate BETWEEN @OrderDateFrom AND @OrderDateTo)
