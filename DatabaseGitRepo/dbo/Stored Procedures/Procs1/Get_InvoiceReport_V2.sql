...
  SET @JSONString ='{"STATUSKEY":2, "DATEFROM":'2021-01-01',"DateTo":2022-09-16,"CustomerType": 2}'

  EXEC [Get_InvoiceReport_V2] @UserKey, @JSONString, @Status OUTPUT, @Reason OUTPUT, @IsDebug
  SELECT @Status Status, @Reason Reason


DECLARE @UserKey INT = 953, 
		@JSONString NVARCHAR(MAX) ='{"StatusKey":2, "DateFrom":"2021-01-01","DateTo":"2022-09-16","CustomerType": 2}',
		@Status BIT = 0,
		@Reason VARCHAR(1000), 
		@IsDebug BIT = 0
...
CREATE PROCEDURE [dbo].[Get_InvoiceReport_V2]  -- Get_InvoiceReport @STATUSKEY = 2, @DATEFROM = '2021-01-01', @CUSTKEY = 0, @DateTo='2022-09-16' ,@CustomerType=2
(
	@UserKey		INT = 488,
...
BEGIN 
	...
	SELECT @DateFrom = DateFrom, @DateTo = DateTo, @StatusKey = StatusKey, @CustKey = CustKey, 
		   @City=City,@CSRKey= CSRKey, @CreateUserKey =CreateUserKey, @CustomerType = CustomerType, 
           @BillingCompanykey =BillingCompanykey, @MarketLocationKey =MarketLocationKey
	...
	select top 200   DI.InvoiceKey,
			DI.InvoiceNo,
			DI.InvoiceDate,
			DI.STATUS as StatusName,
			DI.STATUS,
			DI.StatusKey,
			DI.CustKey,
			DI.CustID,
...
--		inner join [User] U with(nolock) on u.UserKey = VIS.CreateUserKey
	...
	From Data_InvoiceReport DI WITH (NOLOCK)
