...
CREATE PROCEDURE [dbo].[Get_OrderHeaderSearchWithAddress_BASE20250523]
/*
Order Screen List
*/
@CustomerKey		INT=0,
...
BEGIN
	...
	SELECT COUNT (1) AS ContainerCount ,OrderKey,(SELECT STRING_AGG(ContainerNo,',')within  GROUP (ORDER BY OrderKey ASC)) AS ContainerNos INTO #ContainerCount
	FROM OrderDetail	
	GROUP BY OrderKey;

	--select * from #ContainerCount
    ...
    WHERE 
			( @OrderDateTo	IS NULL OR @OrderDateFrom IS NULL OR OH.OrderDate IS NULL OR OH.OrderDate BETWEEN @OrderDateFrom AND @OrderDateTo)
