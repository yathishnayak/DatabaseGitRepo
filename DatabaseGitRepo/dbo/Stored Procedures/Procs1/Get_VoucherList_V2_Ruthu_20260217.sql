...
DECLARE 
	@UserKey INT=1144,
	@JSONString NVARCHAR(MAX)='{"SearchCriteriaKey":0,"DriverKeys":"","OrderKeys":"","OrderNo":"","containerNo":"","voucherNo":"","VoucherKeys":"","DriverHubkeys":"","WeekNum":"","MarketLocationKeys":"","TruckTypeKeys":"","CarrierMoveTypeKeys":"","SearchText":"","SortField":"voucherno","IsAscending":true,"PageSize":50,"PageNo":1,"StatusKey":1,"isDriverPay":true}',
	@Status BIT=0, @Debug int = 0,@Reason VARCHAR(100)=''
EXec [Get_VoucherList_V2_Ruthu_20260217] @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @Debug
Select @Status, @Reason
**/
CREATE Procedure [dbo].[Get_VoucherList_V2_Ruthu_20260217] 
(
	@UserKey		int,
...
BEGIN
	...
	Begin
		Set @OrderDateFrom = case when @IsWithFilter = 0 then Getdate() - 90 else  '2025-07-01' end
	End
	...
	Begin
		Set @DeliveryDateFrom =  case when @IsWithFilter = 0 then Getdate() - 90 else '2025-07-01' end
	End
	...
	CREATE TABLE #DriverHubKey
	(
		DriverhubKey INT PRIMARY KEY
	)

	CREATE TABLE #MarketLocationKey
	(
		MarketLocationKey INT PRIMARY KEY
	)

	CREATE TABLE #TruckTypeKey
	(
		TruckTypeKey INT PRIMARY KEY
	)

	CREATE TABLE #CarrierMoveTypeKey
	(
		MoveTypeKey INT PRIMARY KEY
	)
	...
	CREATE TABLE #IsDriverPayDriverKeys 
	(
		DriverKey INT PRIMARY KEY
	)
	...
	Begin
		...
		INSERT INTO #DriverhubKey(DriverhubKey)
		SELECT TRY_CAST(value AS INT)
		FROM STRING_SPLIT(@DriverHubkeys, ',')
		WHERE TRY_CAST(value AS INT) IS NOT NULL
	End
	...
	Begin
		...
		INSERT INTO #MarketLocationKey (MarketLocationKey)
		SELECT TRY_CAST(value AS INT)
		FROM STRING_SPLIT(@marketLocationKeys, ',')
		WHERE TRY_CAST(value AS INT) IS NOT NULL
	End
	...
	Begin
		...
		INSERT INTO #TruckTypeKey (TruckTypeKey)
		SELECT TRY_CAST(value AS INT)
		FROM STRING_SPLIT(@TruckTypeKeys, ',')
		WHERE TRY_CAST(value AS INT) IS NOT NULL
	End
	...
	Begin
		...
		INSERT INTO #CarrierMoveTypeKey (MoveTypeKey)
		SELECT TRY_CAST(value AS INT)
		FROM STRING_SPLIT(@CarrierMoveTypeKeys, ',')
		WHERE TRY_CAST(value AS INT) IS NOT NULL
	End
...
END
	...
	--ELSE IF(@StatusKey=9)
	--BEGIN
	--	SELECT @DeliveryDateFrom = GETDATE()-30 ;
	...
	-- Normalize Status
		...
		BEGIN
			...
			DECLARE @YearNum INT = YEAR(@CurrentDate);
			DECLARE @StartOfYear DATE = DATEFROMPARTS(@YearNum,1,1);
			SET @DeliveryDateFrom = DATEADD(WEEK, @WeekNumInt - 1,
									DATEADD(WEEK, DATEDIFF(WEEK, 0, @StartOfYear), 0));
	...
	DECLARE @VoucherFromDate DATETIME = DATEADD(DAY, -60, GETDATE())
	...
	FROM dbo.[routes] RT WITH (NOLOCK)
		...
		INNER JOIN dbo.OrderHeader OH WITH (NOLOCK)
			ON OH.OrderKey = OD.OrderKey
		LEFT JOIN #OrderKey OKF
			ON OKF.OrderKey = OH.OrderKey
	...
	WHERE 	RTS.Status= @OpenStatusKey	and  VH.VoucherKey IS not NULL		
		...
		AND VH.VoucherDate >= @VoucherFromDate
	...
	--	AND (  isnull(@OrderKeys,'')  ='' OR OH.OrderKey in (Select ORderKey from #OrderKey) )
		...
		--AND ( @OrderDateFrom IS NULL 
  --    OR OH.OrderDate BETWEEN @OrderDateFrom AND @OrderDateTo )
		...
		-- AND (@OrderKeys IS NULL OR @OrderKeys = '' OR OKF.OrderKey IS NOT NULL)
	...
	BEGIN
		...
		WHERE 	 --VH.VoucherKey IS NULL AND 
			...
			AND (
				@OrderDateFrom IS NULL
				OR (
					  OH.OrderDate >= @OrderDateFrom
					  AND OH.OrderDate < DATEADD(DAY,1,@OrderDateTo)
				   )
			)
			...
			AND (
					@DeliveryDateFrom IS NULL
					OR (
						  RT.DeliveryDateFrom IS NOT NULL
						  AND RT.DeliveryDateFrom <> '19000101'
						  AND RT.DeliveryDateFrom >= @DeliveryDateFrom
						  AND RT.DeliveryDateFrom < DATEADD(DAY,1,@DeliveryDateTo)
					   )
				)
			...
			AND (
				...
				OR EXISTS (
						SELECT 1 
						FROM #OrderDetailKeys S 
						WHERE S.OrderDetailKey = OD.OrderDetailKey
				   )
			)
			--AND (  ISNULL(@CarrierMoveTypeKeys,'') = ''  OR MT.MoveTypeKey in (Select MoveTypeKey From #CarrierMoveTypeKey) )
		End
	...
	DECLARE @VoucherFromDate90 DATETIME = DATEADD(DAY, -90, GETDATE())
	...
	FROM dbo.[routes] RT WITH (NOLOCK)
		...
		LEFT JOIN RouteVouchers RV		WITH (NOLOCK) ON RV.RouteKey=RT.RouteKey
		-- LEFT JOIN VoucherHeader VH		WITH (NOLOCK) ON VH.VoucherKey=RV.VoucherKey
		INNER JOIN VoucherHeader VH WITH (NOLOCK) 
		ON VH.VoucherKey=RV.VoucherKey
		AND VH.VoucherDate >= @VoucherFromDate90
 ...
 --   END
	...
	;WITH ChargeCTE AS
		(
			SELECT 
				OE.RouteKey,
				COUNT_BIG(1) AS ChargeCount
			FROM OrderExpense OE WITH (NOLOCK)
			INNER JOIN Item I WITH (NOLOCK) 
				ON OE.ItemKey = I.ItemKey
			INNER JOIN #TEMPTABLE T 
				ON T.RouteKey = OE.RouteKey
			WHERE I.ItemTypeKey IN (4,5)
			GROUP BY OE.RouteKey
		)
		UPDATE T
		SET 
			T.ChargesCount = ISNULL(C.ChargeCount,0),
			T.IsRateVerified = CASE 
									WHEN T.StatusKey = 9 
										 THEN CASE WHEN ISNULL(C.ChargeCount,0) > 0 THEN 1 ELSE 0 END
									ELSE T.IsRateVerified 
							   END
		FROM #TEMPTABLE T
		LEFT JOIN ChargeCTE C 
			ON T.RouteKey = C.RouteKey;
						...
						ORDER BY 
							CASE WHEN @IsAscending = 1 THEN
								CASE @SortField
									...
									WHEN 'voucherdate' THEN CONVERT(VARCHAR, voucherdate, 112)
									WHEN 'ActualDeparture' THEN CONVERT(VARCHAR, ActualDeparture, 112)
								END
							END ASC,
