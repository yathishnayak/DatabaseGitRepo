...
DECLARE 
	@UserKey		INT				= 714,
	@JSONString		NVARCHAR(MAX)	= '{"OrderKey":185173}',
	@Status			BIT				= 0,
	@IsDebug		BIT				= 1,
	@Reason VARCHAR(100)			= ''
EXEC [Container_EditList_V2] @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @IsDebug
Select @Status, @Reason
**/
CREATE PRocedure [dbo].[Container_EditList_V2] 
(
	@UserKey		INT = 714,
...
BEGIN
	...
	*/
	...
	SELECT 
		...
		'OrderTypeDescription'		= ISNULL(OT.OrderType	,'N/A')	,
		...
		--				LEFT JOIN ContainerTypes CT		WITH (NOLOCK)	ON CTL.ContainerTypeKey = CT.ContainerTypeKey
		--				WHERE OD.OrderDetailKey = CTL.OrderDetailKey),
		'ContainerSizeKey' = CSz.ContainerSizeKey	,
...
--		LEFT JOIN OrderDetailStops		ODS			WITH (NOLOCK)	ON OD.OrderDetailKey = ODS.OrderDetailKey
		...
		--LEFT JOIN (SELECT TOP  1 * FROM  OrderDetailStops WITH (NOLOCK)  WHERE OD.OrderDetailKey = OrderDetailKey AND StopTypeKey = 2
		--													AND ISNULL(IsDryRunCustomer,0) = 0 
		--													AND ISNULL(IsDryRunPort,0) = 0 ORDER BY OrderDetailStopKey ASC)
		...
		--LEFT JOIN (SELECT TOP 1 * FROM OrderDetailStops WITH (NOLOCK)	WHERE OD.OrderDetailKey = OrderDetailKey AND StopTypeKey = 4
		--													AND ISNULL(IsDryRunCustomer,0) = 0 
		--													AND ISNULL(IsDryRunPort,0) = 0 ORDER BY OrderDetailStopKey ASC)	
		--													ODS_StpB ON OD.OrderDetailKey = ODS_StpB.OrderDetailKey
