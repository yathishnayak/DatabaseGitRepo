...
/*
	...
	EXEC [Cost_OutputByInvoice_Sv1] @InvoiceKey, @InvoiceNo, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT, @debug
	SELECT @JsonOutput, @Status, @Reason
*/

/*
	DECLARE @InvoiceKey int =0, @InvoiceNo Varchar(50) = '125578',   @JsonOutput nvarchar(max) ='',
			@Status	bit = 0 , @Reason	varchar(500) = '', @debug			bit = 1
	EXEC [Cost_OutputByInvoice_Sv1] @InvoiceKey, @InvoiceNo, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT, @debug
	SELECT @JsonOutput, @Status, @Reason
*/
CREATE Proc [dbo].[Cost_OutputByInvoice_Sv1]
(
	@InvoiceKey		int	= 0,
...
BEGIN
	...
	BEGIN
		...
		Begin
			...
			from #BaseInfo1_Auto_ReturnBaseInfo
			where ORDERDETAILKEY = @_OrderdetailKey AND ContainerNo = @_ContainerNo AND  City is not null
				and ToLocation in ('Shipper', 'Customer', 'Consignee', 'Port') and isnull(IsDryRun,0) = 0 and 
			...
			from #BaseInfo1_Auto_ReturnBaseInfo
			where ORDERDETAILKEY = @_OrderdetailKey AND ContainerNo = @_ContainerNo AND  City is not null
				and ToLocation in ('Shipper', 'Customer', 'Consignee', 'Port') and isnull(IsDryRun,0) = 1
			order by LEgNo DESC
		End
