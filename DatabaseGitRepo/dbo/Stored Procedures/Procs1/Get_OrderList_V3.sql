...
DECLARE 
 ...
 @IsDebug BIT = 1,
 @Reason VARCHAR(100)=''
EXEC [Get_OrderList_V3] @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @IsDebug
Select @Status, @Reason
**/
CREATE PROCEDURE [dbo].[Get_OrderList_V3]
(
 @UserKey  INT = 714,
...
BEGIN
 ...
 Declare
  @CustomerKeys  varchar(1000) = '',
  @OrderDateFrom  DATE='01/01/2020',
  @OrderDateTo  DATE='01/01/2099',
  @CSRKeys   varchar(1000) = '',
  @PropertyKeys  varchar(200) = '',
  @OrderTypeKey  varchar(200) = '',  
  @StatusKey   INT = 0,
  @EntryTypes   varchar(500) = '',
  @marketLocationKeys varchar(1000) = '',
  @SearchText   NVARCHAR(MAX) ='',
  @CreatedUserKeys varchar(1000) = '',
  @PageNo    INT = 1,
  @PageSize   INT = 10,
 ...
 DropDowns = ( SELECT
  ...
  CSRList = (
   SELECT MIN(CSRKey) AS CSRKey, CSRName 
   FROM CSR WITH (NOLOCK)
   WHERE CSRName IS NOT NULL AND IsActive = 1
  ...
  MarketLocList = (
   ...
   WHERE ISNULL(MarketLocation, '') <> ''
   ORDER BY MarketLocation 
   FOR JSON PATH
  ),

  CreatedUserList = (
   SELECT UserKey AS CreateUserKey, UserName AS CreatedUsedName 
  ...
  WarehouseStatus = (
   SELECT DISTINCT StatusKey, Description 
  ...
  PropertiesList = (
   SELECT ContainerTypeKey, TypeID 
  ...
  DeliveryLocationList = (
   SELECT DISTINCT DT.AddrKey, DT.AddrName + ',' + DT.Address1 AS Address1 
   FROM [Address] DT WITH (NOLOCK)
   INNER JOIN OrderStops OST ON DT.AddrKey = OST.StopAddrKey 
    AND OST.StopTypeKey = 3
   WHERE ISNULL(DT.AddrName, '') <> ''
   ORDER BY DT.AddrName
   FOR JSON PATH
  )
  FOR JSON PATH
 ),
 ...
 BEGIN
  ...
  DropDowns = ( SELECT
   ...
   CustomerList = (
    SELECT CustKey, CustName 
    FROM Customer WITH (NOLOCK)
    WHERE IsActive = 1 AND ISNULL(CustName, '') <> ''
    ORDER BY CustName 
    FOR JSON PATH
   ),

   CSRList = (
    SELECT MIN(CSRKey) AS CsrKey, CSRName AS CsrName 
    FROM CSR WITH (NOLOCK)
    WHERE IsActive = 1 
      AND ISNULL(LTRIM(RTRIM(CSRName)), '') <> ''
    GROUP BY CSRName
    ORDER BY CSRName 
    FOR JSON PATH
   ),
