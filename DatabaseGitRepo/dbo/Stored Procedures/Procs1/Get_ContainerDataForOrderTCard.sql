/*
DECLARE @UserKey		INT,
	@JsonString		VARCHAR(MAX)='{"OrderNo":"KNCO125064388"}',
	@Status			BIT	= 0 ,
	@Reason			VARCHAR(1000) = '' 

	EXEC [Get_ContainerDataForOrderTCard] @UserKey,@JsonString,@Status output,@Reason output
	SELECT @Status AS Status, @Reason AS Reason

*/

CREATE Procedure [dbo].[Get_ContainerDataForOrderTCard]  
(
	@UserKey		INT,
...
BEGIN
	...
	CREATE TABLE #OrderNoData
	(
		OrderNo			VARCHAR(50)
	)

	INSERT INTO #OrderNoData(OrderNo)
	SELECT OrderNo
	...
	select @FromOrderKey = OrderKey from OrderHeader WITH(NOLOCK) where OrderNo = (SELECT OrderNo FROM #OrderNoData)
	...
	WHERE 
		( @FromOrderKey= 0 OR OH.OrderKey = @FromOrderKey )
...
-- SELECT * FROM #ContainerRoute
		...
		SELECT distinct OrderKey,A.OrderDate,A.OrderDetailkey,A.OrderTypeKey,A.OrderNo,A.CsrName,
			...
			--CT.Comments AS ContainerProperties
