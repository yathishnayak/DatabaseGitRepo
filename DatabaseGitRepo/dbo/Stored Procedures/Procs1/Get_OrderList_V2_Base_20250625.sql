...
**/
CREATE PRocedure [dbo].[Get_OrderList_V2_Base_20250625] -- [Get_OrderList_V2] @StatusKey=2, @PageNo = 1,@SearchText='WHSU5296890'
(
	@UserKey		INT = 714,
...
FROM	dbo.OrderHeader OH   with ( NOLOCK) 	
  ...
  --          ), 1, 1, '') AS Properties
    ...
    FROM #FilteredOrderKeys FOH 
		...
		LEFT JOIN #OrdersWithProps CP WITH (NOLOCK) on OH.OrderKey = CP.OrderKey
		LEFT JOIN vContainerTypeByOrder CTO WITH (NOLOCK) on OH.OrderKey = CTO.OrderKey
		LEFT JOIN OrderStops OSF WITH (NOLOCK) ON OSF.OrderKey = FOH.OrderKey  AND OSF.StopTypeKey = 1
		LEFT JOIN [Address] SR	 with ( NOLOCK)			ON	SR.AddrKey= OSF.StopAddrKey
		LEFT JOIN OrderStops OST WITH (NOLOCK) ON OST.OrderKey = FOH.OrderKey  AND OST.StopTypeKey = 3
