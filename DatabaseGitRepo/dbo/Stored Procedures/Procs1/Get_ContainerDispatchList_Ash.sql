...
CREATE PROCEDURE [dbo].[Get_ContainerDispatchList_Ash] -- Get_ContainerDispatchList @StatusKey=2, @InvoiceNo = '5'
@StatusKey		INT= 0,
@CustomerKey	INT= 0,
@OrderKey		INT= 0,
...
BEGIN
	...
	SELECT OH.OrderKey, 
		...
		RevisionDate,OH.BrokerRefNo, CU.IsFactored, OD.VesselETA, isnull(VIB.BalanceAmount,IH.InvoiceAmount) as BalanceAmount
	INTO #Toinvoice		
	...
	GROUP BY OH.OrderKey,oh.OrderNo,od.ContainerNo,CU.CustName,IH.IsInvoiceApproved,
		...
		,IsPrinted, PrintedUserKey, PaymentRecdUserKey, PaymentRecdDate, PrintedDate,IsRevised, 
		RevisionDate, BrokerRefNo, CU.IsFactored, OD.VesselETA, VIB.BalanceAmount,
	...
	OrderNo,	
		...
		A.BrokerRefNo, IsFactored, A.VesselETA, A.BalanceAmount
	FROM #Toinvoice A		
	...
	GROUP BY A.OrderKey,OrderNo,CustID,CustName,A.City,A.IsInvoiceApproved,A.StatusKey,A.OrderDetailKey,
		...
		, BrokerRefNo, IsFactored, VesselETA, A.BalanceAmount
