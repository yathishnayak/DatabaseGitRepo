...
DECLARE @UserKey INT = 714, @JSOnString NVARCHAR(MAX) = '', @Status BIT, @IntError NVARCHAR(MAX), @Reason VARCHAR(1000), @IsDebug BIT = 1
		, @FirebaseID VARCHAR(100) = 'FBID00003',@IsLogout BIT
SET @JSONString = '{"DriverKey":3,"OrderDetailKey":73896,"RouteKey":261874,"DriverExceptionKey":4,"DriverExceptionText":""}'
EXEC [DA_ConfirmPickupDelivery] @UserKey,@JSOnString,@Status OUTPUT, @IntError OUTPUT, @Reason OUTPUT, @IsDebug, @FirebaseID , @IsLogout OUTPUT
SELECT @Status,@IntError,@Reason, @IsLogout
*/

CREATE PROCEDURE	[dbo].[DA_ConfirmPickupDelivery]
(
	@UserKey		INT = 714,
	@JSONString		NVARCHAR(MAX) = '{"DriverKey":3,"RouteKey":261874,"Latitude":10.2356854,"Longitude":25.365214875,"RouteType":"Pickup"}',
	@Status			BIT	= 0 OUTPUT,
...
BEGIN
	...
	BEGIN TRANSACTION
		...
		DECLARE		@InternalError	VARCHAR(1000) = '', @Latitude DECIMAL(12,10),@Longitude DECIMAL(12,10)		
		SET			@Status = 1
		...
		ELSE
			BEGIN
				SELECT	@DriverKey = DriverKey,@RouteKey = RouteKey,@Latitude = Latitude, @Longitude = Longitude, @RouteType = RouteType
				FROM	OPENJSON(@JSONString, '$')
						WITH (
								DriverKey			INT				'$.DriverKey',
								RouteKey			INT				'$.RouteKey',
								Latitude			DECIMAL(12,10)	'$.Latitude',
								Longitude			DECIMAL(12,10)	'$.Longitude',
				...
				IF(@RouteType = '')
					BEGIN
						SET	@Status = 0
						SET @InternalError = @InternalError + '; RouteType Cannot be blank'
					END
