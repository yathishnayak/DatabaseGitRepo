...
EXEC [Admin_GetDBTableSize_JCBLogs] @UserKey,@JSOnString,@Status OUTPUT, @IntMessage OUTPUT, @ExtMessage OUTPUT, @Result1 OUTPUT, @Result2 OUTPUT
,@Result3 OUTPUT, @IsDebug

SELECT @Status,@IntMessage,@ExtMessage,@Result1,@Result2,@Result3
*/
CREATE PROCEDURE [dbo].[Admin_GetDBTableSize_JCBLogs]
(
	@UserKey		INT = 714,
...
INTO		#DBDate
FROM		JCB_Logs.sys.tables t
INNER JOIN  JCB_Logs.sys.indexes i ON t.object_id = i.object_id
INNER JOIN	JCB_Logs.sys.partitions p ON i.object_id = p.object_id AND i.index_id = p.index_id
INNER JOIN	JCB_Logs.sys.allocation_units a ON p.partition_id = a.container_id
...
FROM JCB_Logs.sys.allocation_units
FOR JSON PATH)
