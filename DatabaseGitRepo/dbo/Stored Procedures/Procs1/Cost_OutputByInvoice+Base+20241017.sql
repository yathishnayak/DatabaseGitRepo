...
/*
	DECLARE @InvoiceKey int =0, @InvoiceNo Varchar(50) = '34759',   @JsonOutput nvarchar(max) ='',@Status	bit = 0 , @Reason	varchar(500) = '' 
	EXEC [Cost_OutputByInvoice] @InvoiceKey, @InvoiceNo, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT
	SELECT @JsonOutput, @Status, @Reason
*/
create PRoc [dbo].[Cost_OutputByInvoice+Base+20241017]
(
	@InvoiceKey		int	= 0,
...
BEGIN
	...
	BEGIN
		...
		update #ContainerSummary set LineItem5_Value = isnull(@AddedAccessorialsTotalCost,0)
			where ContainerNo = @_ContainerNo
		update #ContainerSummary set LineItem6_Value = case when @DryRunType = 'PrePull' then 
