...
--exec [Get_ContainerDataForTCard]  @OrderNoFrom = 'AEU01-2006-01', @OrderNoTo = 'AEU01-2006-07', @selectedOrderDetailKeys=''
CREATE Procedure [dbo].[Get_ContainerDataForTCard_old]  
...
@CreatedDateFrom			DATE='01/12/2099',
...
BEGIN
	...
	where OCT.Description like '%Transload%'
		...
		--SELECT OC.orderdetailkey,[value] as 'Comment',LEFT([value],3) AS ShortComment
		--into #ContTypeData
		--FROM #ContType C  
		--	CROSS APPLY STRING_SPLIT(C.Comment,',')  
		--	INNER JOIN #ContainerListData OC   ON  OC.OrderDetailkey = C.OrderDetailKey					
