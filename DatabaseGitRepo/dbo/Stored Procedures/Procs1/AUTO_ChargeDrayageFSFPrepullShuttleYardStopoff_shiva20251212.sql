...
-- [AUTO_ChargeDrayageFSFPrepullShuttleYardStopoff_shiva20251212] 280784, 1
CREATE PROC [dbo].[AUTO_ChargeDrayageFSFPrepullShuttleYardStopoff_shiva20251212]  
(
...
BEGIN
	...
	select @IsEmptyYardStopOffItemExists  = Case when count(1) > 0 then 1 else 0 end 
			from orderExpense OE WITH (NOLOCK)
			LEft join Item I WITH (NOLOCK) on OE.itemkey = I.ItemKey 
			where OE.OrderDetailKey = @OrderDetailKey and (OE.itemkey = @YardEmptyStopOffItemKey OR I.MasterItemKey = @YardEmptyStopOffItemKey)
	select @IsLoadedYardStopOffItemExists = Case when count(1) > 0 then 1 else 0 end 
			from orderExpense OE WITH (NOLOCK)
			LEft join Item I WITH (NOLOCK) on OE.itemkey = I.ItemKey 
			where OE.OrderDetailKey = @OrderDetailKey and (OE.itemkey = @YardLoadedStopOffItemKey OR I.MasterItemKey = @YardLoadedStopOffItemKey)
			...
			*/
			...
			Begin
				...
				@ShuttleRouteKey, @StopOffRouteKey), @StopOffValue, @StopOffQty, @StopOffValue, Getdate(),  
					1, @YardStopOffFreeTime, @YardStopOffBvsNB, @YardStopOffMin, @YardStopOffMax, 'Auto', 
					@OrderDetailKey, @StopOffDate

				INSERT INTO AuditLogDetail (DateCreated, CreateUser, RefType, RefId, RefKey, Stage, CommentType, Comments)
				Select GETDATE(), 'Admin', 'Container', @ContainerNo, @OrderDetailKey, 'Auto', 'Text' , 
					'Yard Stop-Off Item Added for Container '
			...
			ELSE IF(@CustKey IN (@CenturyCustKey1, @CenturyCustKey2)  AND @IsLinked = 1 AND isnull(@IsYardStopOffItemExists,0) = 0)
			Begin
				...
				UnitCost, Qty, NewUnitCost, CreateDate, 
				CreateUserKey, FreeTime, BvsNB, MinCnt, MaxCnt, ChargeSource, OrderDetailKey, DateFrom)
				Select @YardStopOffItemKey, COALESCE(@DraybaseRouteKey, @FSFRouteKey, @PrePullRouteKey, @StopoffRouteKey, @StopOffRouteKey), 
			...
			BEGIN
				...
				declare 
					@OrderTypeTemp			varchar(10)		= '' ,
					@YardTypeChain			varchar(100)	= '' ,
					@LocationChain			varchar(20)		= '' ,
					@IsStopOff				bit				=  0 ,
					@IsShuttle				bit				=  0 ,
					@IsCustomerApplicable	Bit				=  0 

				Exec GetYardStopOffLogic @OrderDetailKey, @OrderTypeTemp OUTPUT, @YardTypeChain OUTPUT, @LocationChain OUTPUT,
					@IsStopOff OUTPUT, @IsShuttle OUTPUT, @IsCustomerApplicable OUTPUT

				select  @OrderTypeTemp , @YardTypeChain , @LocationChain, @IsShuttle, @IsStopOff, @IsCustomerApplicable
				if(@OrderTypeTemp in ('Import','Export') and @LocationChain like '%C-Y-Y-P%' and 
					 @IsYardShuttleItemExists = 0 )
				BEGIN
					insert into OrderExpense (Itemkey, Routekey, UnitCost, Qty, NewUnitCost, CreateDate, CreateUserKey, FreeTime, 
					BvsNB, MinCnt, MaxCnt, ChargeSource, OrderDetailKey, DateFrom)
					Select @YardShuttleItemKey, @ShuttleRouteKey, @ShuttleValue, @ShuttleQty, @ShuttleValue, Getdate(),  
				...
				if(@OrderTypeTemp in ('Import') and (@LocationChain like '%C-Y-P%' OR @LocationChain like '%C-Y-Y-P%' ) 
					and @IsEmptyYardStopOffItemExists = 0 )
				BEGIN
					insert into OrderExpense (Itemkey, Routekey, UnitCost, Qty, NewUnitCost, CreateDate, CreateUserKey, FreeTime, 
						BvsNB, MinCnt, MaxCnt, ChargeSource, OrderDetailKey, DateFrom)
					Select case when @OrderTypeTemp = 'Import' then @YardEmptyStopOffItemKey else @YardLoadedStopOffItemKey end,
						@StopOffRouteKey, @StopOffValue, @StopOffQty, @StopOffValue, Getdate(),  
						1, @YardStopOffFreeTime, 
						case when @YardTypeChain like '%IE-Port%' then 0 else 1 end, --NB = 0, B = 1
						@YardStopOffMin, @YardStopOffMax, 'Auto', 
						@OrderDetailKey, @StopOffDate

					INSERT INTO AuditLogDetail (DateCreated, CreateUser, RefType, RefId, RefKey, Stage, CommentType, Comments)
					Select GETDATE(), 'Admin', 'Container', @ContainerNo, @OrderDetailKey, 'Auto', 'Text' ,
					'Yard Stop-Off ' + case when @OrderTypeTemp = 'Import' then 'Empty StopOff' else 'Loaded StopOff' end  + ' Added for Container '

					Set @ItemCount = @ItemCount + 1
				END

				if(@OrderTypeTemp in ('Export') and (@LocationChain like '%C-Y-P%' OR @LocationChain like '%C-Y-Y-P%' ) 
					and  @IsLoadedYardStopOffItemExists = 0 )
				BEGIN
					insert into OrderExpense (Itemkey, Routekey, UnitCost, Qty, NewUnitCost, CreateDate, CreateUserKey, FreeTime, 
						BvsNB, MinCnt, MaxCnt, ChargeSource, OrderDetailKey, DateFrom)
					Select case when @OrderTypeTemp = 'Import' then @YardEmptyStopOffItemKey else @YardLoadedStopOffItemKey end,
