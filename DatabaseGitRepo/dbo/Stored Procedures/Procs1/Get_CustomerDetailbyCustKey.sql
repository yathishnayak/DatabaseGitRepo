CREATE PROCEDURE [dbo].[Get_CustomerDetailbyCustKey] -- [Get_CustomerDetailbyCustKey] 15
@CustomerKey INT
AS
BEGIN
	SET NOCOUNT ON
	...
	SELECT
		...
		SellContacts=(SELECT ContactKey,ContactName,ContactEmail,CustomerKey 
					 FROM Customer_SellContacts 
					 WHERE CustomerKey=@CustomerKey FOR JSON PATH)
	...
	WHERE C.custkey =  @CustomerKey --AND StatusName='Active'
END