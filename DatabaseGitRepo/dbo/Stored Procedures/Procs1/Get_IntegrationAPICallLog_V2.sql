...
Declare 
	...
	EXEC [Get_IntegrationAPICallLog_V2] @Userkey, @JSONSTRING, @Status OUTPUT, @Reason Output, @IsDebug
**/

CREATE PROCEDURE [dbo].[Get_IntegrationAPICallLog_V2]
(
	@UserKey		INT = 1144,
...
BEGIN
	...
	SELECT 
		IA.CustKey, 
		C.CustID + ' - '+C.CustName As CustName, 
		(ISNULL(A.AddrName,'') +CHAR(10)+CHAR(13) +ISNULL(A.Address1,'') + CHAR(10)+CHAR(13)+ISNULL(A.Address2,'')
		+CHAR(10)+CHAR(13)+ISNULL(A.City,'')+CHAR(10)+CHAR(13)+ISNULL(A.ZipCode,'')+CHAR(10)+CHAR(13)+ISNULL(A.[State],'')
		+CHAR(10)+CHAR(13)+ISNULL(A.Country,'') ) AS FullAddress,
		AddressKey, 
		RequestString, 
		RepsonseString, 
		ExceptionString, 
