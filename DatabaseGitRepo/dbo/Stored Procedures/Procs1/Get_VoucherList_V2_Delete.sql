...
**/
CREATE Procedure [dbo].[Get_VoucherList_V2_Delete] 
(
	@UserKey		int,
...
INSERT INTO SqlExecutionTimeLog
	...
	(
			...
			IsPaid				bit,
			DriverHubKey		int,
			DriverHubName		varchar(500),
			MarketLocationKey	INT,
			MarketLocation		VARCHAR(500),
			PaidUserKey			int,
			PaidUserName		varchar(500),
			IsLinked			bit default 0, 
			LinkedContainerNo	varchar(100), 
	...
	BEGIN
		...
		SELECT distinct TOP 10  Case when OrdCount = 1 then OH.OrderKey else 0 end AS OrderKey,
