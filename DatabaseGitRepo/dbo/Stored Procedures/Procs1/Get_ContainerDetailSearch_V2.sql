...
DECLARE 
	...
	@IsDebug BIT = 0, 
	@Reason	VARCHAR(100)=''
	EXec [Get_ContainerDetailSearch_V2] @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @IsDebug
	Select @Status AS Status, @Reason AS Reason
*/
CREATE PROCEDURE [dbo].[Get_ContainerDetailSearch_V2]
(
	@UserKey		INT = 0,
	@JSONString		NVARCHAR(MAX) = '',
	@Status			BIT	= 0 OUTPUT,
...
BEGIN
	...
	FROM  dbo.OrderDetail OD			
		...
		INNER JOIN dbo.OrderStatus OS	ON OS.[Status]=OH.[Status]
	...
	WHERE  -- OS.[Description] <>'On Hold' AND
		--    ( @PickupDateFrom   IS NULL OR RT.PickupDate IS NULL OR RT.PickupDate>=@PickupDateFrom)
		--AND ( @PickupDateTo		IS NULL OR RT.PickupDateTo IS NULL OR RT.PickupDateTo<=@PickupDateTo)
		--(@PickupDateFrom is null OR @PickupDateTo is null 
		--OR RT.PickupDate  between @PickupDateFrom and @PickupDateTo OR RT.PickupDateTo between @PickupDateFrom and @PickupDateTo)
		...
		(
		   (@PickupDateFrom IS NULL OR RT.PickupDate >= @PickupDateFrom)
		AND (@PickupDateTo   IS NULL OR RT.PickupDate <= @PickupDateTo)
		)
		AND
		(
		   (@DeleveryDateFrom IS NULL OR RT.DeliveryDate >= @DeleveryDateFrom)
		AND (@DeleveryDateTo   IS NULL OR RT.DeliveryDate <= @DeleveryDateTo)
		)
 ...
 /*
	UPDATE OD SET IsTransLoad = 1
	--SELECT 	OC.orderdetailkey,OC.commentkey,[value] as 'Comment', OD.ContainerNo
	FROM [dbo].[Comment] C  
		CROSS APPLY string_split(C.description,',')  
	INNER JOIN 
		[dbo].[OrderDetailComments] OC   ON  OC.CommentKey = C.CommentKey
	inner join #ContainerListData OD on OD.OrderDetailKey = OC.OrderDetailKey
	INNER JOIN ContainerTypes CT ON [value] = CT.TypeID
	where CT.TypeID = 'TransLoad'
	*/
	...
	where OCT.Description like '%Transload%'
		...
		INNER JOIN dbo.Routes D ON D.RouteKey=A.RouteKey

		--select * from #ContainerRoute2

		SELECT A.CompletedRoutekey,K.ToLocation,A.OrderDetailKey INTO #ComplLegLocation 
		FROM #CompletedLeg A 
		INNER JOIN dbo.Routes K ON K.RouteKey=A.CompletedRoutekey

		SELECT A.OrderDetailKey,R.AddrName INTO #Sourceloc
		FROM #OrderDetl A 
			INNER JOIN  dbo.OrderDetail J on j.OrderDetailKey=A.OrderDetailKey
			INNER JOIN dbo.[Address] R ON R.AddrKey=J.SourceAddrKey

		SELECT A.OrderDetailKey,'Dispatch Complete' AS CompleteStatus INTO #CompletedCont
		FROM #OrderDetl A 
			INNER JOIN dbo.OrderDetail D ON D.OrderDetailKey=A.OrderDetailKey
			INNER JOIN dbo.OrderDetailStatus W ON W.Status=D.Status
			INNER JOIN dbo.Routes RT ON RT.OrderDetailKey=A.OrderDetailKey
			INNER JOIN dbo.RouteStatus RTS ON RTS.[Status]=RT.[Status]
		...
		INNER JOIN dbo.[Address] DT ON DT.AddrKey=RT.DestinationAddrKey
		...
		INNER JOIN dbo.Routes RT	ON RT.RouteKey=A.RouteKey
		INNER JOIN dbo.[Address] DT ON DT.AddrKey=RT.SourceAddrKey
		...
		FROM #OrderDetl A 
			INNER JOIN dbo.Routes RT ON RT.OrderDetailKey=A.OrderDetailKey
			INNER JOIN dbo.RouteStatus RTS ON RT.[Status]=RTS.[Status]
		...
		FROM #ContCurlocation A 
			...
			LEFT JOIN dbo.Routes RT ON RT.RouteKey=A.RouteKey
		...
		SELECT OrderKey,A.OrderDate,A.OrderDetailkey,A.OrderTypeKey,A.OrderNo,A.CsrName,
			...
			--ELSE (CASE WHEN CompleteStatus='Dispatch Confirmed' THEN 0 ELSE ISNULL(MN.LegNo,0) END) END AS VARCHAR(50)) +' of '+ CAST(LegCount AS VARCHAR(50)) END AS CurLeg
			 isTransLoad
			, A.CustID, A.CustName, A.BrokerRefNo, A.VesselETA, A.CreatedUser, A.TerminationDate
			FROM #ContainerListData A 
		...
		WHERE  (@IsTransLoad = 0 or A.isTransLoad = @IsTransLoad)
		FOR JSON PATH
END