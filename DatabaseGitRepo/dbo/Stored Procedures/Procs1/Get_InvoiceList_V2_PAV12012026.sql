...
DECLARE 
	@UserKey INT=714,
	@JSONString NVARCHAR(MAX)='{"Factored":2,"CustApproved":2,"PageNo":1,"PageSize":50,"Ascending":true,"IsAscending":true,"SearchText":"DHLNC2025556","CustomerKey":"","MarketLocationKey":"","InvoicerKey":"","CustCompanyKey":"","ChargeConfirmed":2,"SortField":"TerminationDate","StatusKey":16,"WarehouseStatusKeys":"","DoorToDoor":2,"PaymentStatus":0,"SearchCriteriaKey":4}'
...
CREATE Procedure [dbo].[Get_InvoiceList_V2_PAV12012026] -- Get_InvoiceList_V2 @StatusKey=2, @PageNo = 1,@SearchText='WHSU5296890'
(
	@UserKey		INT = 714,
...
BEGIN
	...
	(
		...
		BEGIN
			...
			BEGIN
				-- Try Order No / Container No first
				...
				WHERE OH.OrderNo = @SearchText
				   OR ContainerNo = @SearchText;

				-- If no OrderDetail found, try Invoice No
				IF (NOT EXISTS (SELECT 1 FROM #OrderDetailKeys ) OR (SELECT COUNT (1) FROM InvoiceContainers WHERE Containerno=@SearchText)>0)
				BEGIN
					INSERT INTO #InvoiceKeys (InvoiceKey)
					SELECT IH.InvoiceKey
					FROM InvoiceHeader IH WITH (NOLOCK)
					JOIN InvoiceContainers IC WITH (NOLOCK) ON IC.InvoiceKey=IH.InvoiceKey
					WHERE IH.InvoiceNo = @SearchText OR ContainerNo=@SearchText;
				END
			END		
			ELSE
			BEGIN
				...
				BEGIN
					...
					-- If no OrderDetail found, try Invoice No
					IF (NOT EXISTS (SELECT 1 FROM #OrderDetailKeys ) OR (SELECT COUNT (1) FROM InvoiceContainers WHERE ContainerNo IN (SELECT VALUE FROM fn_splitparam(@SearchText)))>0)
					BEGIN
						INSERT INTO #InvoiceKeys (InvoiceKey)
						SELECT IH.InvoiceKey
						FROM InvoiceHeader IH WITH (NOLOCK)
				...
				BEGIN
					...
					-- If no OrderDetail found, try Invoice No
					IF (NOT EXISTS (SELECT 1 FROM #OrderDetailKeys ) OR (SELECT COUNT (1) FROM InvoiceContainers WHERE OrderDetailsKey IN (SELECT OrderDetailKey FROM #OrderDetailKeys))>0)
					BEGIN
						INSERT INTO #InvoiceKeys (InvoiceKey)
						SELECT IH.InvoiceKey
						FROM InvoiceHeader IH WITH (NOLOCK)
						JOIN OrderHeader OH WITH (NOLOCK) ON IH.OrderKey = OH.OrderKey
						JOIN InvoiceContainers IC WITH (NOLOCK) ON IC.InvoiceKey=IH.InvoiceKey
							WHERE OrderNo IN (SELECT VALUE FROM fn_splitparam(@SearchText));
					END
				END
				...
				BEGIN
					INSERT INTO #InvoiceKeys (InvoiceKey)
						SELECT InvoiceKey
						FROM InvoiceHeader	IH	WITH (NOLOCK)
						WHERE InvoiceNo IN (SELECT VALUE FROM fn_splitparam(@SearchText))
				END
			END
	END
	...
	--If(@StatusKey in (0,9))
		BEGIN
			INSERT INTO		#ToInvoice (OrderKey, OrderNo, ContainerNo, ActualArrival, CustId, CustName, OD.OrderDetailKey,
							...
							AllowInvoicing, IsCSChargesApproved, CSChargesApproveDate, ExpCount,DoorToDoor, ContainerCount,InvoicePaymentStatus)
	...
	--IF(@StatusKey in (1,2,3,0))
		BEGIN
			INSERT INTO		#ToInvoice (OrderKey, OrderNo, ContainerNo, ActualArrival, CustId, CustName, OrderDetailKey,
							...
							AgingDays,InvoicerKey,AprovedReasonCodeKey,ApprovedReasonCode,CustCompanyName,RouteKey,
							LegID,IsDataSelected,IsSelectedStatusKey,CSR,WarehouseStatus,WarehouseStatusKey, AllowInvoicing,ExpCount,DoorToDoor,ContainerCount,InvoicePaymentStatus)
			SELECT			OH.OrderKey,  OH.OrderNo, '' AS ContainerNo, '' AS ActualArrival, CU.CustId, CU.CustName, 0 OrderDetailsKey,
							...
							case when OT.ordertypekey = 3 then cast ( 1 as bit ) else cast (0 as bit) end as DoorToDoor,
							CCT.ContainerCount,IP.StatusKey
							--case when @DoorToDoor = 1 then cast ( 1 as bit ) else cast (0 as bit) end as DoorToDoor
			FROM			InvoiceHeader  IH WITH (NOLOCK)
	...
	*/
	...
	SELECT			A.OrderKey,
					...
					isnull(ExpCount,0) as ExpCount,DoorToDoor,ContainerCount
