...
(	SlNo						INT,
	...
	SELECT					ROW_NUMBER() OVER(ORDER BY City),Market,Terminal,City,State,ZipCode,Zone,Prepulllocation1
							,CASE WHEN Prepullcost1 = '0.0' THEN '' ELSE Prepullcost1 END,Prepulllocation2
							,CASE WHEN Prepullcost2 = '0.0' THEN '' ELSE Prepullcost2 END,Prepulllocation3
							,CASE WHEN Prepullcost3 = '0.0' THEN '' ELSE Prepullcost3 END,Prepulllocation4
							,CASE WHEN Prepullcost4 = '0.0' THEN '' ELSE Prepullcost4 END,Prepulllocation5
							,CASE WHEN Prepullcost5 = '0.0' THEN '' ELSE Prepullcost5 END,Stopofflocation1
							,CASE WHEN Stopoffcost1 = '0.0' THEN '' ELSE Stopoffcost1 END,Stopofflocation2
							,CASE WHEN Stopoffcost2 = '0.0' THEN '' ELSE Stopoffcost2 END,Stopofflocation3
							,CASE WHEN Stopoffcost3 = '0.0' THEN '' ELSE Stopoffcost3 END,Stopofflocation4
							,CASE WHEN Stopoffcost4 = '0.0' THEN '' ELSE Stopoffcost4 END,Stopofflocation5
							,CASE WHEN Stopoffcost5 = '0.0' THEN '' ELSE Stopoffcost5 END,YardshuttledirectionTO1	,YardshuttledirectionFROM1
							,CASE WHEN Yardshuttlecost1 = '0.0' THEN '' ELSE Yardshuttlecost1 END,YardshuttledirectionTO2,YardshuttledirectionFROM2
							,CASE WHEN Yardshuttlecost2 = '0.0' THEN '' ELSE Yardshuttlecost2 END
							,TruckTypeA
	...
	WHILE(@i < = @n)
		BEGIN
			...
			ELSE
				BEGIN
					IF((SELECT COUNT(*) FROM locationdata WHERE City = @City AND State = @State) = 0 )
						BEGIN
							SET @ISSuccess = 0
							SET	@RecordStatus = 0
					...
					ELSE IF ((SELECT COUNT(*) FROM locationdata WHERE City = @City AND State = @State) = 1 )
						BEGIN
							SET @Zipcode = (SELECT ZipCode FROM locationdata WHERE City = @City AND State = @State)
						END
				END
			...
			IF (ISNULL(@Prepulllocation2,'') <> '')
				BEGIN					
					...
					-- LEFT OUTER JOIN #Yard Y ON ( TP.Textvalue = RTRIM(LTRIM(REPLACE(REPLACE(Y.ShortName,'JCT',''),'-','')))  OR RTRIM(LTRIM(TP.Textvalue)) = RTRIM(LTRIM(Y.YardType)))
					LEFT OUTER JOIN #Yard Y ON ( TP.Textvalue = Y.YardType )
					WHERE YardType is NULL)
...
-------------------------------------------------------Truck Type A -------------------------------------
			...
			IF(ISNULL(@TruckTypeABaseCost1 ,'') = '' )
				...
				END
			ELSE IF(ISNUMERIC(@TruckTypeABaseCost1) = 0)
				BEGIN
					SET @ISSuccess = 0
					SET	@RecordStatus = 0
			...
			ELSE
				BEGIN
					SET @TruckTypeABaseCost1 = CAST(@TruckTypeABaseCost1 AS DECIMAL(18,2))
				END

			IF(ISNULL(@TruckTypeAFSF1,'') <> '' AND ISNUMERIC(@TruckTypeAFSF1) = 0)
				BEGIN
					SET @ISSuccess = 0
					SET	@RecordStatus = 0
			...
			ELSE IF (ISNUMERIC(@TruckTypeAFSF1) = 1)
				BEGIN
					SET @TruckTypeAFSF1 = CAST(@TruckTypeAFSF1 AS DECIMAL(18,2))
				END
...
-------------------------------------------------------Truck Type B -------------------------------------
	...
	IF(ISNULL(@TruckTypeB ,'') = '' )
				BEGIN
					SET @ISSuccess = 0
					SET	@RecordStatus = 0
					SET	@RecordRemarks = @RecordRemarks + 'Truck Type B Cannot be blank;'
				END
			ELSE IF ((SELECT COUNT(*) FROM TruckType WHERE TruckType = @TruckTypeB ) = 0)
				BEGIN
					SET @ISSuccess = 0
					SET	@RecordStatus = 0
					SET	@RecordRemarks = @RecordRemarks + 'Truck Type B does not exists in Masters;'
				END

			IF(ISNULL(@TruckTypeBBaseCost1 ,'') = '' )
				BEGIN
					SET @ISSuccess = 0
					SET	@RecordStatus = 0
					SET	@RecordRemarks = @RecordRemarks + 'Truck Type B Base Cost 1 Cannot be blank;'
				END
			ELSE IF(ISNUMERIC(@TruckTypeBBaseCost1) = 0)
				BEGIN
					SET @ISSuccess = 0
					SET	@RecordStatus = 0
			...
			ELSE
				BEGIN
					SET @TruckTypeBBaseCost1 = CAST(@TruckTypeBBaseCost1 AS DECIMAL(18,2))
				END

			IF(ISNULL(@TruckTypeBFSC1,'') <> '' AND ISNUMERIC(@TruckTypeBFSC1) = 0)
				BEGIN
					SET @ISSuccess = 0
					SET	@RecordStatus = 0
			...
			ELSE IF (ISNUMERIC(@TruckTypeBFSC1) = 1)
				BEGIN
					SET @TruckTypeBFSC1 = CAST(@TruckTypeBFSC1 AS DECIMAL(18,2))
				END
		
		SELECT 'Truck A',@TruckTypeAFSF1
		 SELECT 'Truck B',@TruckTypeBFSC1
			...
			ELSE 
				BEGIN					
					...
					SET @TempRemarks = (SELECT DISTINCT 'Truck Type B From 1 Yard Details;' FROM #TEMPPROCESS TP
					LEFT OUTER JOIN #Yard Y ON ( TP.Textvalue = Y.YardType )
					WHERE YardType is NULL)
...
Print 2
	...
	Print 3
		INSERT INTO				COST_FileUploadData_Chicago
								(FileProcesskey,RecordSL,Market
								,Terminal,City,State,ZipCode,Zone,Prepulllocation1
								,Prepullcost1,Prepulllocation2,Prepullcost2,Prepulllocation3,Prepullcost3,Prepulllocation4,Prepullcost4,Prepulllocation5
								,Prepullcost5,Stopofflocation1,Stopoffcost1,Stopofflocation2,Stopoffcost2,Stopofflocation3,Stopoffcost3,Stopofflocation4,Stopoffcost4,Stopofflocation5,Stopoffcost5,YardshuttledirectionTO1
								,YardshuttledirectionFROM1,Yardshuttlecost1,YardshuttledirectionTO2,YardshuttledirectionFROM2,Yardshuttlecost2
								,TruckTypeA,TruckTypeABaseCost1,TruckTypeAFSF1,TruckTypeAFROM1,TruckTypeABaseCost2,TruckTypeAFSF2,TruckTypeAFROM2
								,TruckTypeB,TruckTypeBBaseCost1,TruckTypeBFSC1,TruckTypeBFROM1,TruckTypeBBaseCost2,TruckTypeBFSC2,TruckTypeBFROM2
		...
		SELECT					@FileProcesskey,SlNo,Market
								,Terminal,City,State,@ZipCode,Zone,Prepulllocation1
								,@Prepullcost1,Prepulllocation2,@Prepullcost2,Prepulllocation3,@Prepullcost3,Prepulllocation4,@Prepullcost4,Prepulllocation5
								,@Prepullcost5,Stopofflocation1,@Stopoffcost1,Stopofflocation2,@Stopoffcost2,Stopofflocation3,@Stopoffcost3,Stopofflocation4,@Stopoffcost4,Stopofflocation5,@Stopoffcost5,YardshuttledirectionTO1
								,YardshuttledirectionFROM1,@Yardshuttlecost1,YardshuttledirectionTO2,YardshuttledirectionFROM2,@Yardshuttlecost2
	...
	SELECT					A.*, FileProcesskey
							,RecordStatus,RecordRemarks
							,Market,Terminal,City,State	,ZipCode,Zone,Prepulllocation1,Prepullcost1,Prepulllocation2,Prepullcost2,Prepulllocation3,Prepullcost3,Prepulllocation4,Prepullcost4,Prepulllocation5
							,Prepullcost5,Stopofflocation1,Stopoffcost1,Stopofflocation2,Stopoffcost2,Stopofflocation3,Stopoffcost3,Stopofflocation4,Stopoffcost4,Stopofflocation5,Stopoffcost5,YardshuttledirectionTO1
							,YardshuttledirectionFROM1,Yardshuttlecost1,YardshuttledirectionTO2,YardshuttledirectionFROM2,Yardshuttlecost2
							,TruckTypeA,@TruckTypeABaseCost1,@TruckTypeAFSF1,TruckTypeAFROM1,@TruckTypeABaseCost2,@TruckTypeAFSF2,TruckTypeAFROM2
							,TruckTypeB,@TruckTypeBBaseCost1,@TruckTypeBFSC1,TruckTypeBFROM1,@TruckTypeBBaseCost2,@TruckTypeBFSC2,TruckTypeBFROM2
