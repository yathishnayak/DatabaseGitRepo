...
--select * from driver where DriverID = '05-SR'

Create PROCEDURE [dbo].[Get_DriverManifestList_Ash] --Get_DriverManifestList @StatusKey = 2, @driverKey = 529
@StatusKey           INT= 0,
@DriverKey           INT= 0,
@OrderKey            INT= 0,
...
BEGIN
       ...
       union all 
		...
		UNION ALL
 			  select distinct  DriverID,  FirstName,    LastName,      VH.DriverVoucherNumber,  Vh.DriverVoucherdate,       
				'' as PickUpPoint, '' as DeliveryPoint, '' as     ContainerNo,  
              I.ItemID,     0.00 ExtCost,   VD.Qty,       0.00 UnitCost, 
			  convert(bit,0) IsPaymentApproved, convert(smallint, isnull(@StatusKey,2)) StatusKey,       
			  VH.DriverVoucherAmount,   0  IsInvoiced,   
			  '' InvoiceNo,       GETDATE() InvoiceDate,  
              VD.Description,     0 InvoiceKey,  VH.DriverVoucherKey Voucherkey,    ''   LegID, '' FromLocation, 
			  '' ToLocation,   VH.DriverKey,     ''  DrivingLicenseNo,    
              null DrivingLicenseExpiryDate,  null ActualArrival,      'WK-'+ convert(varchar,VH.WeekNumber) Weeknum, 
			  VD.ExtCost as APDeductions,     0 OrderDetailKey,       '' DriverOrg,   GETDATE() Week_Start_Date,     
              GETDATE() Week_End_Date        
              from   
	...
	order by DriverID, ActualArrival,OrderDetailKey, ContainerNo, LegID, ItemID
