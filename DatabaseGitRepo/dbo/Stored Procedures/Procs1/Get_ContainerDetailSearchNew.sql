...
--select * from int_containerAvailability

--[dbo].[Get_ContainerDetailSearch]  '2021-05-05','2021-05-08'
--  [Get_ContainerDetailSearch_new]  @CSRKey =1
-- [Get_ContainerDetailSearch] @IsTransLoad = 0
CREATE PROCEDURE [dbo].[Get_ContainerDetailSearchNew]   -- [Get_ContainerDetailSearchNew] @statusKey = 0, @searchText = 'ITS'
...
BEGIN
	...
	WHERE  
		...
		OR RT.PickupDate  between @PickupDateFrom and @PickupDateTo OR 
		RT.PickupDateTo between @PickupDateFrom and @PickupDateTo)
		AND ( @DeleveryDateFrom	IS NULL OR RT.DeliveryDate IS NULL OR RT.DeliveryDate>=@DeleveryDateFrom)
		AND ( @DeleveryDateTo	IS NULL OR RT.DeliveryDateTo IS NULL OR RT.DeliveryDateTo<=@DeleveryDateTo)	
		AND ( @CSRKey			IS NULL OR @CSRKey= 0 OR OH.CsrKey= @CSRKey)	
		AND ( @statusKey		IS NULL OR @statusKey = 0 OR OD.[Status] = @statusKey)
		AND ( @IsTransLoad = 0 OR OD.Status <> @OpenStatusKey)
		AND (isnull(@CustomerKey,0) = 0 OR CU.CustKey = @CustomerKey)
		AND (isnull(@CreatedUSer,'') = '' OR UU.UserName like @CreatedUSer + '%')

		CREATE CLUSTERED INDEX ix_tempCLD ON #ContainerListData (OrderDetailkey, OrderKey);
		...
		declare @PostDate datetime = Convert(datetime, '2050-12-31')

		SELECT ROW_NUMBER() OVER ( PARTITION BY A.Orderdetailkey ORDER BY isnull(W.ActualArrival, @Postdate)) AS LegNo,
				A.OrderDetailKey,W.RouteKey 
		INTO #RouteLegNo
		FROM #ContainerListData A 
			INNER JOIN dbo.Routes W WITH (NOLOCK)	ON W.OrderDetailKey=A.OrderDetailKey

		CREATE CLUSTERED INDEX ix_tempRLN ON #RouteLegNo (OrderDetailkey, RouteKey, LegNo);

		SELECT DISTINCT OrderDetailkey INTO #OrderDetl FROM #ContainerListData

		SELECT A.OrderDetailKey,COUNT(DISTINCT RT.RouteKey) AS LegCount 
		INTO #LegCount
		FROM #OrderDetl A 
		...
		declare @LegCompletedStatusKey int = 0
		select @LegCompletedStatusKey = RS.Status  from RouteStatus RS where Description = 'Leg Completed'
		...
		WHERE RT.Status = @LegCompletedStatusKey
		GROUP BY A.OrderDetailKey
		...
		declare @ContainerConfirmedStatusKey int	 = 0
		select @ContainerConfirmedStatusKey = ODS.Status from OrderDetailStatus ODS where [Description]='Dispatch Confirmed'

		SELECT A.OrderDetailKey,'Dispatch Complete' AS CompleteStatus, @ContainerConfirmedStatusKey as CompleteStatusKey 
		INTO #CompletedCont
		FROM #OrderDetl A 
			INNER JOIN dbo.OrderDetail D		WITH (NOLOCK)	ON D.OrderDetailKey=A.OrderDetailKey
			INNER JOIN dbo.Routes RT			WITH (NOLOCK)	ON RT.OrderDetailKey=A.OrderDetailKey
		WHERE D.Status = @ContainerConfirmedStatusKey AND RT.Status = @LegCompletedStatusKey
		GROUP BY A.OrderDetailKey
		...
		SELECT MAX(RouteKey) AS RouteKey, A.OrderDetailKey 
		INTO #AllComplLegdestloc 
		...
		SELECT A.OrderDetailKey, MAX(RT.RouteKey) AS RouteKey 
		INTO #mixedLeg
		FROM #OrderDetl A 
			INNER JOIN dbo.Routes RT		WITH (NOLOCK)	ON RT.OrderDetailKey=A.OrderDetailKey
		WHERE A.OrderDetailKey IN ( SELECT OrderDetailKey FROM #CompletedLeg ) 
			...
			AND RT.Status = @LegCompletedStatusKey
		GROUP by A.OrderDetailKey

		SELECT A.OrderDetailKey,DT.AddrName AS ContCurrLocation,A.RouteKey,2 AS LocType 
		INTO #CurrntLoc3
		FROM #mixedLeg A 
		INNER JOIN dbo.Routes RT	WITH (NOLOCK)	ON RT.RouteKey=A.RouteKey
		INNER JOIN dbo.[Address] DT WITH (NOLOCK)	ON DT.AddrKey=RT.DestinationAddrKey		
		...
		*/
		...
		select CL.OrderDetailkey, 1 AS IsHazmat 
		INTO #HazCont 
		from #ContainerListData CL
		inner join vOrderContainerTypes CT on CL.OrderDetailkey = CT.OrderDetailKey
		where CT.Description = 'Hazard'
		...
		SELECT OrderKey,A.OrderDate,A.OrderDetailkey,A.OrderTypeKey,A.OrderNo,A.CsrName,
			...
			A.NextAction,A.custKey,A.BrokerName,A.[Weight],A.VesselName,A.SealNo,A.CutOffDate,
			...
			--ELSE (CASE WHEN CompleteStatus='Dispatch Confirmed' THEN 0 ELSE ISNULL(MN.LegNo,0) END) END AS VARCHAR(50)) +' of '+ CAST(LegCount AS VARCHAR(50)) END AS CurLeg
			 isTransLoad
			, A.CustID, A.CustName, A.BrokerRefNo, A.VesselETA, A.CreatedUser
			INTO #TempOutputPrev
			FROM #ContainerListData A 
		...
		--WHERE A.ContainerNo='MSCU5187009'
		WHERE  (@IsTransLoad = 0 or A.isTransLoad = @IsTransLoad) --AND
			--(isnull(@statusKey,0) = 0 OR StatusKey = @statusKey)


		select *, convert(Date,'2020-01-01') as Int_LFD, convert(Bit, 0) as IntDataExists 
		into #TempOutput
		from #TempOutputPrev A
		where (isnull(@SearchText ,'') = '') OR (
			A.OrderNo like '%' +  @SearchText + '%' OR
			A.CurLocation like '%' +  @SearchText + '%' OR
			a.B_AddrName like '%' +  @SearchText + '%' OR
			A.D_AddrName like '%' +  @SearchText + '%' OR
			A.CsrName like '%' +  @SearchText + '%' OR
			A.ContainerNo like '%' +  @SearchText + '%' OR
			A.BookingNo like '%' +  @SearchText + '%' OR
			A.BillOfLading like '%' +  @SearchText + '%' OR
			A.OrderType like '%' +  @SearchText + '%' OR
			A.BrokerRefNo like '%' +  @SearchText + '%' OR
			A.CustName like '%' +  @SearchText + '%' 
		)

		update A
			Set Int_LFD = B.LastFreeDay, IntDataExists = convert(bit, 1)
		from #TempOutput A
		inner join Int_ContainerAvailability B on A.OrderDetailkey  = B.OrderDetailKey
		--where B.LastFreeDay > convert(Date, '2020-01-01')
