...
CREATE   PROCEDURE [dbo].[COST_MoveCostOutputData] -- COST_MoveCostOutputData 139
(
	@FileProcessKey		INT
)
AS
BEGIN
		...
		---------------------------------------------Truck Type From---------------------------------------------------------------------------------
			...
			SELECT '#DATARecordSL1', * FROM #DATA WHERE RecordSL = 1
			WHILE (@i <= @n)
			BEGIN
				SET @FileProcessKey = (SELECT FileProcessKey FROM #DATA WHERE SlNO = @i  )
				SET @RecordSL = (SELECT RecordSL FROM #DATA WHERE SlNO = @i  )
...
--WHERE		ISNULL(TruckTypeB,'') <> ''  AND ISNULL(TruckTypeBFROM1,'') <> ''
	...
	IF(@CNT > 0)
		BEGIN
			SELECT			*
			INTO			#TMPDATA
			FROM			(SELECT		A.FileProcessKey, A.RecordSL, Market,B.Terminal,City,State,ZipCode,Zone, TruckTypeA DriverType
							...
							UNION ALL
							SELECT		A.FileProcessKey, A.RecordSL, Market,B.Terminal,City,State,ZipCode,Zone, TruckTypeA DriverType
							...
							UNION ALL
							SELECT		DISTINCT A.FileProcessKey, A.RecordSL, Market,B.Terminal,City,State,ZipCode,Zone, TruckTypeB DriverType
							...
							UNION ALL
							SELECT		DISTINCT A.FileProcessKey, A.RecordSL, Market,B.Terminal,City,State,ZipCode,Zone, TruckTypeC DriverType
							...
							UNION ALL
							SELECT		DISTINCT A.FileProcessKey, A.RecordSL, Market,B.Terminal,City,State,ZipCode,Zone, TruckTypeD DriverType
			...
			INSERT INTO		COST_CostDataOutput
							(FileProcessKey,RecordSL,Market,Terminal,City,State,ZipCode,Zone,DriverType,YardPortType,Cost,FSFCost,FSF,DrayBase,EffectiveDate,EffectiveDateFrom)
			SELECT			A.FileProcessKey,A.RecordSL,A.Market,A.Terminal,A.City,A.State,A.ZipCode,A.Zone,A.DriverType,TruckTypeFROM,A.Cost,A.FSF
