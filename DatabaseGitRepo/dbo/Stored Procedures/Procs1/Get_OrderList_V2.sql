...
DECLARE 
	@UserKey INT=714,
	@JSONString NVARCHAR(MAX)='{"CustomerKeys":"","CSRKeys":"75","CreatedUserKeys":"","MarketLocationKeys":"","PropertyKeys":"","EntryType":"","OrderTypeKey":"","StatusKey":12,"SearchText":"","PageNo":1,"PageSize":50,"IsAscending":false,"SortField":"CreateDate","OutputType":"","Addrkey":0,"SearchCriteriaKey":0}',
	@Status BIT=0,@IsDebug		BIT = 1,
	@Reason VARCHAR(100)=''
EXEC [Get_OrderList_V2] @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @IsDebug
Select @Status, @Reason
**/
CREATE PRocedure [dbo].[Get_OrderList_V2] -- [Get_OrderList_V2] @StatusKey=2, @PageNo = 1,@SearchText='WHSU5296890'
(
	@UserKey		INT = 714,
...
--	SELECT top 1000 OrderKey FROM OrderHeader WHERE Status<>14 
--END
--ELSE
--BEGIN
--	INSERT INTO #FilteredOrderKeys
...
IF(ISNULL(@SearchText, '') <> '')
 BEGIN
	IF(Charindex(',',@SearchText)=0)
		BEGIN
			...
			inner join OrderHeader OH WITH (NOLOCK) on OD.orderKey = OH.orderKey
			where ContainerNo= @SearchText OR 
					OrderNo = @SearchText OR 
					BillOfLading =@SearchText OR 
					ISNULL(OD.BookingNo,OH.BookingNo) = @SearchText  OR
					BrokerRefNo =@SearchText 
			ORDER BY OH.OrderKey
		END
		ELSE
		Begin
			print '@SearchText not null'
			print @SearchText 
		...
		BEGIN
			...
			SELECT DISTINCT OH.OrderKey
				FROM OrderHeader OH WITH (NOLOCK)
				INNER JOIN OrderDetail OD WITH (NOLOCK) on OD.OrderKey = OH.OrderKey
				WHERE ISNULL(OH.BillOfLading,OD.BillOfLadding) IN (SELECT VALUE FROM fn_splitparam(@SearchText))
		END
		END
 END 
 ELSE
 ...
 IF(@IsDebug = 1)
 BEGIN
	SELECT '#FilteredOrderKeys', * FROM #FilteredOrderKeys
 END
  ...
  --          ), 1, 1, '') AS Properties
    ...
    FROM #FilteredOrderKeys FOH 
		...
		OUTER APPLY (SELECT TOP 1 *
                    ...
                    WITH (NOLOCK) WHERE OrderKey=OH.OrderKey
                    ORDER  BY OH.OrderNo) OD
        LEFT JOIN dbo.Customer CUS	 with ( NOLOCK)		ON CUS.CustKey = OH.CustKey      
        LEFT JOIN dbo.[Broker] BR	 with ( NOLOCK)		ON OH.BrokerKey = BR.BrokerKey
        LEFT JOIN dbo.OrderType OT	 with ( NOLOCK)		ON OH.OrderTypeKey = OT.OrderTypeKey   
        LEFT JOIN dbo.OrderStatus OS with ( NOLOCK)		ON OS.[Status] = OH.[Status]
	...
	WHERE 
		...
		AND ( ISNULL(@CSRKeys,'')='' OR O.CsrKey in (Select CSRKey from #CSRKeyList) )	
	...
	--SELECT			S.status as StatusKey, S.Description , ISNULL(count(1),0) AS OrderCount,OrderBy
	--INTO			#DashBoard
	--FROM			OrderStatus S WITH (NOLOCK)
	--LEFT JOIN		#OrderListCountData Z on S.Status = Z.Status
	--where S.IsActive = 1
	--GROUP BY S.Status, S.Description , OrderBy

	--insert into #DashBoard (StatusKey,Description, OrderCount,OrderBy  )
	--select 100, 'All Orders', sum(Ordercount), 10 from #Dashboard 

	-- When search is active, map the found order's status to its tab group
-- StatusKey 12 = Open tab, StatusKey 1 = In Progress tab, StatusKey 9 = Completed tab
...
-- Added by Ruthu
	...
	BEGIN
		...
		SELECT  S.status,
				S.Description,
				CASE 
					WHEN S.Status = 12 AND @FoundStatus = 12 THEN 1
					WHEN S.Status = 1  AND @FoundStatus = 1  THEN 1
					WHEN S.Status = 9  AND @FoundStatus = 9  THEN 1
					ELSE 0
				END,
				S.OrderBy
		FROM    OrderStatus S WITH (NOLOCK)
		...
		INSERT INTO #DashBoard (StatusKey, Description, OrderCount, OrderBy)
		SELECT  100, 'All Orders',
				CASE WHEN @FoundStatus IN (12, 1, 9) THEN 1 ELSE 0 END,
				10
	END
	ELSE
	BEGIN
		INSERT INTO #DashBoard (StatusKey, Description, OrderCount, OrderBy)
		SELECT  S.status, S.Description, ISNULL(COUNT(Z.Status), 0), S.OrderBy
		FROM    OrderStatus S WITH (NOLOCK)
		LEFT JOIN #OrderListCountData Z ON S.Status = Z.Status
		WHERE   S.IsActive = 1
		GROUP BY S.Status, S.Description, S.OrderBy
...
-- Till here
	...
	BEGIN
		...
		SELECT '#ContainerCount', * from #ContainerCount where ContainerNos = 'ACER250101390'
	END
	...
	Begin
		...
		DropDowns = ( SELECT
			CustomerList	=	(SELECT DISTINCT CustKey, CustName FROM  Customer  WITH (NOLOCK)
									WHERE -- IsSelectedStatusKey = 1 AND 
									ISNULL(CustName,'')<>''	ORDER BY CustName FOR JSON PATH),
			...
			CSRList			=	(SELECT DISTINCT CSRKey AS CsrKey, CSRName AS CsrName FROM CSR WHERE CSRName IS NOT NULL AND IsActive = 1
									AND LTRIM(RTRIM(CsrName)) <> ''
									ORDER BY CsrName FOR JSON PATH ),
			MarketLocList	=	(SELECT DISTINCT MarketLocationKey,MarketLocation FROM MarketLocation WITH (NOLOCK) 
									WHERE -- IsSelectedStatusKey = 1 AND 
									ISNULL(MarketLocation,'')<>''	ORDER BY MarketLocation FOR JSON PATH ),
			CreatedUserList =	(SELECT DISTINCT UserKey AS CreateUserKey,UserName AS CreatedUsedName FROM [User]  WITH (NOLOCK)
									WHERE -- IsSelectedStatusKey = 1 AND 
									ISNULL(UserName,'')<>'' ORDER BY UserName FOR JSON PATH ),
			--(SELECT DISTINCT CreateUserKey,CreatedUsedName FROM #FinalData_Temp  WITH (NOLOCK)
			--						WHERE -- IsSelectedStatusKey = 1 AND 
			--						ISNULL(CreatedUsedName,'')<>'' ORDER BY CreatedUsedName FOR JSON PATH ),
			WarehouseStatus =   (SELECT DISTINCT StatusKey,Description from WarehouseStatus Order by StatusKey For JSON PATH ),
			PropertiesList	=   (SELECT DISTINCT A.ContainerTypeKey,A.TypeID from ContainerTypes A Order by A.TypeID For JSON PATH ),
		...
		Dashboard = (
			SELECT * FROM #DashBoard WITH (NOLOCK) Order by OrderBy 
			For JSON PATH
		)
	END
	ELSE
	BEGIN
		...
		DropDowns = ( SELECT
			CustomerList	=	(SELECT DISTINCT CustKey, CustName FROM  Customer  WITH (NOLOCK)
									WHERE -- IsSelectedStatusKey = 1 AND 
									ISNULL(CustName,'')<>''	ORDER BY CustName FOR JSON PATH),
			--CSRList			=	(SELECT DISTINCT CSRKey as CsrKey, CSRName as CsrName FROM #FinalData_Temp  WITH (NOLOCK)
			--						WHERE -- IsSelectedStatusKey = 1 AND 
			--						ISNULL(CSRName,'')<>''	ORDER BY CSRName FOR JSON PATH ),
			CSRList         =	(SELECT DISTINCT CSRKey AS CsrKey, CSRName AS CsrName FROM CSR WHERE CsrName IS NOT NULL AND IsActive = 1
									AND LTRIM(RTRIM(CsrName)) <> ''
									ORDER BY CsrName FOR JSON PATH ),
			MarketLocList	=	(SELECT DISTINCT MarketLocationKey,MarketLocation FROM MarketLocation WITH (NOLOCK) 
