...
DECLARE 
	...
	@JSONString NVARCHAR(MAX)='{"DriverKey":0,"StatusKey":9,"VoucherNo":"","ContainerNo":"","OrderNo":"", "PageNumber" : 1, "PageSize" : 50}',
	@Status BIT=0, 
	@Debug INT = 1,
	@Reason VARCHAR(100)=''
EXEC Get_DriverDispatchList_V3 @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @Debug
...
DECLARE 
	@UserKey INT=1144,
	@JSONString NVARCHAR(MAX)='{"DriverKey":"1679","OrderDateFrom":"2025-06-14T18:30:00.000Z","OrderDateTo":"2025-06-29T18:30:00.000Z","OrderKey":0,"StatusKey":9,"VoucherNo":"M-00065","ContainerNo":"","OrderNo":"","DispatchDateFrom":null,"DispatchDateTo":null,"PageNumber":1,"PageSize":25}',
	@Status BIT=0, 
	@Debug INT = 1,
	@Reason VARCHAR(100)=''
EXEC Get_DriverDispatchList_V3 @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @Debug
...
DECLARE 
	@UserKey INT=1144,
	@JSONString NVARCHAR(MAX)='{"DriverKey":0,"OrderDateFrom":"2026-03-31T18:30:00.000Z","OrderDateTo":"2026-04-28T18:30:00.000Z","OrderKey":0,"StatusKey":9,"VoucherNo":"","ContainerNo":"","OrderNo":"","PageNumber":1,"PageSize":25}',
	@Status BIT=0, 
	@Debug INT = 1,
	@Reason VARCHAR(100)=''
EXEC Get_DriverDispatchList_V3 @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @Debug
Select @Status, @Reason
*/
CREATE PROCEDURE [dbo].[Get_DriverDispatchList_V3] 
(
	@UserKey		INT,
...
BEGIN
	...
	SELECT 
		...
		@DriverKey = ISNULL(DriverKey, 0),
		-- @OrderKey = ISNULL(OrderKey, 0), 
		@OrderDateFrom = ISNULL(OrderDateFrom, '01/01/2020'),
		@OrderDateTo = ISNULL(OrderDateTo, '12/31/2099') , 
		@DeliveryDateFrom = ISNULL(DeliveryDateFrom, '01/01/2020'),
		@DeliveryDateTo = ISNULL(DeliveryDateTo, '12/31/2099'), 
		@OrderNo = OrderNo,
		@ContainerNo = ISNULL(ContainerNo, ''), 
		@VoucherNo = ISNULL(VoucherNo, ''),
		-- @VoucherKey = VoucherKey, 
		@DriverHubKey = DriverHubKey,
		-- @WeekNum = WeekNum, 
	...
	WITH(
		...
		DeliveryDateFrom	DATE			'$.DispatchDateFrom',
		DeliveryDateTo		DATE			'$.DispatchDateTo',
	...
	IF(@OrderDateFrom < CONVERT(DATE,GETDATE()-365) and @DeliveryDateFrom < CONVERT(DATE,GETDATE()-365)
			and ISNULL(@OrderNo,'')='' and ISNULL(@containerNo,'')=''  and ISNULL(@voucherNo,'')=''  and ISNULL(@VoucherKey,0)=0  and ISNULL(@DriverKey,0)=0)
	...
	BEGIN
		SET @OrderDateFrom = GetDate() - 60  --60
		SET @OrderDateTO = '2050-12-31'
		SET @DeliveryDateFrom = GetDate() - 30  --30
		SET @DeliveryDateTo = '2050-12-31'
	END
	IF(ISNULL(@WeekNum ,'')<>'')
	BEGIN
	...
	SELECT @OpenStatusKey = STATUS FROM RouteStatus WHERE DESCRIPTION = 'Leg Completed' -- @OpenStatusKey = 5
	...
	WHERE 	RTS.Status= @OpenStatusKey	and  VH.VoucherKey IS not NULL AND OD.Status != 15		
	...
	BEGIN
		...
		FROM dbo.[routes] RT WITH (NOLOCK) 
			...
			LEFT JOIN MarketLocation ML WITH (NOLOCK) ON OH.MarketLocationKey =  ML.MarketLocationKey
		WHERE 	 VH.VoucherKey IS NULL AND RT.ActualArrival IS NOT NULL	AND OD.Status != 15
	...
	SELECT
				...
				convert(datetime,isnull(voucherdate,'01-01-1900')) as VoucherDare,
