/*
declare @UserKey        INT,
    @JSONString     NVARCHAR(MAX)='{"SearchCriteriaKey":1,"DriverKeys":"","OrderKeys":"","OrderNo":"","containerNo":"","voucherNo":"","VoucherKeys":"","DriverHubkeys":"","WeekNum":"","MarketLocationKeys":"","TruckTypeKeys":"","CarrierMoveTypeKeys":"","SearchText":"TGCU5240826","SortField":"voucherno","IsAscending":true,"PageSize":50,"PageNo":1,"StatusKey":4,"isDriverPay":false}',
    @Status         BIT = 0 ,
    @Reason         VARCHAR(1000) = '',
    @IsDebug        BIT = 1

	exec Get_VoucherList_V2 @UserKey,@JSONString,@Status output,@Reason output,@IsDebug
	*/


-- Created by GitHub Copilot in SSMS - review carefully before executing
CREATE PROCEDURE [dbo].[Get_VoucherList_V2]
(
    @UserKey        INT,
...
BEGIN
    ...
    DECLARE 
        ...
        @isDriverPay BIT = 0, @SearchCriteriaKey INT = 0, @IsWithFilter BIT = 0,
        @IsSearchActive BIT = 0, @OpenStatusKey SMALLINT = 0;

    SELECT  
        @containerNo = ISNULL(ContainerNo, ''), @StatusKey = StatusKey,
        ...
        @OrderNo = OrderNo, @voucherNo = voucherNo, @VoucherKeys = VoucherKeys,
        @DriverHubkeys = DriverHubkeys, @WeekNum = WeekNum,
        @MarketLocationKeys = MarketLocationKeys, @TruckTypeKeys = TruckTypeKeys,
        ...
        @IsAscending = ISNULL(IsAscending, 1), @isDriverPay = isDriverPay,
    ...
    IF ISNULL(@voucherNo, '') <> '' OR ISNULL(@containerNo, '') <> '' OR ISNULL(@OrderNo, '') <> '' OR ISNULL(@SearchText,'')<>''
        SET @IsWithFilter = 1;


    IF  @OrderDateFrom='' OR @OrderDateFrom IS NULL OR @OrderDateFrom IN ('0001-01-01', '1900-01-01')
        SET @OrderDateFrom = CASE WHEN @IsWithFilter = 0 THEN DATEADD(DAY, -90, GETDATE()) ELSE DATEADD(DAY, -180, GETDATE()) END;
    ...
    IF @DeliveryDateFrom='' OR @DeliveryDateFrom IS NULL OR @DeliveryDateFrom IN ('0001-01-01', '1900-01-01')
        SET @DeliveryDateFrom = CASE WHEN @IsWithFilter = 0 THEN DATEADD(DAY, -90, GETDATE()) ELSE DATEADD(DAY, -180, GETDATE()) END;
    ...
    DECLARE @VoucherDateThreshold DATE = DATEADD(DAY, CASE WHEN @isDriverPay = 1 THEN -90 ELSE -60 END, GETDATE());
    ...
    -- ====================================================================
    -- 1. PRE-FILTER DRIVER TABLE (1.2K rows -> subset)
    -- ====================================================================
    ...
    BEGIN
        ...
        BEGIN
            IF EXISTS (SELECT 1 FROM dbo.VoucherHeader WITH (NOLOCK) WHERE VoucherNo = @SearchText)
                INSERT INTO #VoucherKey SELECT VoucherKey FROM dbo.VoucherHeader WITH (NOLOCK) WHERE VoucherNo = @SearchText;
            ELSE
            BEGIN
                INSERT INTO #OrderDetailKeys SELECT OrderDetailKey FROM dbo.OrderDetail WITH (NOLOCK) WHERE ContainerNo = @SearchText;
        ...
        BEGIN
            IF @SearchCriteriaKey = 1
                INSERT INTO #OrderDetailKeys SELECT DISTINCT OrderDetailKey FROM dbo.OrderDetail WITH (NOLOCK)
    ...
    ELSE IF @StatusKey = 9
    BEGIN
        SET @DeliveryDateFrom = DATEADD(DAY, -30, GETDATE());
        SET @DeliveryDateTo = DATEADD(DAY, 5, GETDATE());--GETDATE();
    END

    -- ====================================================================
    -- 2. PRE-FILTER ORDERHEADER TABLE (198K rows -> subset by date range)
    -- ====================================================================
    ...
    -- ====================================================================
    -- 3. PRE-FILTER ORDERDETAIL TABLE (234K rows -> subset)
    -- ====================================================================
    ...
    -- ====================================================================
    -- 4. PRE-FILTER ROUTES TABLE (695K rows -> subset by Status + Driver)
    -- ====================================================================
    ...
    -- ====================================================================
    -- 5. PRE-FILTER VOUCHERHEADER TABLE (235K rows -> subset by date)
    -- ====================================================================
    ...
    WHERE (@IsWithFilter=1 OR VH.VoucherDate > @VoucherDateThreshold)
      AND (@IsSearchActive = 0 OR VH.VoucherKey IN (SELECT VoucherKey FROM #VoucherKey) 
           OR EXISTS (SELECT 1 FROM #OrderDetailKeys));

    -- ====================================================================
    -- 6. PRE-FILTER ROUTEVOUCHERS TABLE (295K rows -> subset)
    -- ====================================================================
    ...
    -- ====================================================================
    -- 7. PRE-AGGREGATE VOUCHER VIEW DATA (eliminates multiple view scans)
    -- ====================================================================
    ...
    -- ====================================================================
    -- 8. PRE-COMPUTE WEEK DATES (replaces CROSS APPLY)
    -- ====================================================================
    ...
    SELECT DISTINCT CAST(FR.ActualArrival AS DATE),
    ...
    CREATE TABLE #TEMPTABLE (
        orderkey INT, orderdetailkey INT, voucheramount NUMERIC(18,5), routekey INT,
        destinationaddrkey INT, voucherkey INT, StatusKey SMALLINT, DocumentCount INT,
        DocCounts VARCHAR(50), orderno VARCHAR(50), containerno VARCHAR(50),
        driverid VARCHAR(20), firstname VARCHAR(100), lastname VARCHAR(100),
        voucherno VARCHAR(50), LegTypeID VARCHAR(100), Workflow VARCHAR(100),
        DestinationCity VARCHAR(50), weekNum VARCHAR(10), DriverKey INT,
        DriverOrg VARCHAR(100), BrokerRefNo VARCHAR(50), VesselETA DATETIME,
        ActualDeparture DATETIME, voucherdate DATETIME, WeekStart DATETIME,
        WeekEnd DATETIME, PaidDate DATETIME, CompleteDate DATETIME,
        ispaymentapproved BIT, IsDocumentVerified BIT, IsRateVerified BIT DEFAULT 0,
        IsPaid BIT, DriverHubKey INT, DriverHubName VARCHAR(100),
        MarketLocationKey INT, MarketLocation VARCHAR(200), PaidUserKey INT,
        PaidUserName VARCHAR(100), IsLinked BIT DEFAULT 0, LinkedContainerNo VARCHAR(20),
        LinkedOrderDetailKey INT, LegID VARCHAR(100), LegKey INT, ChargesCount INT,
        OrgName VARCHAR(200),
	...
	BEGIN
		...
		SELECT '#FilteredRoutes', * from #FilteredRoutes FR
        SELECT '#FilteredRouteVouchers', * from  #FilteredRouteVouchers FRV 
        SELECT '#FilteredVouchers', * from  #FilteredVouchers FV 
        SELECT '#VoucherAggregates', * from  #VoucherAggregates
        SELECT '#WeekDates',* FROM #WeekDates
	END
    -- ====================================================================
    -- MAIN QUERY - Uses all pre-filtered temp tables (much faster)
    -- ====================================================================
    ;WITH VoucherData AS (
        SELECT 
            ...
            CASE WHEN VA.OrdCount = 1 THEN FOH.OrderNo ELSE 'Multiple Orders (' + CAST(VA.OrdCount AS VARCHAR(50)) + ')' END AS orderno,
            CASE WHEN VA.ContCount = 1 THEN VA.ContNo ELSE 'Multiple Containers (' + CAST(VA.ContCount AS VARCHAR(50)) + ')' END AS containerno,
            ISNULL(VA.MinArrival, '2022-01-01') AS ActualDeparture,
            FD.DriverID, FD.FirstName, FD.LastName,
            ISNULL(FV.IsPaymentApproved, 0) AS ispaymentapproved,
            ISNULL(FV.StatusKey, 9) AS StatusKey,
            VA.VoucherAmt AS voucheramount, 0 AS routekey, NULL AS destinationaddrkey, 
            FV.VoucherKey AS voucherkey,
            CASE WHEN @isDriverPay = 1 THEN ISNULL(DV.DriverVoucherNumber, FV.VoucherNo) ELSE FV.VoucherNo END AS voucherno,
            FV.VoucherDate AS voucherdate, '' AS Workflow, '' AS LegTypeID, '' AS DestinationCity,
            ...
            FR.IsDocumentVerified, FR.IsRateVerified, NULL AS CompleteDate, '' AS DocCounts,
            VA.Week_Start_Date AS WeekStart, VA.Week_End_Date AS WeekEnd,
            FV.IsPaid, FV.PaidDate, FOH.BrokerRefNo, FOD.VesselETA,
            CASE WHEN ISNULL(FD.OrgName, '') = '' THEN '' 
                 ELSE CONCAT(FD.OrgName, ' ', FD.OrgCity, ' ', FD.OrgZipCode, ' ', FD.OrgState, ' ', FD.OrgCountry) END AS DriverOrg,
            FD.DriverHubKey, ML.MarketLocationKey, ML.MarketLocation,
            FV.PaidUserKey, UI.UserID AS PaidUserName, 
            FOD.IsLinked, UPPER(FOD.LinkedContainerNo) AS LinkedContainerNo, 
            FOD.LinkedOrderDetailKey, '' AS LegID, 0 AS LegKey, NULL AS ChargesCount, FD.OrgName,
		...
		LEFT JOIN DriverVoucher DVD WITH (NOLOCK) ON DVD.LinkedVoucherKey=FV.VoucherKey
    )
    INSERT INTO #TEMPTABLE (
        orderkey, orderdetailkey, orderno, containerno, ActualDeparture, driverid, firstname, lastname,
        ...
        IsDocumentVerified, IsRateVerified, CompleteDate, DocCounts, WeekStart, WeekEnd, IsPaid, PaidDate,
        BrokerRefNo, VesselETA, DriverOrg, DriverHubKey, MarketLocationKey, MarketLocation,
        PaidUserKey, PaidUserName, IsLinked, LinkedContainerNo, LinkedOrderDetailKey, LegID, LegKey, ChargesCount, OrgName
    )
    SELECT orderkey, orderdetailkey, orderno, containerno, ActualDeparture, DriverID, FirstName, LastName,
           ispaymentapproved, StatusKey, voucheramount, routekey, destinationaddrkey, voucherkey, voucherno, voucherdate,
           Workflow, LegTypeID, DestinationCity, DocumentCount, weekNum, DriverKey, DriverHubName,
           IsDocumentVerified, IsRateVerified, CompleteDate, DocCounts, WeekStart, WeekEnd, IsPaid, PaidDate,
           BrokerRefNo, VesselETA, DriverOrg, DriverHubKey, MarketLocationKey, MarketLocation,
           PaidUserKey, PaidUserName, IsLinked, LinkedContainerNo, LinkedOrderDetailKey, LegID, LegKey, ChargesCount, OrgName
    FROM VoucherData WHERE rn = 1;

    -- ====================================================================
    -- PENDING VOUCHERS QUERY (uses pre-filtered temp tables)
    -- ====================================================================
    --IF @isDriverPay = 0 AND @voucherNo = ''
    IF @voucherNo = ''
    BEGIN
        INSERT INTO #TEMPTABLE (
            orderkey, orderdetailkey, orderno, containerno, ActualDeparture, driverid, firstname, lastname,
        ...
        SELECT 
            ...
            FR.ActualArrival, FD.DriverID, FD.FirstName, FD.LastName,
            0, 9, 0, FR.RouteKey, FR.DestinationAddrKey, NULL, '', NULL,
            L.Instruction, LG.LegID, DST.City, ISNULL(CDC.DocumentCount, 0),
            'WK-' + CONVERT(VARCHAR, DATEPART(ISO_WEEK, FR.ActualArrival)),
            FR.DriverKey, DH.DriverHubName, FR.IsDocumentVerified, 0, FOD.CompleteDate, '',
            WD.Week_Start_Date, WD.Week_End_Date, 0, NULL, FOH.BrokerRefNo, FOD.VesselETA,
            CASE WHEN ISNULL(FD.OrgName, '') = '' THEN '' 
                 ELSE CONCAT(FD.OrgName, ' ', FD.OrgCity, ' ', FD.OrgZipCode, ' ', FD.OrgState, ' ', FD.OrgCountry) END,
            FD.DriverHubKey, ML.MarketLocationKey, ML.MarketLocation, NULL, NULL,
            FOD.IsLinked, UPPER(FOD.LinkedContainerNo), FOD.LinkedOrderDetailKey, LG.LegID, FR.LegKey, NULL, FD.OrgName
        ...
        LEFT JOIN dbo.DriverHUB DH WITH (NOLOCK) ON FD.DriverHubKey = DH.DriverHubKey
        WHERE FR.ActualArrival IS NOT NULL
          --AND (FR.DeliveryDateFrom IS NULL OR FR.DeliveryDateFrom = '1900-01-01' 
               --OR FR.DeliveryDateFrom BETWEEN @DeliveryDateFrom AND @DeliveryDateTo);
    END

    -- Update charges
    UPDATE T SET ChargesCount = ISNULL(B.ChargeCount, 0),
        IsRateVerified = CASE WHEN T.StatusKey = 9 AND ISNULL(B.ChargeCount, 0) > 0 THEN 1 ELSE T.IsRateVerified END
    FROM #TEMPTABLE T
    INNER JOIN (
    ...
    UPDATE #TEMPTABLE SET IsRateVerified = 0 WHERE StatusKey = 9 AND ChargesCount IS NULL;

    -- Dashboard
    CREATE TABLE #Dashboard (StatusKey INT, StatusName VARCHAR(50), StatusCount INT);
    INSERT INTO #Dashboard SELECT VS.StatusKey, VS.Description, 0 FROM dbo.VoucherStatus VS WITH (NOLOCK)
    UNION ALL SELECT 9, 'Open', 0;

    UPDATE D SET StatusCount = ISNULL(T.cnt, 0) FROM #Dashboard D
    LEFT JOIN (SELECT StatusKey, COUNT(1) AS cnt FROM #TEMPTABLE GROUP BY StatusKey) T ON D.StatusKey = T.StatusKey;
    INSERT INTO #Dashboard SELECT 0, 'All', ISNULL(SUM(StatusCount), 0) FROM #Dashboard;

    -- Final output
    SELECT 
        ISNULL(orderkey, 0) AS orderkey, ISNULL(orderdetailkey, 0) AS orderdetailkey,
        ISNULL(voucheramount, 0) AS voucheramount, ISNULL(routekey, 0) AS routekey,
        ISNULL(destinationaddrkey, 0) AS destinationaddrkey, ISNULL(voucherkey, 0) AS voucherkey,
        ISNULL(StatusKey, 0) AS StatusKey, ISNULL(DocumentCount, 0) AS DocumentCount,
        ISNULL(DocCounts, '0') AS DocCounts, ISNULL(orderno, '') AS orderno,
        ISNULL(containerno, '') AS containerno, ISNULL(driverid, '') AS driverid,
        ISNULL(firstname, '') AS firstname, ISNULL(lastname, '') AS lastname,
        ISNULL(firstname, '') + ' ' + ISNULL(lastname, '') AS DriverName, DriverKey,
        ISNULL(voucherno, '') AS voucherno, ISNULL(LegTypeID, '') AS LegTypeID,
        ISNULL(Workflow, '') AS Workflow, ISNULL(DestinationCity, '') AS DestinationCity,
        ISNULL(weekNum, '') AS weekNum, ISNULL(DriverOrg, '') AS DriverOrg,
        ISNULL(BrokerRefNo, '') AS BrokerRefNo, ISNULL(VesselETA, '') AS VesselETA,
        ...
        ISNULL(IsPaid, CONVERT(BIT, 0)) AS IsPaid, ISNULL(DriverHubKey, 0) AS DriverHubKey,
        DriverHubName, ISNULL(MarketLocationKey, 0) AS MarketLocationKey, MarketLocation,
    ...
    IF @SortField NOT IN ('orderkey', 'orderdetailkey', 'voucheramount', 'routekey', 'voucherkey', 
        'StatusKey', 'orderno', 'containerno', 'driverid', 'firstname', 'lastname', 'DriverName', 
        'voucherno', 'LegTypeID', 'weekNum', 'ActualDeparture', 'voucherdate', 'WeekStart', 
        'WeekEnd', 'PaidDate', 'DriverHubName', 'MarketLocation')
    ...
    ;WITH FinalData AS (
        SELECT *, ROW_NUMBER() OVER (ORDER BY ' + QUOTENAME(@SortField) + N' ' + @SortDirection + N') AS RowNum FROM #TempPrev
    )
    SELECT VoucherList = (SELECT *, @RecCount AS RecCount FROM FinalData WHERE RowNum BETWEEN @RecFrom AND @RecTo FOR JSON PATH),
    DropDowns = (SELECT
        CarrierList = (SELECT DISTINCT DriverKey, driverid AS DriverName FROM #TempPrev WHERE driverid <> '''' ORDER BY driverid FOR JSON PATH),
        DriverHubList = (SELECT DISTINCT DriverHubKey, DriverHubName FROM #TempPrev WHERE DriverHubName IS NOT NULL ORDER BY DriverHubName FOR JSON PATH),
        MarketLocList = (SELECT DISTINCT MarketLocation, MarketLocationKey FROM #TempPrev WHERE MarketLocation IS NOT NULL ORDER BY MarketLocation FOR JSON PATH),
        TruckTypeList = (SELECT DISTINCT TruckTypeKey, TruckType FROM dbo.TruckType WITH (NOLOCK) WHERE TruckType IS NOT NULL ORDER BY TruckType FOR JSON PATH),
        MoveTypeList = (SELECT DISTINCT MoveTypeKey, MoveTypeName FROM dbo.CarrierMoveType WITH (NOLOCK) WHERE MoveTypeName IS NOT NULL ORDER BY MoveTypeName FOR JSON PATH)
        FOR JSON PATH),
    Dashboard = (SELECT StatusKey, StatusName, StatusCount FROM #Dashboard FOR JSON PATH)
    FOR JSON PATH;';
