CREATE PROCEDURE [dbo].[DispatchEntry_GetList_Praveen] -- DispatchEntry_GetList_Praveen @CarrierStatus='4', @Status='1', @IsDebug = 1
(
	@Weekday				CHAR(3)='',
	@Customer				VARCHAR(50)='',
	@OrderNo				VARCHAR(20)='',
...
BEGIN	
	...
	FROM dbo.OrderDetail OD   WITH (NOLOCK) 
		...
		LEFT JOIN (SELECT TOP 1 HZ1.* FROM vContainerType HZ1
		LEFT JOIN OrderDetail OD1 ON HZ1.ORDERDETAILKEY=OD1.OrderDetailKey AND HZ1.ContainerTypeKey = @HazardTypeKey) HZ ON HZ.ORDERDETAILKEY = OD.OrderDetailKey
		LEFT JOIN (SELECT TOP 1 CTL1.* FROM ContainerTypesLink CTL1 
		LEFT JOIN OrderDetail OD1 ON CTL1.OrderDetailKey=OD1.OrderDetailKey AND CTL1.IsSelected = 1) CTL ON HZ.ORDERDETAILKEY = OD.OrderDetailKey
		...
		LEFT JOIN (select ODI.OrderDetailKey,
					CASE WHEN DRA.Description ='Reject' AND ISNULL(RT.DriverKey,0)=0 THEN 1 --rejected
					WHEN RT.Status in (2,3) THEN 2 --in progress
					WHEN RT.Status=4 THEN 3 --driver assigned
					WHEN RT.Status=1 THEN 4 --pending
					WHEN DRA.Description='Accept' THEN 5 --accept
					ELSE 0
					END AS RouteStatus
					from Orderdetail ODI
					LEFT JOIN Routes RT ON (RT.OrderDetailKey=ODI.OrderDetailKey)
					LEFT JOIN (SELECT  * FROM (select ROW_NUMBER() over (partition by RouteKey order by acceptancekey) slno,routekey, description 
								from driverrouteacceptance where isnull(description,'')<>'pending') A where slno=1) 
								DRA ON DRA.RouteKey=RT.RouteKey --AND RT.Legno=OD.CurrentLegNo
					--WHERE RT.Status=4
					) CS ON CS.OrderDetailKey=OD.OrderDetailKey 
