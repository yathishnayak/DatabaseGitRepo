...
EXEC [Admin_GetDBTableSize_Integration] @UserKey,@JSOnString,@Status OUTPUT, @IntMessage OUTPUT, @ExtMessage OUTPUT, @Result1 OUTPUT, @Result2 OUTPUT
,@Result3 OUTPUT, @IsDebug

SELECT @Status,@IntMessage,@ExtMessage,@Result1,@Result2,@Result3
*/
CREATE PROCEDURE [dbo].[Admin_GetDBTableSize_Integration]
(
	@UserKey		INT = 714,
...
INTO		#DBDate
FROM		Integration_jcb.sys.tables t
INNER JOIN  Integration_jcb.sys.indexes i ON t.object_id = i.object_id
INNER JOIN	Integration_jcb.sys.partitions p ON i.object_id = p.object_id AND i.index_id = p.index_id
INNER JOIN	Integration_jcb.sys.allocation_units a ON p.partition_id = a.container_id
...
FROM Integration_jcb.sys.allocation_units
FOR JSON PATH)
