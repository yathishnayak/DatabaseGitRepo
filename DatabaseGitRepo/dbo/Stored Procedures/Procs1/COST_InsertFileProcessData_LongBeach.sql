...
COST_InsertFileProcessData_LongBeach 98,''

*/

CREATE PRocEDURE [dbo].[COST_InsertFileProcessData_LongBeach] -- COST_InsertFileProcessData_LongBeach 98,''
(
	@FileProcesskey		INT,
...
BEGIN
	...
	DECLARE @ISSuccess BIT = 1, @Remarks VARCHAR(100) = 'Record Uploaded Successfully', 
...
--- Date Validation Logic End
	...
	DECLARE		@Market VARCHAR(100) ,@Terminal VARCHAR(100) ,@City VARCHAR(100) ,@State VARCHAR(100) ,@ZipCode VARCHAR(20) ,@Zone VARCHAR(10) ,@Prepulllocation1 VARCHAR(100) ,@Prepullcost1 VARCHAR(100) 
	...
	DECLARE @SelectedMarket VARCHAR(100) = (SELECT ISNULL(MarketLocation,'') FROM COST_FileProcessInfo WHERE FileProcessKey = @FileProcesskey)
...
Print 2
	...
	Print 3
		INSERT INTO				COST_FileUploadData_LongBeach
		...
		SELECT					@FileProcesskey,SlNo,Market,Terminal,City,State,ZipCode,Zone,Prepulllocation1,Prepullcost1,Prepulllocation2,Prepullcost2
								...
								,@RecordStatus,@RecordRemarks
		FROM					#FileUploadData
		WHERE					SlNo = @i -- AND ISNULL(TruckType2,'') <> ''
