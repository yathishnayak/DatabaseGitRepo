...
DECLARE 
	@UserKey INT=512,
	@JSONString NVARCHAR(MAX)='{"InvoiceKey":138060}',
	@Status BIT=0,
	@Reason VARCHAR(100)=''
...
CREATE PROCEDURE [dbo].[Get_Invoice_V2]
(
	@UserKey		int,
...
BEGIN
	...
	@InvoiceKey  INT,
	@InvoiceCompanyKey INT=0,@CustCompanyKey int=0, @Marketlocationkey int=0
	...
	SELECT DISTINCT OrderDetailKey INTO #OrderDetailKeys 
	FROM InvoiceDetail ID WITH (NOLOCK)
	WHERE InvoiceKey=@InvoiceKey

	--select * from #OrderDetailKeys
	--SELECT ActualPickupDate AS PickupDate FROM OrderDetailStops	ODSPI    WITH (NOLOCK)
	--				INNER JOIN #OrderDetailKeys TOD ON TOD.OrderDetailKey=ODSPI.OrderDetailKey
	--				WHERE ODSPI.StopTypeKey=1 AND ISNULL(ODSPI.IsDryRunPort,0)=0 AND ISNULL(ODSPI.IsDryRunCustomer,0)=0 

	--  SELECT ActualDeliveryDate AS DeliveryDate FROM OrderDetailStops	ODSDI    WITH (NOLOCK)
	--				INNER JOIN #OrderDetailKeys TOD ON TOD.OrderDetailKey=ODSDI.OrderDetailKey
	--				WHERE ODSDI.StopTypeKey=3 AND ISNULL(ODSDI.IsDryRunCustomer,0)=0 AND ISNULL(ODSDI.IsDryRunPort,0)=0 
 --     SELECT ActualDeliveryDate AS ReturnDate FROM OrderDetailStops  ODSRI    WITH (NOLOCK)
	--				INNER JOIN #OrderDetailKeys TOD ON TOD.OrderDetailKey=ODSRI.OrderDetailKey
	...
	SELECT 
	  ...
	  ,@DriverType as TruckType -- TruckType Tag Added
	...
	--  , U6.UserName AS UpdatedUserName
	  ...
	  ,InvoiceDetail =(
			...
			WHERE id.InvoiceKey = @InvoiceKey and ID.OrderDetailKey is not null --ORDER BY OrderDetailKey  
	  ...
	  --PickupDates=(SELECT ActualPickupDate AS PickupDate FROM OrderDetailStops	ODSPI    WITH (NOLOCK)
			--		INNER JOIN #OrderDetailKeys TOD ON TOD.OrderDetailKey=ODSPI.OrderDetailKey
			--		WHERE ODSPI.StopTypeKey=1 AND ISNULL(ODSPI.IsDryRunPort,0)=0 AND ISNULL(ODSPI.IsDryRunCustomer,0)=0  FOR JSON PATH),
	  --DeliveryDates=(SELECT ActualDeliveryDate AS DeliveryDate FROM OrderDetailStops	ODSDI    WITH (NOLOCK)
			--		INNER JOIN #OrderDetailKeys TOD ON TOD.OrderDetailKey=ODSDI.OrderDetailKey
			--		WHERE ODSDI.StopTypeKey=3 AND ISNULL(ODSDI.IsDryRunCustomer,0)=0 AND ISNULL(ODSDI.IsDryRunPort,0)=0 FOR JSON PATH),
   --   ReturnDates=(SELECT ActualDeliveryDate AS ReturnDate FROM OrderDetailStops  ODSRI    WITH (NOLOCK)
			--		INNER JOIN #OrderDetailKeys TOD ON TOD.OrderDetailKey=ODSRI.OrderDetailKey
			--		WHERE ODSRI.StopTypeKey=5 AND ISNULL(ODSRI.IsDryRunPort,0)=0 AND ISNULL(ODSRI.IsDryRunCustomer,0)=0 FOR JSON PATH),
	  ...
	  --ISNULL(ISNULL(OD.BillOfLadding, OH.BillOfLading), 'NA') AS MBL
	...
	--LEFT JOIN dbo.OrderDetail OD	WITH (NOLOCK) ON OD.OrderKey=IH.OrderKey
