...
DECLARE 
	@UserKey INT=512,
	@JSONString NVARCHAR(MAX)='{"DriverKeys":"","OrderKeys":"","OrderNo":"","containerNo":"","voucherNo":"","VoucherKeys":"","DriverHubkeys":"","WeekNum":"","MarketLocationKeys":"","TruckTypeKeys":"","CarrierMoveTypeKeys":"","SearchText":"","SortField":"voucherno","IsAscending":true,"PageSize":50,"PageNo":1,"StatusKey":9}',
	@Status BIT=0, @Debug int = 1,@Reason VARCHAR(100)=''
EXEC Get_VoucherList_V2_BASE20251009 @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @Debug
Select @Status, @Reason
**/
CREATE Procedure [dbo].[Get_VoucherList_V2_BASE20251009] 
(
	@UserKey		int,
...
BEGIN
	...
	Begin
		Set @OrderDateFrom = case when @IsWithFilter = 0 then Getdate() - 180 else  '2020-01-01' end
	End
	...
	(
			...
			IsPaid				bit,
			DriverHubKey		int,
			DriverHubName		varchar(400),
			MarketLocationKey	INT,
			MarketLocation		VARCHAR(200),
			PaidUserKey			int,
			PaidUserName		varchar(300),
			IsLinked			bit default 0, 
			LinkedContainerNo	varchar(50), 
	...
	BEGIN
		...
		OD.IsLinked, upper(OD.LinkedContainerNo)  as LinkedContainerNo, 
		OD.LinkedOrderDetailKey,'' LegID, 0  as Legkey,OrgName
