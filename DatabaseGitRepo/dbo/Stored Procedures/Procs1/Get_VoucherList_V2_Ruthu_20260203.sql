...
EXec [Get_VoucherList_V2_Ruthu_20260203] @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @Debug
Select @Status, @Reason
**/
CREATE Procedure [dbo].[Get_VoucherList_V2_Ruthu_20260203] 
(
	@UserKey		int,
...
BEGIN
    ...
    BEGIN
        ...
        (DriverKey)
        SELECT DriverKEY FROM Driver WHERE TRY_CAST(LEFT(DRIVERID, PATINDEX('%[^0-9]%', DRIVERID + 'A') - 1) AS INT)  BETWEEN 700 AND 948
    END
    ELSE
    BEGIN
        INSERT INTO #IsDriverPayDriverKeys
        (DriverKey)
        SELECT DriverKEY FROM Driver WHERE TRY_CAST(LEFT(DRIVERID, PATINDEX('%[^0-9]%', DRIVERID + 'A') - 1) AS INT)  Not BETWEEN 700 AND 948
    END
		...
		INNER JOIN dbo.OrderHeader OH WITH (NOLOCK)
			   ...
			   AND (
					...
					OR EXISTS (
						SELECT 1 
						FROM #OrderKey OK 
						WHERE OK.OrderKey = OH.OrderKey
					)
				   )
		INNER JOIN dbo.Driver D WITH (NOLOCK)
			ON D.DriverKey = RT.DriverKey
		   AND (
				ISNULL(@DriverKeys,'') = '' 
				OR EXISTS (
					SELECT 1 
					FROM #DriverKey DK 
					WHERE DK.DriverKey = D.DriverKey
				)
			   )
