...
DECLARE 
	@UserKey INT=714,
	@JSONString NVARCHAR(MAX)='{"Factored":2,"CustApproved":2,"PageNo":1,"PageSize":50,"Ascending":true,"IsAscending":true,"SearchText":"FFAU1452564","CustomerKey":"","MarketLocationKey":"","InvoicerKey":"","CustCompanyKey":"","SortField":"TerminationDate","StatusKey":9}',
...
CREATE Procedure [dbo].[Get_InvoiceList_V2_Base20241010] -- Get_InvoiceList_V2 @StatusKey=2, @PageNo = 1,@SearchText='WHSU5296890'
(
	@UserKey		INT = 714,
...
BEGIN
	...
	INNER JOIN		dbo.RouteStatus RTS	WITH (NOLOCK) ON RTS.[Status]=RT.[Status]	
	INNER JOIN		#Toinvoice G WITH (NOLOCK) ON G.OrderDetailKey=RT.OrderDetailKey
	WHERE			RTS.[Description] ='Leg Completed' AND isnull(RT.IsRateVerified,0)=0 AND G.statuskey = 9
	...
	WHERE				(  ISNULL(@OrderKey,0) =0  OR OrderKey=@OrderKey )	
					...
					AND	(ISNULL(@SearchText,'') =  '' OR (OrderNo like '%' + @SearchText + '%' OR ContainerNo  like '%' + @SearchText + '%'  OR 
							...
							BrokerRefNo like '%' + @SearchText + '%' OR BillOfLading  like '%' + @SearchText + '%'  OR City  like '%' + @SearchText + '%' 
