...
*/
CREATE PROC [dbo].[Auto_ChargeYardStorage_Base06122024] -- [Auto_ChargeYardStorage] 126489, 1
(
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				SELECT			I.ItemKey, @LoadedRouteKey, ISNULL(A.Rate, I.UnitCost), @LoadedDateDiff, ISNULL(A.Rate, I.UnitCost), Getdate(),  
		...
		BEGIN
			...
			BEGIN
				...
				INSERT INTO		OrderExpense (ItemKey, Routekey, UnitCost, Qty, NewUnitCost, CreateDate, CreateUserKey, FreeTime, 
								BvsNB, MinCnt, MaxCnt, ChargeSource, OrderDetailKey, DateFrom, DateTo)
				SELECT			I.ItemKey, @EmptyRouteKey, ISNULL(A.Rate, I.UnitCost), @EmptyDateDiff, ISNULL(A.Rate, I.UnitCost), Getdate(),  
	...
	END
		...
		IF(@IsDebug = 1)
		BEGIN
			SELECT  @LoadedDateDiff as LoadedDateDiff, 
					...
					@EmptyDateDiff as EmptyDateDiff ,
