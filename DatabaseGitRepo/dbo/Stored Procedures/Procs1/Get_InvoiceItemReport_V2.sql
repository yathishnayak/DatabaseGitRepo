/*
DECLARE @UserKey INT = 951, @JSONString NVARCHAR(MAX),@Status BIT = 0,@Reason VARCHAR(1000), @IsDebug BIT = 0 
SET @JSONString ='{
    "CustKey": 3241,
    "DateFrom": "2020-02-05T18:30:00.000Z",
    "DateTo": "2026-02-05T18:30:00.000Z",
    "ItemId": "40:42:",
    "City": "",
    "AddrKey": 0
}' 
EXEC [Get_InvoiceItemReport_V2] @UserKey, @JSONString, @Status OUTPUT, @Reason OUTPUT, @IsDebug
SELECT @Status Status, @Reason Reason 
*/

/*
DECLARE @UserKey INT = 951, @JSONString NVARCHAR(MAX),@Status BIT = 0,@Reason VARCHAR(1000), @IsDebug BIT = 0 
SET @JSONString ='{"CustKey":1562,"DateFrom":"2020-01-09T18:30:00.000Z","DateTo":"2026-01-09T18:30:00.000Z","ItemId":"18:","City":"","AddrKey":"25508"}' 
EXEC [Get_InvoiceItemReport_V2] @UserKey, @JSONString, @Status OUTPUT, @Reason OUTPUT, @IsDebug
SELECT @Status Status, @Reason Reason 
*/

CREATE PROCEDURE [dbo].[Get_InvoiceItemReport_V2]
(
	@UserKey	INT,
...
BEGIN
	...
	Begin
            ...
            (
                ...
                FROM (
					...
					inner join Item I WITH (NOLOCK) on B.itemkey = I.ItemKey
					where A.CustKey = @CustKey and ( a.InvoiceDate BETWEEN @DateFrom AND @DateTo) 
	...
	SELECT ItemID FROM Item WITH (NOLOCK) WHERE ItemKey IN (SELECT ITEMKEY FROM #ITEMKEY)
	...
	INTO #DATA
		...
		inner join Invoicedetail B WITH (NOLOCK) on (A.InvoiceKey = B.InvoiceKey)
		inner join Customer C WITH (NOLOCK) on (A.CustKey = C.CustKey)
		left join OrderDetail D WITH (NOLOCK) on (B.OrderDetailKey = D.OrderDetailKey)
		left join OrderHeader E WITH (NOLOCK) on (D.OrderKey = E.OrderKey)
		left join Address F WITH (NOLOCK) on (A.BillToAddrKey = F.AddrKey)
		...
		LEFT JOIN CustomerAddress CA WITH (NOLOCK) ON (CA.AddrKey=F.AddrKey) AND (CA.CustKey=C.CustKey)
		...
		WHERE	(@CustKey = 0 OR C.CustKey = @CustKey) AND
				( a.InvoiceDate BETWEEN @DateFrom  AND @DateTo) AND
	...
	EXEC sp_executesql
    @STRSQL,
    N'@JSONOutput NVARCHAR(MAX) OUTPUT',
    @JSONOutput = @JSONOutput OUTPUT;
	IF (@JSONOutput IS NULL OR LEN(@JSONOutput) = 0)
	BEGIN
		SELECT @JSONOutput = (
		Select '' CustomerID, '' CustomerName, '' InvoiceNo, getdate() [InvoiceDate],  
		''  [OrderNumber],'' Container,'' City, ''BrokerRefNo, 0.00InvoiceAmount,  ''   Status, 
