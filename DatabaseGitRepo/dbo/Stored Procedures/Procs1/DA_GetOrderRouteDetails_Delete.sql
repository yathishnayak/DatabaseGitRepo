...
CREATE PROCEDURE	[dbo].[DA_GetOrderRouteDetails_Delete]
(
	@UserKey		INT = 714,
	...
	BEGIN TRANSACTION
		...
		ELSE
			...
			FROM		(SELECT			OD.OrderDetailKey, ContainerNo,OD.ContainerSizeKey, CS.Description AS ContainerSize, R.RouteKey
						, CASE WHEN L.ToLocation = 'Yard' THEN ISNULL(ScheduledPickupDate,ScheduledDeparture)  WHEN L.FromLocation = 'Yard' 
						THEN ISNULL(R.ScheduledArrival,R.DeliveryDateTo) ELSE ISNULL(ScheduledPickupDate,ScheduledDeparture) END OrderByDate
						,OH.OrderDate, L.FromLocation,L.ToLocation,R.DriverKey, ISNULL(ScheduledPickupDate,ScheduledDeparture) ScheduledPickupDate
						, R.Status AS RouteStatus
						,AD.AddrName SourceAddrName,AD.Address1 SourceAdd1, AD.Address2 SourceAdd2, AD.City SourceCity, AD.State SourceState
						,AD.ZipCode SourceZip
						,AD1.AddrName DestAddrName,AD1.Address1 DestAdd1, AD1.Address2 DestAdd2, AD1.City DestCity, AD1.State DestState
						,AD1.ZipCode DestZip
						,ISNULL(R.ScheduledArrival,R.DeliveryDateTo) ScheduledArrival, DRC.Description OrderDesc, ISNULL(DRC.ActionDate
						,DRC.CreateDate) RejectedDate
						,  ScheduledDeparture
						...
						)A  WHERE RouteKey IN (604848,872216) --   OD.OrderDetailKey = 49045
			--WHERE		(CASE WHEN @IsCompleted = 0 THEN A.RouteStatus ELSE 0 END IN (2,4) OR CASE WHEN @IsCompleted = 1 
			--			THEN A.RouteStatus ELSE 0 END IN (3,5) 
			...
			IF(@IsDebug = 1)
				BEGIN
					-- SELECT * FROM #DriverRouteAcceptance
					SELECT * FROM #Data
			...
			SET @JsonRes =  (SELECT @ActiveRouteKey AS ActiveRouteKey, RouteDetails =  (( SELECT * FROm #FinalData DA 
