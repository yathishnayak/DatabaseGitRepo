...
set @JsonString = '{"OrderDetailKey":284225,"ItemType":"Service"}'
exec Charge_GetItemsByOrderDetailKey @UserKey, @JSONString, @Status output, @Reason output
select @Status, @Reason
*/
CREATE proc [dbo].[Charge_GetItemsByOrderDetailKey]
(
	@UserKey		int,
...
Begin
	...
	SELECT * INTO #Items FROM (Select OrderExpenseKey,OE.Itemkey,IM.Description AS LineItem,
		...
		OE.FreeTime,OE.BvsNB,OE.MinCnt,OE.MaxCnt,CustomerRate,ChargeSource,isCSApproved,CSApprovedDate,CSUserKey,IsInvoiced,
	...
	UNION ALL
	Select OrderExpenseKey,OE.Itemkey,IM.Description AS LineItem,
	OE.RouteKey,OE.UnitCost,OE.Qty,NewUnitCost,DateFrom,DateTo,ExtAmt = 0, --isnull(OE.UnitCost,0) * isnull(OE.Qty,0),
	OE.CreateDate,OE.CreateUserKey, L.LegKey, L.LegID, @OrderDetailKey AS OrderDetailKey,
	OE.LastUpdateDate,OE.UpdateUserKey,ExpenseItemKey,OE.TimeDuration,
	InternalNotes,PvsNP,IsCSRApproved,IsCustomerApproved,OE.IsChargeSharedWithCustomer as IsSharedWithCustomer,
	OE.FreeTime,OE.BvsNB,OE.MinCnt,OE.MaxCnt,CustomerRate,ChargeSource,isCSApproved,CSApprovedDate,CSUserKey,IsInvoiced,
	OE.WarehouseItemKey,  od.ContainerNo, IT.PriceBasisKey,
	...
	LEFT JOIN OrderDetail OD WITH (NOLOCK) ON RT.OrderDetailKey = OD.OrderDetailKey
	LEFT JOIN [USER] UC WITH (NOLOCK) ON ISNULL(oe.UpdateUserKey, OE.CreateUserKey) = UC.UserKey
	WHERE OE.OrderDetailKey = @OrderDetailKey AND TT.ItemType LIKE '%' + @ItemType + '%' 
	AND OE.ItemKey NOT IN(SELECT ItemKey FROM OrderExpense WHERE OrderDetailKey = @OrderDetailKey)) A
