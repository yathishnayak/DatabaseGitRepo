...
DECLARE 
	@UserKey INT=714,
	@JSONString NVARCHAR(MAX)='{"Factored":2,"CustApproved":2,"PageNo":1,"PageSize":50,"Ascending":true,"IsAscending":true,"SearchText":"FL01202526478","CustomerKey":"","InvoiceNo":"","ContainerNo":"","OrderNo":"","BOL":"","MarketLocationKey":"","InvoicerKey":"","CustCompanyKey":"","ChargeConfirmed":2,"SortField":"TerminationDate","StatusKey":16,"WarehouseStatusKeys":"","DoorToDoor":2,"PaymentStatus":0,"SearchCriteriaKey":4}'
	,@Status BIT=0,@IsDebug		BIT = 1
	,@Reason VARCHAR(100)=''
EXEC Get_InvoiceList_V2 @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT,@IsDebug
...
DECLARE 
	@UserKey INT=714,
	@JSONString NVARCHAR(MAX)='{"Factored":2,"CustApproved":2,"PageNo":1,"PageSize":50,"Ascending":true,"IsAscending":true,"SearchText":"MSDU6322175","CustKey":"","InvoiceNo":"","ContainerNo":"","OrderNo":"","BOL":"","MarketLocationKey":"","InvoicerKey":"","CustCompanyKey":"","ChargeConfirmed":2,"SortField":"TerminationDate","StatusKey":9,"WarehouseStatusKeys":"","ReasonCodeKey":null,"DoorToDoor":2,"PaymentStatus":0,"SearchCriteriaKey":1}'
	,@Status BIT=0,@IsDebug		BIT = 1
	,@Reason VARCHAR(100)=''
EXEC Get_InvoiceList_V2 @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT,@IsDebug
Select @Status, @Reason
...
CREATE Procedure [dbo].[Get_InvoiceList_V2] -- Get_InvoiceList_V2 @StatusKey=2, @PageNo = 1,@SearchText='WHSU5296890'
(
	@UserKey		INT = 714,
...
BEGIN
	...
	SELECT			@StatusKey			= StatusKey,			@CustKey		= CustKey,		@OrderKey			= OrderKey,			
					...
					@WarehouseStatusKeys=WarehouseStatusKey,    @InvoiceDateFrom =isnull(InvoiceDateFrom ,''), 
					@InvoiceDateTo = isnull(InvoiceDateTo ,''),		@ReasonCode         = ReasonCode,		@DoorToDoor         = DoorToDoor,
					@PaymentStatus  = PaymentStatus,			@SearchCriteriaKey	= SearchCriteriaKey
	...
	IF(@InvoiceDateTo IS NULL)
		BEGIN
			SET @InvoiceDateTo = Getdate()  + 30
		END

	SET @CustCompanyKey=ISNULL(@CustCompanyKey,'')
	SET @WarehouseStatusKeys=ISNULL(@WarehouseStatusKeys,'')
	SET @CustKey=ISNULL(@CustKey,'')
	SET @SortField = ISNULL(@SortField,'TerminationDate')
	SET @IsDoorToDoor=CASE WHEN @DoorToDoor =1 THEN CAST(1 AS BIT) WHEN @DoorToDoor= 0 THEN CAST(0 AS BIT)  END
	IF(@InvoiceDateFrom IS NULL)
		BEGIN
			set @InvoiceDateFrom = '2020-01-01' -- Getdate() - 90
		END

	CREATE TABLE #WarehouseStatusKeys
	(
		StatusKey		int,
		StatusName		varchar(100)
	)
	IF(LEN(ISNULL(@WarehouseStatusKeys,'')) > 0)
	BEGIN
		insert into #WarehouseStatusKeys(StatusKey)
		select value from dbo.Fn_SplitParamCol(@WarehouseStatusKeys)
	END
	   	
	IF(@IsDebug = 1)
		BEGIN
			SELECT @StatusKey			AS  StatusKey,			@CustKey		AS  CustomerKey,	@OrderKey			AS  OrderKey,			
					...
					@CustCompanyKey		AS  CustCompanyKey,		@ChargeConfirmed    AS  ChargeConfirmed, @WarehouseStatusKeys AS WarehouseStatusKeys,
					@InvoiceDateFrom    AS  InvoiceDateFrom,    @InvoiceDateTo      AS  InvoiceDateTo,   @ReasonCode        AS   ReasonCode,
	...
	IF(ISNULL(@CustKey,'') <> '')
		BEGIN
			...
			SELECT VALUE FROM dbo.Fn_SplitParamCol(@CustKey)
		END		
	...
	IF(@IsDebug = 1)
		BEGIN
			...
			SELECT '#WarehouseStatusKeys',* from #WarehouseStatusKeys
		END
...
BEGIN
    ...
    BEGIN
        ...
        IF @SearchCriteriaKey = 2 -- OrderNo
        BEGIN
            INSERT INTO #OrderDetailKeys (OrderDetailKey)
            SELECT DISTINCT OD.OrderDetailKey
            FROM dbo.OrderDetail OD WITH (NOLOCK)
        ...
        IF @SearchCriteriaKey = 4 -- InvoiceNo
        BEGIN
            INSERT INTO #InvoiceKeys (InvoiceKey)
            SELECT DISTINCT IH.InvoiceKey
            FROM dbo.InvoiceHeader IH WITH (NOLOCK)
...
END
	...
	BEGIN
		...
		SELECT '#InvoiceKeys', * FROM #InvoiceKeys;
	END
	...
	--If(@StatusKey in (0,9))
		BEGIN
			INSERT INTO		#ToInvoice (OrderKey, OrderNo, ContainerNo, ActualArrival, CustId, CustName, OD.OrderDetailKey,
							...
							RevisionDate, BrokerRefNo, IsFactored, VesselETA, BalanceAmount, CustKey, OrderDate, BillOfLading, OrderTypeKey, OrderType,
							OH.BookingNo, TerminationDate,  AddrName, MarketLocationKey, MarketLocation,CustApproved ,ReasonCodeKey,ReasonCodeName,CustCompanyKey,
							AgingDays,InvoicerKey,AprovedReasonCodeKey,ApprovedReasonCode,CustCompanyName,RouteKey,
							LegID,IsDataSelected,IsSelectedStatusKey,CSR,WarehouseStatus, WarehouseStatusKey, 
							AllowInvoicing, IsCSChargesApproved, CSChargesApproveDate, ExpCount,DoorToDoor, 
							ContainerCount,InvoicePaymentStatus,MasterInvoiceNo)
			SELECT			OH.OrderKey,  OH.OrderNo, OD.ContainerNo AS ContainerNo, 
							...
							, OH.CustKey , OH.OrderDate, OH.BillOfLading, OT.OrderTypeKey, OT.OrderType,
							...
							case when isnull(CT.OrderDetailKey ,0) = 0 then -1 else isnull(WS.Statuskey,1) end as WarehouseStatusKey,
							...
							CCT.ContainerCount,null,''
							--case when @DoorToDoor = 1 then cast ( 1 as bit ) else cast (0 as bit) end as DoorToDoor
			...
			LEFT JOIN		#ContainerCount CCT with ( NOLOCK)		ON CCT.OrderKey=OH.OrderKey			
			WHERE			--RT.[Status] =5 AND 
							OD.status in (6,10,12,13,14) AND RI.InvoiceKey is null
							--AND (NOT EXISTS (SELECT 1 FROM #OrderDetailKeys) OR OD.OrderDetailKey IN (SELECT OrderDetailKey FROM #OrderDetailKeys)) -- Added for advance search
							AND (@IsSearchActive = 0 OR OD.OrderDetailKey IN (SELECT OrderDetailKey FROM #OrderDetailKeys))
							--AND InvoiceDate between @InvoiceDateFrom AND @InvoiceDateTo
			GROUP BY		OH.OrderKey,oh.OrderNo,od.ContainerNo,CU.CustName, AD.City,OH.DestinationAddrKey, OD.OrderDetailKey,
							...
							OH.CustKey, OH.OrderDate, OH.BillOfLading, OT.OrderTypeKey, OrderType, OH.BookingNo, OD.CompleteDate, 
							ad.AddrName,ML.MarketLocationKey,ML.MarketLocation,CU.CustomerCompanyKey,CC.CompanyName,
							OD.CurrentRouteKey,
							--RT.RouteKey, L.LegID,
							CSR.CsrName,WS.Description, WS.Statuskey,CT.OrderDetailKey, OD.isChargesSharedWithCust, ChargeSharedWithCustDate,
							OE.ExpCount,CCT.ContainerCount
		END

		IF @IsDebug = 1
		BEGIN
			Select '#ToInvoice 1', Count(1) FROM #ToInvoice;
			Select '#ToInvoice 1', * FROM #ToInvoice;			
		END
	...
	--IF(@StatusKey in (1,2,3,0))
		BEGIN
			INSERT INTO		#ToInvoice (OrderKey, OrderNo, ContainerNo, ActualArrival, CustId, CustName, OrderDetailKey,
							...
							AgingDays,InvoicerKey,AprovedReasonCodeKey,ApprovedReasonCode,CustCompanyName,RouteKey,
							LegID,IsDataSelected,IsSelectedStatusKey,CSR,WarehouseStatus,WarehouseStatusKey, AllowInvoicing,ExpCount,DoorToDoor,
							ContainerCount,InvoicePaymentStatus,MasterInvoiceNo)
			SELECT			OH.OrderKey,  OH.OrderNo, '' AS ContainerNo, '' AS ActualArrival, CU.CustId, CU.CustName, 0 OrderDetailsKey,
							...
							,CC.CompanyName AS CustCompanyName, 0, '',0,0,CSR.CsrName,'', 0, 0,0, 
							case when OT.ordertypekey = 3 then cast ( 1 as bit ) else cast (0 as bit) end as DoorToDoor,
							CCT.ContainerCount,IP.StatusKey,ISNULL(MIL.MasterInvoiceNo,'')
							--case when @DoorToDoor = 1 then cast ( 1 as bit ) else cast (0 as bit) end as DoorToDoor
			...
			LEFT JOIN 
						(
							SELECT 
								IPI.*,
								ROW_NUMBER() OVER (PARTITION BY IPI.InvoiceKey ORDER BY IPI.PaymentKey DESC) AS rn
							FROM InvoicePayment IPI
						) IP ON IP.InvoiceKey = IH.InvoiceKey
							AND IP.rn = 1
			--CROSS APPLY 
			--			(
			--				SELECT TOP 1 *
			--				FROM InvoicePayment IPI
			--				WHERE IPI.InvoiceKey = IH.InvoiceKey
			--				ORDER BY IPI.PaymentKey DESC
			--			) IP
			WHERE			AIH.InvoiceKey IS NULL --AND NOT(IH.StatusKey = 3 and IH.CreateDate <= Getdate() - 60)
							AND NOT(IH.StatusKey = 3 and IH.CreateDate <= Getdate() - 60 
								AND (Isnull(@InvoiceNo,'') = '' AND isnull(@SearchText,'') = ''))
							AND (@IsSearchActive = 0 OR IH.InvoiceKey IN (SELECT InvoiceKey FROM #InvoiceKeys))
							--AND ((SELECT Count(1) FROM #InvoiceKeys)=0 OR IH.InvoiceKey IN (SELECT InvoiceKey FROM #InvoiceKeys)) -- Added for advance search
							--AND (NOT EXISTS (SELECT 1 FROM #InvoiceKeys) OR IH.InvoiceKey IN (SELECT InvoiceKey FROM #InvoiceKeys)) -- Added for advance search
							--	AND  (ISNULL(@InvoiceDateFrom,'')='' AND ISNULL(@InvoiceDateTo,'')='') OR 
							--CAST(InvoiceDate AS DATE)between CAST(@InvoiceDateFrom AS DATE) AND CAST(@InvoiceDateTo AS DATE)
							--AND (Isnull(@Invoiceno,'') <> '' AND IH.InvoiceNo = @InvoiceNo)
		...
		BEGIN
			...
			Select '#ToInvoice 2', InvoiceKey,* FROM #ToInvoice;			
		END
		
	IF(@IsDebug = 1)
		BEGIN
			SELECT '#ToInvoice', COUNT(1) FROM #ToInvoice
			Select WarehouseStatusKey , count(1)
			from #ToInvoice
			Group by WarehouseStatusKey
	...
	*/
	...
	WHERE				(  ISNULL(@OrderKey,0) =0  OR OrderKey=@OrderKey )	
					AND (  ISNULL(@CustKey,'') = ''  OR CustKey IN (SELECT CustomerKey FROM #CustomerKeys))	
					...
					AND (  ISNULL(@WarehouseStatusKeys,'')='' OR ISNULL(WarehouseStatusKey,0) in (SELECT StatusKey FROM #WarehouseStatusKeys))
					--AND  (ISNULL(@InvoiceDateFrom,'')='' AND ISNULL(@InvoiceDateTo,'')='') OR 
					--		CAST(InvoiceDate AS DATE)between CAST(@InvoiceDateFrom AS DATE) AND CAST(@InvoiceDateTo AS DATE)
					AND (
					(ISNULL(@InvoiceDateFrom,'')='' AND ISNULL(@InvoiceDateTo,'')='')
					OR
					CAST(InvoiceDate AS DATE) BETWEEN CAST(@InvoiceDateFrom AS DATE)
												  AND CAST(@InvoiceDateTo AS DATE)
					)
					AND ( ISNULL(@ReasonCode,0)= 0 OR ReasonCodeKey=@ReasonCode )
					--AND (ISNULL(@DoorToDoor,2)= 2 OR ISNULL(DoorToDoor,0) = @IsDoorToDoor)
					AND (ISNULL(@DoorToDoor, 2) = 2 OR DoorToDoor = @IsDoorToDoor)
					...
					--		BillOfLading  like '%' + @SearchText + '%'  
					--		OR City  like '%' + @SearchText + '%' 
					--		OR CustID  like '%' + @SearchText + '%'  OR 
					--		CustName  like '%' + @SearchText + '%' 
					--		OR InvoiceNo  like '%' + @SearchText + '%'  )
					--AND ( ISNULL (@StatusType,0)=0 OR InvoicePaymentStatus=@StatusType) 
	...
	SELECT			A.OrderKey,
					...
					, A.CustKey, A.OrderDate, BillOfLading, BookingNo, TerminationDate,
					...
					A.CustCompanyKey,AgingDays,InvoicerKey,AprovedReasonCodeKey,A.OrderTypeKey, A.OrderType,
					...
					isnull(ExpCount,0) as ExpCount,DoorToDoor,ContainerCount,MasterInvoiceNo
	...
	WHERE			1 = 1  
	AND IsDataSelected = 1
	AND (ISNULL(@DoorToDoor, 2) = 2 OR DoorToDoor = @IsDoorToDoor)
	AND ( ISNULL (@PaymentStatus,0)=0 OR InvoicePaymentStatus=@PaymentStatus) 
	--AND (NOT EXISTS (SELECT 1 FROM #OrderDetailKeys) OR OD.OrderDetailKey IN (SELECT OrderDetailKey FROM #OrderDetailKeys))
	--AND (NOT EXISTS (SELECT 1 FROM #InvoiceKeys) OR M.InvoiceKey IN (SELECT InvoiceKey FROM #InvoiceKeys))
	...
	--WHERE			IsDataSelected = 1 and ( (ISNULL(@InvoiceNo,'') <> '') OR (ISNULL(@SearchText,'') <>''  And StatusKey = @StatusKey) OR 
	...
	SELECT	InvoiceList = (
		...
		DropDowns = ( SELECT
			...
			ReasonCodeList =	(SELECT DISTINCT	ReasonCodeKey,ReasonCodeName FROM #InvoiceListData WITH (NOLOCK) 
								WHERE				IsSelectedStatusKey = 1 AND ISNULL(ReasonCodeName,'')<>''	ORDER BY ReasonCodeName FOR JSON PATH )
			FOR JSON PATH
		),
		Dashboard = (
