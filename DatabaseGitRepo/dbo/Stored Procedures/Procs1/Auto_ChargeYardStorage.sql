...
*/
CREATE PROC [dbo].[Auto_ChargeYardStorage] -- [Auto_ChargeYardStorage] 265605, 1
(
...
print 'after'
	...
	BEGIN
		...
		BEGIN
			...
			SET					@EmptyDateDiff = Case when Datepart(D, @EmptyDelivery) = Datepart(D, @EmptyPickup)
											then  0 else DATEDIFF(D, @EmptyDelivery, @EmptyPickup) end
			...
			IF(@EmptyDateDiff > 0 AND isnull(@EmptyItemExists,0) = 0)
			BEGIN
