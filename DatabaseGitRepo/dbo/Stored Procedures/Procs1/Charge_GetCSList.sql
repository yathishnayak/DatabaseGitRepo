...
set @JsonString = '{"ContainerNo":"","ThisWeek":false,"Today":false,"Arrived":false,"NextWeek":false,"ThisMonth":false,"DemurrageStatus":false,"DetentionStatus":false,"NearingDemurrage":false,"WithDemurrage":false,"WithDetention":false,"Terminal":"","PageNo":1,"PageSize":50,"SortField":"OrderNo","IsAscending":true,"CSMKeys":"","CSRKeys":"","ContainerStatusKeys":"","CustKeys":"","SalesPersonKeys":"","HoldStatus":"","TerminalNames":"","TerminalCodes":"","VesselIMOs":"","MarketKeys":"","SearchText":"","DischargeYN":"","PickupAvailable":"","HoldTypes":"","PickUpFrom":null,"PickUpTo":null,"Deliverylocationkeys":"3 harbors:"}'
exec Charge_GetCSList @UserKey, @JSONString, @Status output, @Reason output
select @Status, @Reason
*/
CREATE PROCEDURE [dbo].[Charge_GetCSList]   
(
	@UserKey		int,
...
BEGIN
	...
	DECLARE
		...
		@IsExport			bit = 0,
		@CSRManagerKeys		varchar(max),
		@SalesPersonKeys		varchar(max),
		@LoggedUserKey		int = 0,
		@isShowAll			bit = 0,
		@marketLocationKeys	varchar(max),
		@Pickuplocationkeys   varchar(max),
		@Deliverylocationkeys varchar(max),
	...
	Select @PickupDateFrom = PickupDateFrom, @PickupDateTo = PickupDateTo, @DeleveryDateFrom = DeleveryDateFrom, 
		@DeleveryDateTo = DeleveryDateTo, @CSRKeys = CSRKeys, @statusKey = statusKey, @IsTransLoad = IsTransLoad, 
		@CustomerKeys = CustomerKeys, @PageNo = PageNo,@PageSize = PageSize, @SortField = SortField, 
		@IsAscending=IsAscending, @CreatedUSer=CreatedUSer, @SearchText=SearchText, @IsExport=IsExport, 
		@CSRManagerKeys=CSRManagerKeys, @SalesPersonKeys=SalesPersonKeys, @LoggedUserKey=LoggedUserKey, 
		@isShowAll=isShowAll, @marketLocationKeys=marketLocationKeys, @Pickuplocationkeys=Pickuplocationkeys, 
		@Deliverylocationkeys=Deliverylocationkeys,@WarehouseStatusKeys=WarehouseStatusKey
	...
	WITH (
		...
		CustomerKeys			varchar(max)	'$.CustKeys',
		PageNo					INT				'$.PageNo',
		...
		CSRManagerKeys			varchar(max)	'$.CSMKeys',
		SalesPersonKeys			varchar(max)	'$.SalesPersonKeys',
		LoggedUserKey			int				'$.LoggedUserKey',
		isShowAll				bit				'$.isShowAll',
		marketLocationKeys		varchar(max)	'$.MarketKeys',
		Pickuplocationkeys		varchar(max)	'$.Pickuplocationkey',
		Deliverylocationkeys	varchar(max)	'$.Deliverylocationkeys',
		WarehouseStatusKey		varchar(max)	'$.WarehouseStatusKey'
	)
	...
	BEGIN
		...
		Update CK set CustKey = C.CustKey
		from Customer C WITH (NOLOCK) 
		Inner join #CustKeys CK On C.CustName = CK.CustName
	END
	...
	IF(LEN(ISNULL(@CSRManagerKeys,'')) > 0)
	BEGIN
		insert into #CSMKeys(CSMName)
		select value from dbo.Fn_SplitParamCol(@CSRManagerKeys)
	...
	BEGIN
		insert into #SalesPersonKeys(SalesPersonName)
	...
	BEGIN
		 ...
		 SELECT VALUE FROM Fn_SplitParamCol(@marketLocationKeys)
	...
	BEGIN
		insert into #DelieverLocationKeys(AddrName)
	...
	Begin
		Select @PickupDateFrom  as  PickupDateFrom, @PickupDateTo  as  PickupDateTo, @DeleveryDateFrom  as  DeleveryDateFrom, 
			@DeleveryDateTo  as  DeleveryDateTo, @CSRKeys  as  CSRKey, @statusKey  as  statusKey, @IsTransLoad  as  IsTransLoad, 
			@CustomerKeys  as  CustomerKey, @PageNo  as  PageNo,@PageSize  as  PageSize, @SortField  as  SorField, 
			@IsAscending as IsAscending, @CreatedUSer as CreatedUSer, @SearchText as SearchText, @IsExport as IsExport, 
			@CSRManagerKeys as CSRManagerKey, @SalesPersonKeys as SalesPersonKey, @LoggedUserKey as LoggedUserKey, 
			@isShowAll as isShowAll, @marketLocationKeys as marketLocationKey, @Pickuplocationkeys as Pickuplocationkey, 
			@Deliverylocationkeys as Deliverylocationkey, @WarehouseStatusKeys AS WarehouseStatusKeys

		Select '#CSMKeys',* from #CSMKeys
		select '#CSRKeys',* from #CSRKeys
		select '#CustKeys',* from #CustKeys
		Select '#DelieverLocationKeys',* from #DelieverLocationKeys
		Select '#MarketKeys',* from #MarketKeys
		Select '#SalesPersonKeys',* from #SalesPersonKeys
	End
	...
	Declare @RecCount	int, @RowNum int
		...
		SELECT
			...
			isnull(OD.isWhseChargesConfirmed,convert(bit,0)) as		isWhseChargesConfirmed, 
			isnull(OD.isCSChargeConfirmed,convert(bit,0)) as		isCSChargeConfirmed,
			isnull(OD.isChargesSharedWithCust,convert(bit,0)) as	isChargesSharedWithCust, 
			isnull(OD.isCustApprovedCharges,convert(bit,0)) as		isCustApprovedCharges,
			GICF.Holds as HoldType,
			GICF.HoldStatus, 
			case when isnull(CT.OrderDetailKey ,0) = 0 then 'N/A' else isnull(WS.Description,'Open') end as WarehouseStatus,
			ISNULL(WS.StatusKey,1) AS WarehouseStatusKey
		into #BaseData
		FROM  dbo.OrderDetail OD					WITH (NOLOCK)		
			...
			LEft join Gnosis_Integration_Container_Final GICF WITH (NOLOCK) on OD.OrderDetailKey = GICF.OrderDetailKey
		...
		WHERE  1=1 AND OH.OrderDate >= Convert(Date, '2024-10-07') AND RT.[Status] =5 and OD.status in (6,10,12,13,14) and RI.InvoiceKey is null AND ISNULL(isCustApprovedCharges,0)=0  -- AND OH.OrderNo = 'C&B20242164'
			--and isnull(CT.OrderDetailKey,00) <> 0
		...
		from #BaseData
			...
			AND (  Isnull(@CSRKeys, '') = '' OR CsrName in (Select CsrName from #CSRKeys)) 
			AND (  isnull(@statusKey,0) = 0 OR  [Status] =  @statusKey ) 
			AND (  isnull(@CustomerKeys,'') = '' OR CustKey in (Select CustKey from #CustKeys)  ) 
			...
			AND (  isnull(@SearchText ,'') = '' OR
			 ...
			 AND (  Isnull(@CSRManagerKeys, '') = '' OR CSManagerName in (Select CsrName from #CSMKeys)) 
			 AND (  ISNULL(@SalesPersonKeys,'') = '' OR SalesPersonName in (Select SalesPersonName from #SalesPersonKeys) ) 
			 AND (  ISNULL(@marketLocationKeys,'') = '' OR MarketLocation in (Select MarketLocation from #MarketKeys)) 
			-- AND (ISNULL(@Pickuplocationkey,0)=0 OR S_AddrKey=@Pickuplocationkey)
			 AND (ISNULL(@Deliverylocationkeys,'')='' OR ltrim(rtrim(D_AddrName)) in (select ltrim(rtrim(AddrName)) from #DelieverLocationKeys))
			 AND (ISNULL(@WarehouseStatusKeys,'')='' OR ltrim(rtrim(WarehouseStatusKey)) in (select ltrim(rtrim(StatusKey)) from #WarehouseStatusKeys))
		
		Select @RecCount = COUNT(1) from #Temp A
		

		Select OE.OrderDetailKey, count(1) as ItemCount
		into #ItemsList
		from OrderExpense OE WITH (NOLOCK)
		Inner join #Temp T on OE.OrderDetailKey = T.OrderDetailkey
		...
		select 
			...
			IsLinked,
			IL.ItemCount,
			isWhseChargesConfirmed, 
			isCSChargeConfirmed,
			isChargesSharedWithCust, 
			isCustApprovedCharges,
			HoldStatus,
			HoldType,
			WarehouseStatus
		into #Temp2
		...
		LEft join #ItemsList IL on A.OrderDetailkey = IL.OrderDetailKey
		...
		if(@IsCountWithFilters = 1)
		Begin
			Select @cnt = count(1) from #Temp2
		End
		else
		Begin
			Select @cnt = count(1) from #BaseData
		End	
		...
		DropDowns = ( SELECT
			CSRList = (Select distinct CsrName from  #Temp2 order by  CsrName for JSON PATH),
			CSMList = (SElect distinct CSManagerName from #Temp2 Order by CSManagerName For JSON PATH ),
			CustList = (Select distinct CustKey, CustName  from #Temp2 Order by CustName FOR JSON PATH) ,
			HoldTypeList = (Select distinct HoldType  from #Temp2 Order by HoldType FOR JSON PATH),
			HoldStatus = (SElect distinct HoldStatus from #Temp2 Order by HoldStatus For JSON PATH ),
			SalesPersonList = (SElect distinct SalesPersonName from #Temp2 Order by SalesPersonName For JSON PATH ),
			DeliveryLocationList = (SElect distinct  ltrim(rtrim(D_AddrName)) as LocationName from #Temp2 Order by ltrim(rtrim(D_AddrName)) For JSON PATH ),
			MarketLocList = (SElect distinct MarketLocation from #Temp2 Order by MarketLocation For JSON PATH ),
