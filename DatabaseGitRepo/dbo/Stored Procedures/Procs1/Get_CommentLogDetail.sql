...
CREATE Procedure [dbo].[Get_CommentLogDetail]-- [Get_CommentLogDetail] 'Container',107927
(
...
Begin
	...
	from OrderDetailComments ODC
	inner join Comment C on ODC.CommentKey = C.CommentKey
	LEft join [User] U on C.CreateUserKey = u.UserKey
	inner join OrderDetail OD on ODC.OrderDetailKey = OD.OrderDetailKey
	...
	from OrderHeaderComments ODC
	inner join Comment C on ODC.CommentKey = C.CommentKey
	LEft join [User] U on C.CreateUserKey = u.UserKey
	inner join OrderDetail OD on ODC.OrderKey = OD.OrderKey
	...
	select 1, Getdate(), U.UserName,'Warehouse Notes', OD.ContainerNo, OD.OrderDetailKey,
			...
			''
	from OrderDetail OD
	LEft join [User] U on OD.CreateUserKey = U.UserKey
	...
	from SchedulerDriverComment ODC
	Left Join Routes RT on ODC.RouteKey = RT.RouteKey
