...
DECLARE 
	@UserKey INT=512,
	@JSONString NVARCHAR(MAX)='{"Factored":2,"CustApproved":2,"PageNo":1,"PageSize":50,"Ascending":true,"IsAscending":true,"SearchText":"hmmu6023472","CustomerKey":"","MarketLocationKey":"","InvoicerKey":"","CustCompanyKey":"","ChargeConfirmed":2,"SortField":"TerminationDate","StatusKey":16,"WarehouseStatusKeys":""}',
	@Status BIT=0,@IsDebug		BIT = 1,
	@Reason VARCHAR(100)=''
EXec Get_InvoiceList_V2_deleted @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT,@IsDebug
Select @Status, @Reason
**/


CREATE Procedure [dbo].[Get_InvoiceList_V2_deleted] -- Get_InvoiceList_V2_deleted @StatusKey=2, @PageNo = 1,@SearchText='WHSU5296890'
(
	@UserKey		INT = 714,
...
BEGIN
	...
	--VALUes (@UserKey,'Get_InvoiceList_V2_deleted','Procedure Entered','',GETDATE())
	...
	select '#ContainersNotInvoiced',* from #ContainersNotInvoiced where orderdetailkey=167725
	...
	--If(@StatusKey in (0,9))
		BEGIN
			...
			GROUP BY		OH.OrderKey,oh.OrderNo,od.ContainerNo,CU.CustName, AD.City,OH.DestinationAddrKey, OD.OrderDetailKey,
							...
							OH.CustKey, OH.OrderDate, OH.BillOfLading, OT.OrderTypeKey, OrderType, BookingNo, OD.CompleteDate, 
	...
	--IF(@StatusKey in (1,2,3,0))
		BEGIN
			INSERT INTO		#ToInvoice (OrderKey, OrderNo, ContainerNo, ActualArrival, CustId, CustName, OrderDetailKey,
							...
							AgingDays,InvoicerKey,AprovedReasonCodeKey,ApprovedReasonCode,CustCompanyName,RouteKey,
							LegID,IsDataSelected,IsSelectedStatusKey,CSR,WarehouseStatus,WarehouseStatusKey, AllowInvoicing,ExpCount)
	...
	IF(@IsDebug = 1)
		BEGIN
			SELECT '#ToInvoice', Containerno, * FROM #ToInvoice
