...
DECLARE 
	@UserKey INT=952,
	@JSONString NVARCHAR(MAX)= '{"OrderDetailKey":144918,"ContainerNo":"MSKU7493563", "Weight":50.5}',
	@Status			BIT=0, @IsDebug		BIT = 1, @Reason			VARCHAR(100)=''
	EXec [Container_Update_Weight] @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @IsDebug
	Select @Status, @Reason

*/

CREATE PROCEDURE [dbo].[Container_Update_Weight]
(
	@UserKey      INT=512,
...
BEGIN
	...
	SELECT @OrderDetailKey = OrderDetailKey, @ContainerNo = ContainerNo, @Weight = [Weight]
	...
	BEGIN TRY
		UPDATE OrderDetail 
		SET [Weight]= @Weight, LastUpdateDate = GETDATE(), UpdateUserKey = @UserKey  
		WHERE OrderDetailKey= @OrderDetailKey AND ContainerNo = @ContainerNo;

		INSERT INTO AuditLogDetail (DateCreated, CreateUser, RefType, RefId, RefKey, Stage, CommentType, Comments)
		Select GETDATE(), @USerName, 'Container', @ContainerNo, @OrderDetailKey, 'Container Size', 'Text' , 'Container Weight Updated'
