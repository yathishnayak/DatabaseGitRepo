...
EXec [Get_VoucherList_V2_yogish] @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @Debug
Select @Status, @Reason
**/
CREATE Procedure [dbo].[Get_VoucherList_V2_Yogish] 
(
	@UserKey		int,
...
BEGIN
	...
	BEGIN
		...
		SELECT A.* INTO #OpenRoute FROM dbo.[routes] A  WITH (NOLOCK) 
		INNER JOIN RouteStatus S  WITH (NOLOCK) ON S.[Status]=A.[Status] 
		WHERE S.Status= @OpenStatusKey
