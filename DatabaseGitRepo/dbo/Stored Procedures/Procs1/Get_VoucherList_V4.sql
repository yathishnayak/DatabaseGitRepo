...
/*
    DECLARE 
        @UserKey        INT,
        @JSONString     NVARCHAR(MAX)='{"DriverKeys":"","OrderKeys":"","OrderNo":"","ContainerNo":"","VoucherNo":"","VoucherKeys":"","DriverHubKeys":"","WeekNum":"","MarketLocationKeys":"","TruckTypeKeys":"","CarrierMoveTypeKeys":"","SearchText":"","SortField":"voucherno","IsAscending":true,"PageSize":50,"PageNo":1,"StatusKey":9,"SearchCriteriaKey":0,"IsDriverPay":false}',
        @Status         BIT = 0 ,
        @Reason         VARCHAR(1000) = '',
        @IsDebug        BIT = 1

    EXEC Get_VoucherList_V4 @UserKey,@JSONString,@Status output,@Reason output,@IsDebug
    SELECT @Status AS Status, @Reason AS Reason
*/
CREATE PROCEDURE [dbo].[Get_VoucherList_V4]
(
    @UserKey        INT,
...
BEGIN
    ...
    BEGIN
        ...
        WHERE FR.ActualArrival IS NOT NULL
          AND (FR.DeliveryDateFrom IS NULL 
               OR FR.DeliveryDateFrom = '1900-01-01' 
    ...
    FROM #Dashboard D
    LEFT JOIN (
        SELECT StatusKey, COUNT(1) AS cnt 
        FROM #TEMPTABLE 
        GROUP BY StatusKey
