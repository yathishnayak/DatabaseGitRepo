...
CREATE Procedure [dbo].[Get_InvoiceList_JSON] -- Get_InvoiceList_JSON @StatusKey=2, @PageNo = 1,@SearchText='WHSU5296890'
(
	@StatusKey		INT= 0,
	@CustomerKey	INT= 0,
	@OrderKey		INT= 0,
...
BEGIN
	...
	Begin
		...
		where RT.[Status] =5 and OD.status in (6,10,12,13,14) and RI.InvoiceKey is null AND OH.Status<>14
	...
	Begin
		...
		where (@StatusKey = 0 OR  IH.Statuskey = @StatusKey) and AIH.InvoiceKey IS NULL AND OH.Status<>14
	End
