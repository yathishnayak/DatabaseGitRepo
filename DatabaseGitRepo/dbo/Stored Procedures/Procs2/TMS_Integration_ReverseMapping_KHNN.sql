...
CREATE Proc [dbo].[TMS_Integration_ReverseMapping_KHNN] -- TMS_Integration_ReverseMapping_KHNN 476
(
	@DataKey	int
)
as
Begin
	Declare @SiteId varchar(20) = 'KHNN'
			--@CustKey	int = 1966
	select *
	into #KHNN_Customers
	from Integration_JCB.dbo.TMS_Integration_CustomerMapping
	where Siteid = @SiteId

	update SL set StopShortCode = case 
		when  stopNumber = 1 then  'SF'
		when  stopNumber = 2 then 'ST'
		when stopNumber = 3 then 'RT'
		else 'NA' end
	from Integration_JCB.dbo.KHNN_Header KH  WITH (NOLOCK)
	inner join Integration_JCB.dbo.KHNN_ContainerList CL  WITH (NOLOCK) on Kh.DataKey = CL.DataKey
	inner join Integration_JCB.dbo.KHNN_StopList SL WITH (NOLOCK) on CL.ContainerKey = SL.ContainerKey
	where KH.dataKey = @DataKey
	...
	SElect	IH.DataKey as TKT_DataKey, 
			...
			@SiteId as SiteID
	Into #Temp_KHNN
	from Integration_JCB.dbo.KHNN_Header IH WITH (NOLOCK) 
	Inner Join (Select datakey, ContainerKey, equipmentNumber from  Integration_JCB.dbo.KHNN_ContainerList WITH (NOLOCK) ) IC on IH.DataKey = IC.DataKey
	...
	inner join #KHNN_Customers CU on OH2.CustKey = CU.Custkey
	where  OH.OrderKey is null and isnull(IH.ProcessStatus,'') = '' and IC.equipmentNumber = OD.ContainerNo
	and OH2.OrderKey is not null and IH.isAccepted =1 and IH.DataKey = @DataKey

	select '#Temp_KHNN',* from #Temp_KHNN
	...
	inner join #Temp_KHNN ND on SD.TKT_DataKey = ND.TKT_DataKey and SD.SiteID = @SiteId
	...
	inner join #KHNN_Customers CU on OH.CustKey = CU.Custkey
	...
	delete from TMS_Integration_Routes where Datakey = @DataKey and Siteid = @SiteId 


	insert into TMS_Integration_routes (SiteID, DataKey, ContainerKey, StopKey, TMS_RouteKey, TMS_LegKey, StopType )
	select TH.SiteID, TH.DataKey, TC.ContainerKey, SL.StopKey, RD.RouteKey, RD.TMS_LegKey, SL.StopShortCode
	...
	inner join Integration_JCB.dbo.KHNN_StopList SL on TC.ContainerKey = SL.ContainerKey and RD.LocationType = SL.StopShortCode
	...
	inner join #KHNN_Customers CU on OH.CustKey = CU.Custkey
	where  isnull(H.TMS_OrderKey ,0) = 0 and isnull(ProcessStatus ,'') = ''
	...
	--select *
	from Integration_JCB.dbo.KHNN_ContainerList C
	...
	inner join Integration_JCB.dbo.KHNN_Header H  WITH (NOLOCK) on C.DataKey  = H.DataKey
	...
	INNER JOIN		Integration_JCB.dbo.KHNN_ContainerList FC  WITH (NOLOCK) on SL.ContainerKey = FC.ContainerKey
	LEFT JOIN		TKT_RouteDatanew RT  WITH (NOLOCK) on SL.TMS_RouteKey = RT.Routekey and SL.TMS_LegKey = Rt.TMS_LegKey 
					and RT.LocationType = SL.StopShortCode 
	WHERE			FC.DataKey = @DataKey and  RT.RouteKey is null AND ISNULL(SL.TMS_RouteKey,0) <> 0
	...
	inner join TKT_RouteDatanew RD  WITH (NOLOCK) on  A.TMS_OrderDetailKey = RD.orderDetailKey and RD.LocationType = SL.StopShortCode
	...
	where isnull(SL.TMS_RouteKey,0) <> RD.RouteKey OR isnull(SL.TMS_LegKey,0) <> RD.TMS_legKey  and A.SiteID = @SiteId
		and A.TKT_DataKey = @DataKey and RD.LocationType = SL.StopShortCode

	
	

	select 'TKT_SyncData', * from TKT_SyncData where tkt_datakey = @DataKey
	select 'TMS_Integration_Header', * from TMS_Integration_Header where datakey = @DataKey and SiteID = 'KHNN'
	select 'TMS_Integration_Container', * from TMS_Integration_Container where datakey = @DataKey and SiteID = 'KHNN'
	select 'TMS_Integration_Routes', * from TMS_Integration_Routes where datakey = @DataKey and SiteID = 'KHNN'

	select 'KHNN_Header', * from Integration_JCB.dbo.KHNN_Header where DataKey = @DataKey
	select 'KHNN_ContainerList',* from Integration_JCB.dbo.KHNN_ContainerList where datakey = @DataKey

	select 'KHNN_StopList', b.* from Integration_JCB.dbo.KHNN_ContainerList A
	inner join Integration_JCB.dbo.KHNN_StopList B on A.ContainerKey = B.ContainerKey
	where A.DataKey = @DataKey

	drop table #Temp_KHNN
