...
CREATE PROC [dbo].[SELL_GetDrayBaseTariff] -- SELL_GetDrayBaseTariff @MarketKey = 2, @TerminalKey = 6, @zonekey = 10, @City = 'BurBank'
(
	@MarketKey		int = 0,
	@TerminalKey	int = 0,
	@ZoneKey		int = 0,
...
BEGIN
	...
	select @BasePercentSMB = BasePercent from CustomerSegments where CustomerSegment = 'SMB'
	select @BasePercentENT = BasePercent from CustomerSegments where CustomerSegment = 'ENT'
	
	select @SpotCount = count(1) from Sell_DrayBaseSpotTariff
	...
	Begin
		...
		From (
			Select * from (
				select ROW_NUMBER() over(partition by MarketKey, TerminalKey, ZoneKey, city  
							Order by MarketKey, TerminalKey, ZoneKey, city, TotalCost Desc ) as HighestAll,
					ROW_NUMBER() over(partition by MarketKey, TerminalKey, ZoneKey, city , YardPortType
							Order by MarketKey, TerminalKey, ZoneKey, city,YardPortType, TotalCost Desc ) as HighestYardPort,
						* ,
				...
				from (
					select  ML.MarketLocation as Market, ML.MarketLocationKey as MarketKey,
					...
					inner join PriceGrouping T on T.MarketLocationKey = ML.MarketLocationKey AND co.Terminal = t.PriceGrouping
					inner join cost_Zones Z on ML.MarketLocationKey = Z.MarketKey AND co.Zone = z.ZoneName
	...
	Begin
		Select St.MarketKey, TerminalKey, City, State, ZipCode, St.ZoneKey, DrayBaseValue, DrayBaseValue as DrayBase_Cost,
		...
		into #TempData1
		from Sell_DrayBaseSpotTariff ST
		inner join MarketLocation ML on St.MarketKey = ML.MarketLocationKey
		inner join PriceGrouping PG on St.TerminalKey = PG.PriceGroupingKey
		inner join cost_Zones Z on ST.ZoneKey = Z.ZoneKey
