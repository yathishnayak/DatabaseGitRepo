...
CREATE proc [dbo].[TMS_Integration_GetScheduleAndActuals_khnn_Delete] -- [TMS_Integration_GetScheduleAndActuals_khnn_Delete] 117692,3942
(
	@TMS_OrderKey	int,
	@DataKey		int
)
...
BEGIN
	...
	SELECT * FROM TKT_RouteDataNew DT
			...
			where  OT.OrderType = 'IMPORT' and isnull(RT.IsDryRun ,0) = 0 and DT.LocationType = 'SF'
	...
	Begin
		...
		from (
			...
			INNER JOIN TMS_Integration_Routes TR ON SD.TKT_DataKey = TR.DataKey AND TR.SiteID =@SiteID
				AND TR.TMS_RouteKey = RT.RouteKey 
			inner join integration_jcb.dbo.KHNN_StopList SL on TR.StopKey = SL.StopKey and Dt.LocationType = SL.StopShortCode
			where  OT.OrderType = 'IMPORT' and isnull(RT.IsDryRun ,0) = 0 and DT.LocationType = 'SF'
			and SD.SiteID = @SiteID --and SL.stopNumber = 1
			and OD.Orderkey = @TMS_OrderKey and TR.DataKey = @DataKey
		) A
