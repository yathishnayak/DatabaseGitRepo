...
CREATE proc [dbo].[TMS_Integration_GetOrderInfo_Century_Delete]
as
SET NOCOUNT ON
SET FMTONLY OFF
...
INNER JOIN	(SELECT * FROM TMS_Integration_Customers WITH (NOLOCK) WHERE SiteID = 'Century' ) TIC ON OH.CustKey = TIC.CustKey 
INNER JOIN	(SELECT * FROM OrderDetail WITH (NOLOCK) WHERE ContainerNo = 'MSDU5636783') OD  on OD.OrderKey = OH.OrderKey
