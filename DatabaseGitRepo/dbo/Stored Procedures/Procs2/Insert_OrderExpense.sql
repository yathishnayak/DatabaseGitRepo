CREATE PROCEDURE [dbo].[Insert_OrderExpense]  -- [Insert_OrderExpense] '5|10|15|01-17-2021 10:45:00|01-07-2021 11:45:00',277,1,1
/*
Scheduler Screen
*/
...
BEGIN
	...
	END   
		...
		BEGIN TRY
			...
			IF @IsVerified =1
			BEGIN
				INSERT INTO [dbo].[OrderExpense]([Itemkey],[RouteKey],[UnitCost],NewUnitCost,Qty,
					[CreateDate],[CreateUserKey],DateFrom,DateTo, OrderDetailKey)
			...
			BEGIN	
				...
				SELECT distinct	I.ItemKey,@RouteKey,ISNULL(FR.Rate,0) AS UnitCost,I.Qty AS Qty,NULL AS NewUnitCost,
						GETDATE() AS CreateDate,@CreateUserKey ,I.DateFrom,I.DateTo, @OrderDetailKey
				FROM #ItemData I 
					INNER JOIN dbo.Item IT ON I.Itemkey=IT.ItemKey
