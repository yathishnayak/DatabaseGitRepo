...
CREATE PROC [dbo].[TMS_INTEGRATION_InsertPPStops_Century]
AS
...
BEGIN
	...
	LEFT JOIN		Integration_JCB.DBO.Century_StopList SL ON CL.ContainerKey = SL.ContainerKey AND RTN.LocationType = SL.facilityCode
	WHERE			rtn.LocationType = 'PP'   and sl.stopkey is null -- and FH.DataKey = 13142  
	...
	SELECT			ContainerKey,'Pre-Pull', AddrName, 3, 'PP', 3,
					address1,Address2, City,State, Country, ZipCode, equipmentNumber, equipmentTypeCode,0,'PP'
	FROM			#TMP
