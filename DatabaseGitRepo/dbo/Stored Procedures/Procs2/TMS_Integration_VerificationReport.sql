...
EXEC TMS_Integration_VerificationReport  '','FANU3008357','','', @Res OUTPUT
SELECT @Res
*/

CREATE PROCEDURE [dbo].[TMS_Integration_VerificationReport]
(
		 @OrderNo			VARCHAR(20)	 = ''
...
where (OrderNo =  @OrderNo  OR 
		...
		case when isnull(OD.bookingno,OH.BookingNo) = '' then '--' else isnull(OD.bookingno,OH.BookingNo) end = @WorkOrderNo)
