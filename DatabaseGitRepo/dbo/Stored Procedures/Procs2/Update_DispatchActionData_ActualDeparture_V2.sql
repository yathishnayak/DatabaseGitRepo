...
Declare 
	...
	@Reason			VARCHAR(1000) = '',
	@IsDebug		BIT = 0,
	@JSONSTRING		NVARCHAR(Max) = '{"RouteKey":727504, "ActualDeparture":"2025-07-06 08:32", "UpdateType":"JCB User"}'
	EXEC [Update_DispatchActionData_ActualDeparture_V2] @Userkey, @JSONSTRING, @Status OUTPUT, @Reason Output, @IsDebug
    SELECT @Status AS Status, @Reason AS Reason
**/

CREATE PROCEDURE [dbo].[Update_DispatchActionData_ActualDeparture_V2]
-- EXEC [Update_DispatchActionData_ActualDeparture] 727577, '2025-12-09', 953, 'JCB User' 
/*Dispatch Screen*/
(
	@UserKey		INT ,
...
(
    ...
    ActualDeparture		DATETIME    '$.ActualDeparture',
	...
	SET 		
		...
		ActualDepartureUpdateMethod=@UpdateType
	WHERE RouteKey= @RouteKey;

	update ODS SET 
		ActualPickupDate = @ActualDeparture,
		ActualPickupUserKey = @UserKey,
		ActualPickupSetDateTime = GetDate()
	...
	SET @Comment = 'Actual Departure added for leg '+@LegId + ', changed by '+@USerName -- + @USerName + ' on ' + convert(varchar, getdate(),101) + ' ' + convert(varchar, getdate(),108);
	...
	if(@cnt > 0 )
	begin
		update DriverRoute set DriverStartDate = @ActualDeparture
	...
	Begin
		...
		select @RouteKey, @driverKey, @ActualDeparture, null
	End
