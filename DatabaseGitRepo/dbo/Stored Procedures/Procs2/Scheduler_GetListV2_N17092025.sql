...
DECLARE 
	...
	@Reason VARCHAR(100)=''
EXec Scheduler_GetListV2_N17092025 @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @IsDebug
Select @Status, @Reason
**/
CREATE PROCEDURE [dbo].[Scheduler_GetListV2_N17092025]   
	@UserKey		int,
...
INSERT INTO SqlExecutionTimeLog
	...
	VALUes (@UserKey,'Scheduler_GetListV2_N17092025','Procedure Entered','',GETDATE())
	SET NOCOUNT ON;
	SET FMTONLY OFF;
	--SET ARITHABORT ON;
	...
	END
		...
		SELECT
			...
			--ODS.ActualPickupDate AS ActualPickupDate,
			--CONVERT(VARCHAR(10), CAST(ODSD.ActualPickupDate AS TIME), 0) ActualPickupTime,
			--ODSD.ActualDeliveryDate AS ActualDeliveryDate,
			--CONVERT(VARCHAR(10), CAST(ODSD.ActualPickupDate AS TIME), 0) ActualDeliveryTime,
