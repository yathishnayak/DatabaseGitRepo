...
DECLARE 
		@UserKey		INT,
		@JSONString		NVARCHAR(max) = '{"OrderNo":"","ContainerNo":"FANU3261658", "ShipmentRefNo":"", "WorkOrderNo":""}',
		@Status			BIT = 0,
		@Reason			VARCHAR(1000) = '',
		@IsDebug		BIT = 0
EXEC [TMS_Integration_VerificationReport_V2] @Userkey, @JSONSTRING, @Status OUTPUT, @Reason Output, @IsDebug
SELECT @Status, @Reason 
*/

CREATE PROCEDURE [dbo].[TMS_Integration_VerificationReport_V2]
(
		@UserKey		INT = 1144,
...
WITH(
	...
	ShipmentRefNo		VARCHAR(50)	'$.ShipmentRefNo',
...
where (OrderNo =  @OrderNo  OR 
		...
		case when OH.BookingNo = '' then '--' else OH.BookingNo end = @WorkOrderNo)
...
BEGIN
	INSERT INTO #TmpOrderKey 
	SELECT OH.OrderKey FROM ORderHeader OH WHERE OH.BookingNo LIKE '%' + @WorkOrderNo + '%'
END
...
Begin
	...
	from Integration_JCB.dbo.Flexpro_Header FH WITH (NOLOCK) 
	inner join Integration_JCB.dbo.Flexpro_ContainerList FC WITH (NOLOCK)  on FH.DataKey = FC.DataKey
...
Begin
	insert into #Integration (SiteID, DataKey, TMS_ORderKey,  ContainerNo, WorkOrderNumber, TMS_ORderDetailKey)
	select @SiteID, FH.datakey, FH.TMS_OrderKey, FC.EquipmentNumber, FH.WorkOrderNumber, FC.TMSOrderDetailKey
	from Integration_JCB.dbo.Robinson_Header FH WITH (NOLOCK) 
	inner join Integration_JCB.dbo.Robinson_ContainerList FC WITH (NOLOCK)  on FH.DataKey = FC.DataKey
...
Begin
	insert into #Integration (SiteID, DataKey, TMS_ORderKey,  ContainerNo, WorkOrderNumber, TMS_ORderDetailKey)
	select @siteID, FH.datakey, FH.TMS_OrderKey, FC.EquipmentNumber, FH.WorkOrderNumber, FC.TMSOrderDetailKey
	from Integration_JCB.dbo.DHL_Header FH  WITH (NOLOCK) 
	inner join Integration_JCB.dbo.DHL_ContainerList FC WITH (NOLOCK)  on FH.DataKey = FC.DataKey
...
Begin
	insert into #Integration (SiteID, DataKey, TMS_ORderKey,  ContainerNo, WorkOrderNumber, TMS_ORderDetailKey)
	select @SiteID, FH.datakey, FH.TMS_OrderKey, FC.EquipmentNumber, FH.WorkOrderNumber, FC.TMSOrderDetailKey
	from Integration_JCB.dbo.EDRAY_Header FH WITH (NOLOCK) 
	inner join Integration_JCB.dbo.EDRAY_ContainerList FC WITH (NOLOCK)  on FH.DataKey = FC.DataKey
...
Begin
	...
	from Integration_JCB.dbo.KHNN_Header FH WITH (NOLOCK) 
	inner join Integration_JCB.dbo.KHNN_ContainerList FC WITH (NOLOCK)  on FH.DataKey = FC.DataKey
...
Begin
	insert into #Integration (SiteID, DataKey, TMS_ORderKey,  ContainerNo, WorkOrderNumber, TMS_ORderDetailKey)
	select @SiteID, FH.datakey, FH.TMS_OrderKey, FC.EquipmentNumber, FH.WorkOrderNumber, FC.TMSOrderDetailKey
	from Integration_JCB.dbo.ACER_Header FH WITH (NOLOCK) 
	inner join Integration_JCB.dbo.ACER_ContainerList FC WITH (NOLOCK)  on FH.DataKey = FC.DataKey
...
LEFT JOIN			TMS_Integration_Container TC WITH (NOLOCK)  ON TH.DataKey = TC.DataKey AND TH.SiteID = TC.SiteID 
...
select @SiteID as Siteid, A.DataKey, DocKey, DocContent, DocControlNo, DocCreated, DocUploaded 
	from Integration_JCB.dbo.Flexpro_990DocData A WITH (NOLOCK) 
...
insert into #990 (SiteID, DataKey, DocKey, DocContent, DocControlNo, DocCreated, DocUploaded )
select @SiteID as Siteid, A.DataKey, DocKey, DocContent, DocControlNo, DocCreated, DocUploaded 
	from Integration_JCB.dbo.Robinson_990DocData A WITH (NOLOCK) 
	inner join #SelectedRecord I on a.DataKey = I.DataKey
	where SiteID = @SiteID
End
if(@Siteid = 'DHL')
...
insert into #990 (SiteID, DataKey, DocKey, DocContent, DocControlNo, DocCreated, DocUploaded )
select @SiteID as Siteid, A.DataKey, DocKey, DocContent, DocControlNo, DocCreated, DocUploaded 
	from Integration_JCB.dbo.DHL_990DocData A WITH (NOLOCK) 
	inner join #SelectedRecord I on a.DataKey = I.DataKey
	where SiteID = @SiteID
End
...
Begin
	...
	select @SiteID as Siteid, A.DataKey, DocKey, Doc_Content, '', Doc_Date, Doc_Date
		from Integration_JCB.dbo.EDRAY_990Docs A WITH (NOLOCK) 
		inner join #SelectedRecord I on a.DataKey = I.DataKey
		where SiteID = @SiteID
End
...
Begin
	...
	select @SiteID as Siteid, A.DataKey, DocKey, '', --Doc_Content, 
		'', Doc_Date, Doc_Date
		from Integration_JCB.dbo.KHNN_990Docs A WITH (NOLOCK) 
		inner join #SelectedRecord I on a.DataKey = I.DataKey
		where SiteID = @SiteID
End
...
Begin
	...
	select 'Flexport' as Siteid,FH.FileProcessKey, FH.DataKey, DocKey, DocContent, DocControlNo, DocCreated, DocUploaded 
		from Integration_JCB.dbo.Flexpro_997DocData A WITH (NOLOCK) 
		inner join Integration_JCB.dbo.Flexpro_Header FH WITH (NOLOCK)  on A.FileProcessKey = FH.FileProcessKey
...
Begin
	...
	select 'Robinson' as Siteid,FH.FileProcessKey, FH.DataKey, DocKey, DocContent, DocControlNo, DocCreated, DocUploaded 
		from Integration_JCB.dbo.Robinson_997DocData A WITH (NOLOCK) 
		inner join Integration_JCB.dbo.Robinson_Header FH WITH (NOLOCK)  on A.FileProcessKey = FH.FileProcessKey
		inner join #SelectedRecord I on FH.DataKey = I.DataKey
		where SiteID = @SiteID
End
...
Begin
	...
	select 'DHL' as Siteid, FH.FileProcessKey, FH.DataKey, DocKey, DocContent, DocControlNo, DocCreated, DocUploaded 
		from Integration_JCB.dbo.DHL_997DocData A WITH (NOLOCK) 
		inner join Integration_JCB.dbo.DHL_Header FH  WITH (NOLOCK) on A.FileProcessKey = FH.FileProcessKey
		inner join #SelectedRecord I on FH.DataKey = I.DataKey
		where SiteID = @SiteID
End
...
Begin
	...
	from Integration_JCB.dbo.Flexpro_214DocData A WITH (NOLOCK) 
	inner join #SelectedRecord I on A.DataKey = I.DataKey
	where SiteID = @SiteID
End
if(@Siteid = 'Robinson')
Begin
	...
	from Integration_JCB.dbo.Robinson_214DocData A WITH (NOLOCK) 
	inner join #SelectedRecord I on A.DataKey = I.DataKey
	where SiteID = @SiteID
End
if(@Siteid = 'DHL')
Begin
	...
	from Integration_JCB.dbo.DHL_214DocData A WITH (NOLOCK) 
	inner join #SelectedRecord I on A.DataKey = I.DataKey
	where SiteID = @SiteID
End
...
Begin
	...
	from Integration_JCB.dbo.EDRAY_214Docs A WITH (NOLOCK) 
	inner join #SelectedRecord I on A.DataKey = I.DataKey
	where SiteID = @SiteID
End
...
Begin
	...
	from Integration_JCB.dbo.ACER_214DocData A WITH (NOLOCK) 
	inner join #SelectedRecord I on A.DataKey = I.DataKey
	where SiteID = @SiteID
End
...
Begin
	...
	select FH.DataKey, ContainerKey, EquipmentNumber, EquipmentTypeCode, PieceCount, GrossWeight, WeightUOM, Volume, VolumeUOM, 
			FreightDescription, IsHazmat, SealNumberList, TMS_ContainerSizeKey, TMSOrderDetailKey, HazardInfo
	from Integration_JCB.dbo.Flexpro_ContainerList FH WITH (NOLOCK)  
	inner join #SelectedRecord I on Fh.DataKey = I.DataKey
...
Begin
	...
	select FH.DataKey, ContainerKey, EquipmentNumber, EquipmentTypeCode, PieceCount, GrossWeight, WeightUOM, Volume, VolumeUOM, 
			FreightDescription, IsHazmat, SealNumberList, TMS_ContainerSizeKey, TMSOrderDetailKey, HazardInfo
	from Integration_JCB.dbo.Robinson_ContainerList FH WITH (NOLOCK)  
	inner join #SelectedRecord I on Fh.DataKey = I.DataKey
...
Begin
	...
	select FH.DataKey, ContainerKey, EquipmentNumber, EquipmentTypeCode, PieceCount, GrossWeight, WeightUOM, Volume, VolumeUOM, 
			FreightDescription, IsHazmat, SealNumberList, TMS_ContainerSizeKey, TMSOrderDetailKey, HazardInfo
	from Integration_JCB.dbo.DHL_ContainerList FH WITH (NOLOCK)  
	inner join #SelectedRecord I on Fh.DataKey = I.DataKey
	where I.SiteId = 'DHL'
End
...
Begin
	...
	select FH.DataKey, ContainerKey, EquipmentNumber, EquipmentTypeCode, PieceCount, GrossWeight, WeightUOM, Volume, VolumeUOM, 
			FreightDescription, IsHazmat, SealNumberList, TMS_ContainerSizeKey, TMSOrderDetailKey, HazardInfo
	from Integration_JCB.dbo.EDRAY_ContainerList FH  WITH (NOLOCK) 
	inner join #SelectedRecord I on Fh.DataKey = I.DataKey
...
Begin
	...
	from Integration_JCB.dbo.KHNN_ContainerList FH WITH (NOLOCK)  
	inner join #SelectedRecord I on Fh.DataKey = I.DataKey
	where I.SiteId = 'KHNN'
End
if(@Siteid = 'ACER')
Begin
	...
	select FH.DataKey, ContainerKey, EquipmentNumber, EquipmentTypeCode, PieceCount, GrossWeight, WeightUOM, Volume, VolumeUOM, 
			FreightDescription, IsHazmat, SealNumberList, TMS_ContainerSizeKey, TMSOrderDetailKey, HazardInfo
	from Integration_JCB.dbo.ACER_ContainerList FH WITH (NOLOCK)  
	inner join #SelectedRecord I on Fh.DataKey = I.DataKey
	where I.SiteId = 'ACER'
End
