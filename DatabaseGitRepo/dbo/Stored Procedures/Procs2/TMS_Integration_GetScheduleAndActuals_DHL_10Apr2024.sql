...
--select * from TMS_Integration_Header where siteid = 'DHL' and Datakey = 2815

CREATE proc [dbo].[TMS_Integration_GetScheduleAndActuals_DHL_10Apr2024] -- TMS_Integration_GetScheduleAndActuals_DHL 85325, 4238
(
	@TMS_OrderKey	int,
	@DataKey		int
)
	...
	Begin
		...
		from (
			...
			INNER JOIN TMS_Integration_Routes TR ON SD.TKT_DataKey = TR.DataKey AND TR.SiteID = @SiteID
				AND TR.TMS_RouteKey = RT.RouteKey
			where OT.OrderType = 'Export' and isnull(RT.IsDryRun ,0) = 0
			and (  L.ToLocation = 'PORT' and isnull(RT.IsEmpty,0) = 0)
			and SD.SiteID = @SiteID
			and OD.OrderKey = @TMS_OrderKey and TR.DataKey = @DataKey
		) A
	End
