...
CREATE Proc [dbo].[TMS_Integration_ReverseMapping_Melrose] -- TMS_Integration_ReverseMapping_Melrose 4
(
	@DataKey	int
)
as
Begin
	Declare @SiteId varchar(20) = 'Melrose'
			--@CustKey	int = 1715
	--select * from TKT_SyncData where left(TMS_BrokerRefNo,5) <> 'FLEX-'
	print '1'
	select custkey into #MelroseCustKeys from Customer where custname like '%robins%'

	DECLARE @ContainerList INT = (SELECT ContainerKey FROM Integration_JCB.dbo.Melrose_ContainerList WHERE DataKey = @DataKey )
	...
	from Integration_JCB.dbo.Melrose_Header KH  WITH (NOLOCK)
	inner join OrderHeader OH  WITH (NOLOCK) on KH.TMS_OrderKey = OH.orderkey
	inner join Integration_JCB.dbo.Melrose_ContainerList CL  WITH (NOLOCK) on Kh.DataKey = CL.DataKey
	...
	SElect	IH.DataKey as TKT_DataKey, 
			...
			@SiteId as SiteID
	Into #Temp_Melrose
	from Integration_JCB.dbo.Melrose_Header IH WITH (NOLOCK) 
	Inner Join (Select datakey, ContainerKey, equipmentNumber from  Integration_JCB.dbo.Melrose_ContainerList WITH (NOLOCK) ) IC on IH.DataKey = IC.DataKey
	...
	inner join #MelroseCustKeys CK on OH.CustKey = CK.CustKey
	...
	select '#Temp_Melrose',* from #Temp_Melrose
	...
	inner join #Temp_Melrose ND on SD.TKT_DataKey = ND.TKT_DataKey and SD.SiteID = @SiteId
	...
	from #Temp_Melrose ND WITH (NOLOCK) 
	...
	update H set TMS_OrderKey = A.TMS_OrderKey, TMS_CSRKey = OH.CsrKey, TMS_CarrierKey = OH.CarrierKey,
		...
		TMS_MarketLocationKey = OH.MarketLocationKey
	--select *
	...
	--select *
	from Integration_JCB.dbo.Melrose_ContainerList C
	...
	inner join Integration_JCB.dbo.Melrose_Header H  WITH (NOLOCK) on C.DataKey  = H.DataKey
	...
	inner join Integration_JCB.dbo.Melrose_ContainerList FC  WITH (NOLOCK) on SL.ContainerKey = FC.ContainerKey
	...
	where A.OrderDetailKey is null   and SD.TKT_DataKey = @DataKey )
	...
	delete from TMS_Integration_Routes where Siteid = 'Melrose' and  TMS_RouteKey in (
	...
	from TKT_RouteData RD WITH (NOLOCK) 
	inner join TKT_SyncData SD  WITH (NOLOCK) on SD.TMS_OrderDetailKey = RD.OrderDetailKey
	inner join Integration_JCB.dbo.Melrose_StopList SL  WITH (NOLOCK) on SL.ContainerKey = SD.TKT_ContainerKey and RD.StopType = SL.stopNumber
	...
	SELECT 'TMS_Integration_Routes', * FROM TMS_Integration_Routes WHERE SiteID = 'Melrose'
	Print '20'
	update  SL set TMS_LegKey = RD.legKey, TMS_RouteKey = RD.RouteKey, TMS_SourceAddrKey = RT.SourceAddrKey,
		TMS_DestinationAddrKey = RT.DestinationAddrKey
	--select *
	...
	SELECT 'TMS_Integration_Routes 11', * FROM TMS_Integration_Routes WHERE SiteID = 'Melrose'
	EXEC TMS_Integration_Melrose_PortToConsignee @DataKey
	EXEC TMS_Integration_Melrose_ConsigneetoPort @DataKey
	SELECT 'TMS_Integration_Routes 22', * FROM TMS_Integration_Routes WHERE SiteID = 'Melrose'
	...
	select 'Melrose_Header', * from Integration_JCB.dbo.Melrose_Header where DataKey = @DataKey
	select 'Melrose_ContainerList',* from Integration_JCB.dbo.Melrose_ContainerList where datakey = @DataKey

	select 'Melrose_StopList', b.* from Integration_JCB.dbo.Melrose_ContainerList A
	inner join Integration_JCB.dbo.Melrose_StopList B on A.ContainerKey = B.ContainerKey
	where A.DataKey = @DataKey

	drop table #Temp_Melrose
