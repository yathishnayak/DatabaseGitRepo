...
CREATE	PROCEDURE	[dbo].[TMS_integration_PickupDropAddressUpdate_Melrose_BKP20240620] -- TMS_integration_PickupDropAddressUpdate_Melrose '[{"Datakey":188}]'
(
	@Json NVARCHAR(MAX)
)
...
INTO		#TMPData
FROM		(SELECT		H.Datakey Datakey, Min(CL.ContainerKey)ContainerKey, MIN(SL.Stopkey)Stopkey, stopType, stopName,address1,Address2,city,state,country,postalCode, TMS_CustKey , TMS_MarketLocationKey			
			...
			GROUP BY	H.Datakey,stopType, stopName,address1,Address2,city,state,country,postalCode, TMS_CustKey , TMS_MarketLocationKey	) A	
...
WHILE (@i <= @n)
	BEGIN
		...
		SELECT		@LocationName = stopName ,@Addr1 = address1,@Addr2 = Address2,@City = city,@State = state, @Country = country,@Postal = postalCode 
					,@DataKey = Datakey, @StopType = StopType, @CustKey = TMS_CustKey, @StopName = stopName, @MarketLocationKey = TMS_MarketLocationKey
		FROM		#TMPData
		WHERE		Sl = @i
