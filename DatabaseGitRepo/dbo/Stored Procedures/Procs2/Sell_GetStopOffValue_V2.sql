...
Declare 
	...
	@Reason			VARCHAR(1000) = '',
	@IsDebug		BIT = 0,
	@JSONSTRING		NVARCHAR(Max) = '{"MarketLocationKey" : 3, "ZoneKey": 11, "TerminalKey" : 4, "YardType" : "Local"}'
	EXEC [Sell_GetStopOffValue_V2] @Userkey, @JSONSTRING, @Status OUTPUT, @Reason Output, @IsDebug
	SELECT @Status AS Status, @Reason AS Reason
**/


CREATE PROCEDURE [dbo].[Sell_GetStopOffValue_V2] -- Sell_GetStopOffValue 3, 11, 4, 'Local'
(
	@UserKey		INT = 1144,
...
Begin
	...
	select isnull(Sc.SellConfigKey,0) as SellConfigKey,  
		max(isnull(isnull(SC.StopOffValue,PP.StopOffCost),0)) as StopOffCost
	from MarketLocation ML WITH(NOLOCK) 
	LEft join PriceGrouping T WITH(NOLOCK) on ML.MarketLocationKey = T.MarketLocationKey
	LEft Join COST_CostDataOutput_StopOff PP WITH(NOLOCK) on ml.MarketLocation = PP.Market and PP.Terminal = T.PriceGrouping
...
--	where ML.MarketLocationKey = @MarketKey and T.PriceGroupingKey = @TerminalKey and Z.ZoneKey = @ZoneKey
	where ML.MarketLocationKey = @MarketKey and T.PriceGroupingKey = @TerminalKey and Z.ZoneKey = @ZoneKey and
		PP.StopOfflocation = case when isnull(@YardType,'ALL') = 'ALL' then PP.StopOfflocation else  @YardType end
