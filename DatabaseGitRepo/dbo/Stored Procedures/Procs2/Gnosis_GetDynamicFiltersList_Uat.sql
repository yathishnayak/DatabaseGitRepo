...
exec Gnosis_GetDynamicFiltersList_Uat @UserKey, @JSONString, @Status output, @Reason output
select @Status, @Reason
*/
CREATE PROCEDURE [dbo].[Gnosis_GetDynamicFiltersList_Uat]   
(
	@UserKey		int,
...
BEGIN
	...
	BEGIN
		...
		from #CSMKeys C
		inner join CSR R on REPLACE(REPLACE(LTRIM(RTRIM(C.CSMName)),CHAR(10),''),CHAR(9),'') = R.CsrName 
	END
	...
	select *    from #MktLocation

		SElect distinct  CD.Container_type as ContainerSize,
			cd.ContainerStatus,
			REPLACE(REPLACE(LTRIM(RTRIM(isnull(C.OrderCSR, CM.CsrName))),CHAR(10),''),CHAR(9),'') CSRName, CM.CsrName as  CSMName,OH.CSRManagerKey,
			C.Customer, CD.Available_for_pickup as PickupAvailable,
