...
DECLARE 
	...
	@Reason VARCHAR(100)=''
EXec Scheduler_GetListV2_Shiva_20260123 @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @IsDebug
Select @Status, @Reason
**/
CREATE PROCEDURE [dbo].[Scheduler_GetListV2_Shiva_20260123]   
	@UserKey		int,
...
INSERT INTO SqlExecutionTimeLog
	...
	select Od.orderdetailkey, OH.orderKey, OD.Status
	...
	where  Isnull(@SearchText,'') = '' AND (
		--(Isnull(@ContainerNo,'') = '' OR ContainerNo = @ContainerNo) AND
		(ISNULL(@PickupAvailable,0 ) = 0 or @PickupAvailable = ISNULL(CGD.Available,GICF.Available_for_pickup)) AND
		...
		(Isnull(@MarketKeys,'') = '' OR OH.MarketLocationKey in (select MarketKey from #MarketKeys )) AND 
		...
		AND (ISNULL(@Tracking,'')='' OR @Tracking= case when CASE WHEN ISNULL(GICF.OrderDetailKey,0)=0 THEN CAST(0 AS BIT) ELSE CAST(1 AS BIT) END = 1 then 'Yes' else 'No' end )
		AND (ISNULL(@Deliverylocationkeys,'')='' OR ltrim(rtrim(CDT.AddrName)) in (Select DeliverocationKey from #DelieverLocationKeys ))
		AND	(ISNULL(@OrderType ,'') =''  OR COALESCE(OD.OrderTypeKey, OH.OrderTypeKey, 0) in (SElect OrderTypeKey from #OrderTypeKeys) )
		)

		select distinct status, count(1) as cnt from 
		#RequiredRecords
		group by Status
		order by status
		...
		into #TempAll
		FROM  #RequiredRecords OD1					WITH (NOLOCK)	
			INNER JOIN dbo.OrderDetail OD			WITH (NOLOCK)	ON OD.OrderDetailkey = OD1.OrderDetailkey
			INNER JOIN dbo.OrderHeader OH			WITH (NOLOCK)	ON OH.OrderKey=OD.OrderKey
			INNER JOIN dbo.OrderStatus OS			WITH (NOLOCK)	ON OS.[Status]=OH.[Status]
			LEFT JOIN dbo.[Broker]  BR				WITH (NOLOCK)	ON BR.BrokerKey=OH.BrokerKey
