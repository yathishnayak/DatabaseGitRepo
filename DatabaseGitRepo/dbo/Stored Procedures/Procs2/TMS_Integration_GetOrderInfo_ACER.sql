...
CREATE proc [dbo].[TMS_Integration_GetOrderInfo_ACER]
as
SET NOCOUNT ON
SET FMTONLY OFF
...
from  (SELECT * FROM OrderDetail WHERE ISNULL(IsEmpty,0) = 0) OD
...
where OH.CustKey = @CustKey and   isnull(CP.OrderSent,0) = 0  AND LEFT(LTRIM(RTRIM(OD.ContainerNo)),4) NOT IN ('UUUU','JFLT') )A
