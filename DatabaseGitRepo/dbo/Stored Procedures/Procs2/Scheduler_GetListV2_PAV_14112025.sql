...
DECLARE 
	...
	{"ContainerNo":"","ThisWeek":false,"Today":false,"Arrived":false,"NextWeek":false,"ThisMonth":false,"DemurrageStatus":false,"DetentionStatus":false,"NearingDemurrage":false,"WithDemurrage":false,"WithDetention":false,"Terminal":"","PageNo":1,"PageSize":50,"SortField":"OrderNo","IsAscending":true,"CSMKeys":"","CSRKeys":"","ContainerStatusKeys":"","CustKeys":"","SalesPersonKeys":"","HoldStatus":"","TerminalNames":"","TerminalCodes":"","VesselIMOs":"","MarketKeys":"","SearchText":"IMPT2511999,IMPT2511972,IMPT2511966,JFTL2511967,IMPT2511964,UUUU2511965","DischargeYN":"","RangeName":"","ContainerStatus":"","TruckStatus":"","IsMissingUpdates":false,"IsUnapprovedCharges":false,"PickupAvailable":"","HoldTypes":"","PickUpFrom":null,"PickUpTo":null,"StatusKey":15,"OrderType":"","Deliverylocationkeys":"","Tracking":""}',
	@Status BIT=0, @IsDebug bit = 0,
	@Reason VARCHAR(100)=''
EXec Scheduler_GetListV2_PAV_14112025 @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @IsDebug
Select @Status, @Reason
**/
CREATE PROCEDURE [dbo].[Scheduler_GetListV2_PAV_14112025]   
	@UserKey		int,
	@JSONString		nvarchar(max),
	@Status			bit	= 0 output,
...
INSERT INTO SqlExecutionTimeLog
	...
	VALUes (@UserKey,'Scheduler_GetListV2_PAV_14112025','Procedure Entered','',GETDATE())
	SET NOCOUNT ON;
	SET FMTONLY OFF;
	--SET ARITHABORT ON;
	...
	Begin
		...
		BEGIN
			...
			WHERE ContainerNo IN (SELECT VALUE FROM fn_splitparam(@SearchText)) AND 
			OD.CreateDate >= DATEADD(hour, -48, GETDATE());;
		END
		ELSE
		BEGIN
		Insert into #OrderDetailKeys
	...
	BEGIN
		...
		WHERE status IN (1,2,3,7,9,6,12,14,17)
	END
	...
	END
		...
		--select IsChargesApproved FROM #TempAll
		...
		*/
		...
		INSERT INTO SqlExecutionTimeLog
	(UserKEY,ProcedureName,CommentText,AdditionalInfo,CreatedDate)
	VALUes (@UserKey,'Scheduler_GetListV2_PAV_14112025','Procedure Execution end','',GETDATE())
		SET @Status=1
		SET @Reason='SUCCESSS'
