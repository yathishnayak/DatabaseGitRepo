...
--select top 100 * from orderDetailStops order by OrderDetailKey desc
/*
Create proc to get the OrderDetailwise Stops. Proc Name: OrderDetail_GetStopList
The Proc should standard with JSON param and output
Should send all stops with/without data for a orderDetailkey passed
*/
/**
DECLARE @UserKey INT=29,
	@JSONString NVARCHAR(MAX)='{"OrderDetailKey":237291}',@Status BIT=0,@IsDebug		BIT = 0, 
	@JsonOutput nvarchar(max) ='', 	@Reason VARCHAR(100)=''
Exec OrderDetail_GetStopList @UserKey,@JSONString,@JsonOutput OUTPUT,@Status OUTPUT,@Reason OUTPUT, @IsDebug
Select @JsonOutput, @Status, @Reason

**/
CREATE PROC [dbo].[OrderDetail_GetStopList]
(
	@UserKey			int,
...
BEGIN
	...
	Declare @Cnt int 
	select @cnt = count(1) from OrderDetailStops where isnull(StopNumber,0) = 0
	...
	set @IsExists = case when (select count(1) from OrderDetailStops where OrderDetailKey = @OrderDetailKey) > 0 then 1 else 0 end
		...
		Select OrderDetailStopKey, OrderStopKey, OS.StopTypeKey, StopTypeName, OrderDetailKey,StopAddrKey,
					...
					LastDryrunPortSFDate=(SELECT 
										STUFF((
												...
												FROM OrderDetailStops where OrderDetailKey=@ORderDetailKey AND IsDryRunPort=1 AND StopTypeKey=1
												FOR XML PATH('')
					...
					LastDryrunPortRTDate=(SELECT 
										STUFF((
												SELECT ', ' + CONVERT(VARCHAR, DryRunPortSetDateTime, 101)
												FROM OrderDetailStops where OrderDetailKey=@ORderDetailKey AND IsDryRunPort=1 AND StopTypeKey=5
												FOR XML PATH('')
											), 1, 1, '') AS LastDryrunPortDate),
					...
					LastDryrunCustomerDate=(SELECT 
											STUFF((
												...
												FROM OrderDetailStops where OrderDetailKey=@ORderDetailKey AND IsDryRunCustomer=1
												FOR XML PATH('')
