...
CREATE Proc [dbo].[TMS_Integration_ReverseMapping_ACER] -- TMS_Integration_ReverseMapping_ACER 1157
(
	@DataKey		INT
)
as
...
Begin
	...
	Declare @CustKey INT = 3165

	Declare @SiteID varchar(20) = 'ACER'
	...
	UPDATE		AC
	SET			shipmentReferenceNumber = OH.BrokerRefNo
	FROM		Integration_JCB.dbo.ACER_Header AC
	INNER JOIN	OrderHeader OH ON AC.TMS_OrderKey = OH.OrderKey
	WHERE		AC.DataKey = @DataKey AND AC.shipmentReferenceNumber NOT LIKE '%'+OH.BrokerRefNo+'%'


	SELECT		*
	INTO		#TMP_ACER_Header
	FROM		Integration_JCB.dbo.ACER_Header
	...
	FROM		#TMP_ACER_Header KH WITH (NOLOCK) 
	INNER JOIN	OrderHeader OH  WITH (NOLOCK) on KH.TMS_OrderKey = OH.orderkey
	INNER JOIN	Integration_JCB.dbo.ACER_ContainerList CL  WITH (NOLOCK) on Kh.DataKey = CL.DataKey
	...
	SELECT		IH.DataKey as TKT_DataKey, 
				...
				@SiteId as SiteID
	INTO		#Temp_ACER
	FROM		#TMP_ACER_Header IH
	INNER JOIN	(Select datakey, ContainerKey, equipmentNumber from  Integration_JCB.dbo.ACER_ContainerList WITH (NOLOCK) ) IC on IH.DataKey = IC.DataKey
	...
	WHERE		IC.equipmentNumber = OD.ContainerNo
				and OH2.OrderKey is not null and IH.isAccepted =1 and OH2.CustKey = @CustKey
	...
	INNER JOIN	#Temp_ACER ND on SD.TKT_DataKey = ND.TKT_DataKey and SD.SiteID = @SiteId
	...
	INNER JOIN	#Temp_ACER ND  WITH (NOLOCK) ON TS.TKT_DataKey = ND.TKT_DataKey
	...
	INNER JOIN		#Temp_ACER ND  WITH (NOLOCK) ON SD.TKT_DataKey = ND.TKT_DataKey
	...
	-- SELECT * FROM #Temp_ACER

	INSERT INTO	TMS_Integration_Container ( SiteID, DataKey, ContainerKey, ContainerNo,TMS_OrderDetailKey)
	SELECT		DISTINCT SD.SiteID, SD.TKT_DataKey, SD.TKT_ContainerKey, SD.TMS_ContainerNo, SD.TMS_OrderDetailKey
	FROM		TKT_SyncData SD
	INNER JOIN	#Temp_ACER ND ON SD.TKT_DataKey = ND.TKT_DataKey
	...
	INNER JOIN	#Temp_ACER ND  WITH (NOLOCK) ON A.TKT_DataKey = ND.TKT_DataKey
	...
	FROM		Integration_JCB.dbo.ACER_ContainerList C
	...
	INNER JOIN	#TMP_ACER_Header H WITH (NOLOCK)  on C.DataKey  = H.DataKey
	...
	INNER JOIN	Integration_JCB.dbo.ACER_ContainerList FC  WITH (NOLOCK) on SL.ContainerKey = FC.ContainerKey
	...
	SELECT		SD.siteid, SD.TKT_DataKey, SD.TKT_ContainerKey, SL.StopKey, RD.RouteKey, RD.LegKey
	FROM		TKT_RouteData RD WITH (NOLOCK) 
	INNER JOIN	Routes R  WITH (NOLOCK) ON RD.RouteKey = R.RouteKey 
	INNER JOIN	TKT_SyncData SD  WITH (NOLOCK) on SD.TMS_OrderDetailKey = RD.OrderDetailKey
	INNER JOIN	#Temp_ACER ND  WITH (NOLOCK) ON SD.TKT_DataKey = ND.TKT_DataKey
	INNER JOIN	Integration_JCB.dbo.ACER_StopList SL on SL.ContainerKey = SD.TKT_ContainerKey and RD.StopType = SL.stopNumber
	...
	SET			TMS_LegKey = RD.legKey, TMS_RouteKey = RD.RouteKey, TMS_SourceAddrKey = RT.SourceAddrKey,
				TMS_DestinationAddrKey = RT.DestinationAddrKey
				--select *
	FROM		Integration_JCB.dbo.ACER_StopList SL WITH (NOLOCK) 
	INNER JOIN	Integration_JCB.dbo.ACER_ContainerList FC  WITH (NOLOCK) on SL.ContainerKey = FC.ContainerKey
	INNER JOIN	TKT_SyncData A  WITH (NOLOCK) on FC.DataKey = TKT_DataKey and FC.ContainerKey = TKT_ContainerKey and A.SiteID = @SiteId
	INNER JOIN	#Temp_ACER ND  WITH (NOLOCK) ON A.TKT_DataKey = ND.TKT_DataKey
	INNER JOIN	TKT_RouteData RD  WITH (NOLOCK) on  A.TMS_OrderDetailKey = RD.orderDetailKey and RD.stoptype = SL.stopNumber
