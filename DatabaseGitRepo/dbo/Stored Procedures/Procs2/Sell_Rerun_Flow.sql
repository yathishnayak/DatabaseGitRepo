...
/*	73164, , 10035
	DECLARE @InvoiceKey int =0, @InvoiceNo Varchar(50) = '99593',   @JsonOutput nvarchar(max) ='',
	...
	EXEC [SELL_Rerun_Flow] @InvoiceKey, @InvoiceNo, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT, @debug = 1

*/
CREATE PROC [dbo].[Sell_Rerun_Flow]
(
	@InvoiceKey				int	= 0,
...
BEGIN
	...
	BEGIN
		...
		End
		EXEC [Sell_OutputByInvoice] @InvoiceKey, @InvoiceNo, @IsSpotOn, @CustomerSegment, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT, @debug = 0
		Select @JsonOutput as JsonResult

		EXEC SEll_BI_ProcessInvoices_SingleInvoice @InvoiceNo, @JsonOutPut, @Status, @Reason
	END
	...
	BEGIN
		...
		SELECT @IsRerunExists = case when count(1) > 0 then 1 else 0 end
								from Sell_BI_Rerun where InvoiceNo = @InvoiceNo
		...
		BEGIN
			...
			Begin
				select @IsRerunExists as IsRerunExists
			End
			EXEC [Sell_BI_Data_Exists] @InvoiceNo, @is_BI_DataExists = @BI_Data_Exists output
			IF (@BI_Data_Exists = 0)
			BEGIN
				if (@Debug = 1)
				Begin
			...
			BEGIN
				...
				EXEC [Sell_BI_Fetch_DataByInvoice] @InvoiceKey, @InvoiceNo, @IsSpotOn, @CustomerSegment, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT, @Debug = 0
				Select @JsonOutput as JsonResult
				--
				--
				--
