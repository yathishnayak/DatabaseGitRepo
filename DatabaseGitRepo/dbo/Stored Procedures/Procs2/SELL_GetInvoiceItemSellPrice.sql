...
/*
Declare @ItemKey int = 18,@InvoiceKey int = 262327,@ContainerNo varchar(50) = 'TCNU7376989',@JsonOutput	nvarchar(max) = '', @IsDebug bit = 1
...
CREATE PRoc [dbo].[SELL_GetInvoiceItemSellPrice]  
(
	@ItemKey				int = 0,
	@InvoiceKey				int = 0,
...
BEGIN
	...
	select @MarketKey = Case when OH.MarketLocationKey = 0 then C.MarketLocationKey else isnull(OH.MarketLocationKey, C.MarketLocationKey) end,
		...
		@IncludeFSF = isnull(C.IncludeFSF,0),@invoiceDate=InvoiceDate
	...
	LEft join Address A on  A.AddrKey = case when OH.OrderTypeKey = 1 and  L.ToLocation  in ('Shipper','Customer', 'Consignee') then Rt.DestinationAddrKey
		When OH.OrderTypeKey = 2 and  L.FromLocation in ('Shipper','Customer', 'Consignee') then Rt.SourceAddrKey else Rt.DestinationAddrKey end 
	Left join Address HA on  HA.AddrKey  = case when OH.ordertypekey = 1 then OH.DestinationAddrKey 
		...
		when OH.OrderTypeKey = 3 then  OH.DestinationAddrKey 
		ELSE OH.DestinationAddrKey end 
	...
	where 	
		...
		OR
		(@OrderType <> 'Export' and L.ToLocation  in ('Shipper','Customer', 'Consignee')) 


	if(@IsDebug = 1)
	Begin
		select '#BaseInfo_Temp', * from #BaseInfo_Temp 
	End
	...
	Begin
		...
		select Rownum, OutputDataKey, RecordSL, MarketKey, Terminal, City, State, Zip, LocationName,
			...
			Rate, BvsNB, FreeTime, MinCnt, MaxCnt
		from (
		Select ROW_NUMBER() over (partition by Lineitem order by outputdataKey Desc, (CASE 
        ...
        WHEN TRY_CONVERT(datetime, EffectiveDate, 103) IS NOT NULL 
            THEN CONVERT(varchar(10), TRY_CONVERT(datetime, EffectiveDate, 103), 101) END) Desc, City Desc, State DESC, Terminal DESC, Marketkey Desc, 
		...
		where B.CustKey = @CustKey and MarketKey = isnull(@MarketKey,0) and 
			...
			(Zip=ISNULL(@ZipCode,'') OR ISNULL(Zip,0)=0) and
			( LocationName = isnull(@Location,'') OR ISNULL(LocationName,'') ='') and
			(CASE 
        WHEN ISDATE(EffectiveDate) = 1 
            THEN CONVERT(varchar(10), CAST(EffectiveDate AS datetime), 101)
        WHEN TRY_CONVERT(datetime, EffectiveDate, 103) IS NOT NULL 
			...
			(CASE 
        WHEN ISDATE(B.ExpiryDate) = 1 
            THEN CONVERT(varchar(10), CAST(B.ExpiryDate AS datetime), 101)
        WHEN TRY_CONVERT(datetime, B.ExpiryDate, 103) IS NOT NULL 
            THEN CONVERT(varchar(10), TRY_CONVERT(datetime, B.ExpiryDate, 103), 101) end) >= convert(Date,@invoiceDate)
		...
		select  RecordSL, LineItem, MarketLocation as MArket, Terminal,itemKey, --TruckType, YardPort, [Zone], [Group], 
			...
			isYardPort , isTerminal , isMArket , isTruckType , isZone ,  isLocation, isCity, isState, LocationName
		...
		set @JsonOutput = (
			select   RecordSL, LineItem, MArket, Terminal,ItemKey, 
				...
				IsActive, @IncludeFSF as IncludeFSF, @City as City, @State as State,LocationName
			from (
	...
	Begin
		...
		insert into #DrayBase(ContainerNo, DrayBase_Value, Margin_Percent,Margin_Value,DrayBase_Rate, FSF_Percent,FSF_Value,Draybase_Total, 
								NetRevenue, EffectiveDate, EffectiveFrom, FileName, DateUploaded, UploadedBy, RecordSL,LocationName)
		exec SELL_GetDraybaseData 
					@ContainerNo=@ContainerNo,@city =@City ,@State=@state,@ZipCode=@Zipcode,@LocationName=@Location,@MarketKey=@MarketKey, 
			...
			--		(ltrim(rtrim(LocationName)) = ltrim(rtrim(@Location)) OR ISNULL(LocationName,'') ='') and
   ...
   --         THEN CONVERT(varchar(10), TRY_CONVERT(datetime, A.ExpiryDate, 103), 101) end)>= Convert(date, @invoiceDate)
			----ORDER BY convert(datetime, effectivedate) desc, OutputDataKey Desc
			--ORDER BY  LocationName DESC, (CASE 
   --     WHEN ISDATE(EffectiveDate) = 1 
   --         THEN CONVERT(varchar(10), CAST(EffectiveDate AS datetime), 101)
   --     WHEN TRY_CONVERT(datetime, EffectiveDate, 103) IS NOT NULL 
   --         THEN CONVERT(varchar(10), TRY_CONVERT(datetime, EffectiveDate, 103), 101) END) DESC,city DESC, State DESC, Zip DESC--, OutputDataKey Desc
		...
		Begin
			...
			where (City = @city OR City is null) and 
					(State = @State OR State is null) and 
					(LocationName = @Location OR ISNULL(LocationName,'') ='') and  
					MarketKey = @MarketKey and A.Custkey = @CustKey and
		...
		set @JsonOutput = (
			select   RecordSL, Mdescription as LineItem, @Market as MArket, @Terminal Terminal,ItemKey,@IncludeFSF  as IncludeFSF ,
				...
				IsActive, @city as City, @State as State, @CustomerSegment as CustomerSegment, LocationName
			from (
