...
CREATE Proc [dbo].[TMS_Integration_ReverseMapping_DHL_BKP10Apr2024] -- [TMS_Integration_ReverseMapping_DHL] 4378
(
	@DataKey	int = 0
)
as
Begin
	...
	Left Join TKT_SyncData SD  WITH (NOLOCK) on KH.DataKey = SD.TKT_DataKey and KH.TMS_OrderKey = SD.TMS_OrderKey and SD.Siteid = 'DHL'
	...
	LEFT join OrderHeader OH2  WITH (NOLOCK) on (OH2.BrokerRefNo = IH.shipmentReferenceNumber) OR ('DH-' + OH2.BrokerRefNo = IH.shipmentReferenceNumber)
	...
	and OH2.OrderKey is not null and IH.isAccepted =1 and OH2.CustKey = 3170
	...
	inner join OrderHeader OH  WITH (NOLOCK) on SD.TMS_OrderKey = OH.OrderKey and OH.custkey = 3170
	...
	inner join OrderHeader OH  WITH (NOLOCK) on OD.OrderKey = OH.OrderKey and OH.custkey = 3170
	...
	where OH.custkey = 3170 and isnull(H.TMS_OrderKey ,0) = 0 and isnull(ProcessStatus ,'') = ''
		and A.SiteID = @SiteId
	...
	--select *
	from Integration_JCB.dbo.DHL_ContainerList C WITH (NOLOCK)
	...
	--select *
	from Integration_JCB.dbo.DHL_StopList SL WITH (NOLOCK)
	...
	LEft join #Temp_RouteData A  WITH (NOLOCK) on A.OrderDetailKey = RD.OrderDetailKey and A.RouteKey = RD.RouteKey
	where A.OrderDetailKey is null  and OH.CustKey = 3170)
	...
	from (
		...
		from (
			...
			and ((L.FromLocation in ('Shipper','Consignee','Customer') and isnull(RT.IsEmpty,0) = 0)
				OR ( L.ToLocation = 'PORT' and isnull(RT.IsEmpty,0) = 0)
	...
	where routekey in (
		...
		inner join orderDetail OD on RD.OrderDetailKey = OD.OrderDetailKey
		inner join OrderHeader OH on OD.OrderKey = OH.OrderKey and OH.OrderTypeKey = 2
	...
	inner join Integration_JCB.dbo.DHL_StopList SL  WITH (NOLOCK) on SL.ContainerKey = SD.TKT_ContainerKey and RD.StopType = SL.stopNumber
	...
	where TR.DataKey is null and SD.SiteID = @SiteId
