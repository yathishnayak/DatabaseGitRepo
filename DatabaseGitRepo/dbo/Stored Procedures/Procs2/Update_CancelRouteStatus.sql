...
CREATE PROCEDURE [dbo].[Update_CancelRouteStatus] -- [Update_CancelRouteStatus] null, null, null
(
	@RouteKey	 INT = 115,
...
BEGIN
	...
	BEGIN TRY
		...
		SET @CompleteStatusKey= ( SELECT [Status] FROM dbo.RouteStatus WHERE [Description]='Leg Completed' );
		...
		SET @StatusKey=( SELECT [Status] FROM [Routes] WHERE RouteKey= ( SELECT CurrOPenRoutekey FROM #CurrLeg) )
