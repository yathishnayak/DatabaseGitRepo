...
EXEC TMS_Integration_VerificationReport  '000006153','','','', @Res OUTPUT
SELECT @Res
*/

create PROCEDURE [dbo].[TMS_Integration_VerificationReport_workingTill_20240227]
(
		 @OrderNo			VARCHAR(20)	 = ''
...
Select @OrderKey = OH.OrderKey	, @CustKey = CustKey
From OrderHeader OH
...
if(@siteid = 'Flexport')
Begin
	insert into #Integration (SiteID, DataKey, TMS_ORderKey,  ContainerNo, workOrderNumber)
	select @SiteID, FH.datakey, FH.TMS_OrderKey, FC.equipmentNumber, FH.workOrderNumber
...
LEft join			#Integration I on TH.DataKey = I.DataKey and TH.SiteID = I.SiteID
...
if(@Siteid = 'Flexport')
Begin
	insert into #214 (SiteID, DataKey, DocKey,StopKey,StopType, ScheduledActual, DocContent, DocControlNo, DocCreated, DocUploaded )
