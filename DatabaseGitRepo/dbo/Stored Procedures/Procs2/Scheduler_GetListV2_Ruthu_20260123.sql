...
DECLARE 
	...
	@Reason VARCHAR(100)=''
EXec Scheduler_GetListV2_Ruthu_20260123 @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @IsDebug
Select @Status, @Reason
**/
CREATE PROCEDURE [dbo].[Scheduler_GetListV2_Ruthu_20260123]   
	@UserKey		int,
...
BEGIN
	...
	DECLARE @TempAllStartTime DATETIME,
        @TempAllEndTime DATETIME;

	DECLARE @Search bit = CASE WHEN ISNULL(@SearchText,'') = '' THEN 0 ELSE 1 END;
	
	select Od.orderdetailkey, OH.orderKey, OD.Status, OH.orderNo, CONVERT(biT, 0) AS IsDataSelected
	--select 
...
(
    ...
    (
        1 = 1
        AND (
                ISNULL(@PickupAvailable, 0) = 0 
                OR @PickupAvailable = ISNULL(CGD.Available, GICF.Available_for_pickup)
            )

        AND (
                ISNULL(@TerminalNames, '') = ''
                OR Pod_terminal_name IN (
                    SELECT TerminalName FROM #TerminalNames
                )
            )

        AND (ISNULL(@IsCTF, 0) = 0 OR CTF = 'true')
        AND (ISNULL(@IsTMF, 0) = 0 OR TMF = 'true')
        AND (ISNULL(@IsLine, 0) = 0 OR Line = 'true')
        AND (ISNULL(@IsOther, 0) = 0 OR Other = 'true')
        AND (ISNULL(@IsCustoms, 0) = 0 OR Customs = 'true')
        AND (ISNULL(@IsFreight, 0) = 0 OR Freight = 'true')
        AND (ISNULL(@IsClosedArea, 0) = 0 OR ClosedArea = 'true')

        AND (
                ISNULL(@CustKeys, '') = ''
                OR Customer IN (
                    SELECT CustName FROM #CustKeys
                )
            )
        ...
        AND (
                ISNULL(@CSRkeys, '') = ''
                OR OrderCSR IN (
                    SELECT CSRName FROM #CSRKeys
                )
            )

        AND (
                ISNULL(@MarketKeys, '') = ''
                OR ISNULL(ML.MarketLocationKey,0) IN (
                    SELECT MarketKey FROM #MarketKeys
                )
            )

        AND (
                @PickUpFrom IS NULL
                OR Pickup_appointment_dt IS NULL
        ...
        OR (
            @Tracking = 'Yes' 
            AND ISNULL(GICF.OrderDetailKey,0) <> 0
        )
        OR (
            @Tracking = 'No' 
            AND ISNULL(GICF.OrderDetailKey,0) = 0
        ...
        AND (
                ...
                OR LTRIM(RTRIM(CDT.AddrName)) IN (
                    SELECT DeliverocationKey 
                    FROM #DelieverLocationKeys
                )
            )

        AND (
                ISNULL(@OrderType, '') = ''
                OR COALESCE(OD.OrderTypeKey, OH.OrderTypeKey, 0) IN (
                    SELECT OrderTypeKey 
                    FROM #OrderTypeKeys
                )
            )
		...
		if(isnull(@SearchText,'') <>'')
		Begin
			update #FilteredRecords set IsDataSelected = 1 
		END
		ELSE
		Begin
			update #FilteredRecords set IsDataSelected = 1 where (  Status = @StatusKey)
		End

		if(@IsDebug = 1)
		begin	
			select * from #FilteredRecords
			select Status, count(1) from #FilteredRecords group by Status order by status
		...
		SELECT
			...
			CASE 
				WHEN EXISTS (
					...
					WHERE OE.OrderDetailKey = OD1.OrderDetailKey
					  AND OE.isCSRApproved = 0
				)
				AND NOT EXISTS (
					SELECT 1
					FROM InvoiceDetail ID
					WHERE ID.OrderDetailKey = OD1.OrderDetailKey
				)
				THEN CAST(0 AS BIT)
				ELSE CAST(1 AS BIT)
			END AS IsChargesApproved

		into #TempAll
		FROM  #FilteredRecords OD1					WITH (NOLOCK)	
			INNER JOIN dbo.OrderDetail OD			WITH (NOLOCK)	ON OD.OrderDetailkey = OD1.OrderDetailkey
			INNER JOIN dbo.OrderHeader OH			WITH (NOLOCK)	ON OH.OrderKey=OD.OrderKey
			INNER JOIN dbo.OrderStatus OS			WITH (NOLOCK)	ON OS.[Status]=OH.[Status]
			LEFT JOIN dbo.[Broker]  BR				WITH (NOLOCK)	ON BR.BrokerKey=OH.BrokerKey
			...
			WHERE OD1.IsDataSelected = 1

			DECLARE @RowCount INT;
		SELECT @RowCount = COUNT(*) FROM #TempAll;
		PRINT 'Rows in #TempAll: ' + CAST(@RowCount AS VARCHAR);


				SET @TempAllEndTime = GETDATE();

			-- Optionally, print or log the duration
			PRINT 'TempAll processing started at: ' + CONVERT(VARCHAR, @TempAllStartTime, 121);
			PRINT 'TempAll processing ended at: ' + CONVERT(VARCHAR, @TempAllEndTime, 121);
			PRINT 'TempAll processing duration (seconds): ' + 
				  CONVERT(VARCHAR, DATEDIFF(SECOND, @TempAllStartTime, @TempAllEndTime));

		if(@IsDebug = 1)
		Begin
			SELECT '#FilteredRecords' from #FilteredRecords
		...
		update #FilteredRecords set Status = 12 where status in (12,14)
		...
		Begin
			...
			select '#TempDashboard',count(1) from #TempAll 
		End
		...
		--if(@IsDebug = 1)
		--Begin
		--	select '#Temp', count(1) from #TempAll 
		--End
		...
		SET @RecTo = @PageNo *  @PageSize
			...
			SELECT ID, RowNum 
			FROM (
				SELECT ID, ROW_NUMBER() OVER (ORDER BY ' + @SortField + N' ' + 
				CASE @IsAscending WHEN 0 THEN 'DESC' ELSE 'ASC' END + N') AS RowNum
				FROM #TempAll
			) Y
			WHERE RowNum BETWEEN ' + CONVERT(NVARCHAR(10), @RecFrom) + N' AND ' + CONVERT(NVARCHAR(10), @RecTo) + N'
			ORDER BY RowNum;';
