...
/* --19907, 25335 -- Acc Rate : SMB- 47507, ENT- 47502 -- 95737
	...
	EXEC [Sell_BI_Fetch_DataByInvoice] @InvoiceKey, @InvoiceNo, @IsSpotOn, @CustomerSegment, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT, @Debug
	SELECT @JsonOutput, @Status, @Reason
*/
CREATE PROC [dbo].[Sell_BI_Fetch_DataByInvoice]
(
	@InvoiceKey			int	= 0,
...
if(@Debug = 1)
BEGIN
select '#InvoiceSummary'AS TableName, * from #InvoiceSummary
END
...
if(@Debug = 1)
BEGIN
select '#Accessorials'AS TableName, * from #Accessorials
END
...
if(@Debug = 1)
BEGIN
select '#DrayBase'AS TableName, * from #DrayBase
END
...
if(@Debug = 1)
BEGIN
select '#Bobtail'AS TableName, * from #Bobtail
END
