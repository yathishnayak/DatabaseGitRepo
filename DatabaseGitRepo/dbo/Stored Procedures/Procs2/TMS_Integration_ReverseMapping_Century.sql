...
CREATE Proc [dbo].[TMS_Integration_ReverseMapping_Century] -- TMS_Integration_ReverseMapping_Century 3810
(
	@DataKey		INT
)
as
...
Begin
	...
	EXEC		TMS_INTEGRATION_InsertRTStops_Century
	EXEC		TMS_INTEGRATION_InsertRPStops_Century
	...
	WHERE		IC.equipmentNumber = OD.ContainerNo
				and IH.isAccepted =1 
				and OH.CustKey  IN  ( SELECT CustKey FROM TMS_Integration_Customers WITH (NOLOCK) WHERE SiteID = 'Century' ) 
	...
	WHERE		TR.TMS_RouteKey is null and SD.SiteID = @SiteID 
				AND CASE WHEN RD.LocationType IN ('CE') THEN 0 ELSE  ISNULL(R.IsDryRun,0) END = 0
				AND RD.LocationType IN ('SF','RT','RP','PP','CP','ST')
