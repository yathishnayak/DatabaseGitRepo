/*  
DECLARE @UserKey INT = 486, @JSONString NVARCHAR(MAX), @JSONOutput NVARCHAR(MAX), @Status BIT = 0, @Reason VARCHAR(1000) = '',@IsDebug bit = 0  
SET @JSONString = '{"OrderTypeKey":1,"LocationType":"SF","CustomerKey":3615,"MarketKey":2}'  
EXEC [Location_GetByOrderType_V2] @UserKey, @JSONString, @JSONOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT, @ISDebug  
SELECT @Status AS Status, @Reason AS Reason, @JSONOutput AS JSONOutput 
  
*/  
CREATE PROCEDURE [dbo].[Location_GetByOrderType_V2]  
(  
 @UserKey      INT,  
...
BEGIN  
 ...
 Select * into #Yard from #Temp   
  ...
  WHERE S.StatusName='Active' AND( C.CustKey = @CustomerKey  OR 0=@CustomerKey ) And  C.MarketLocationKey = @MarketKey  
  ORDER BY [Name]  
...
BEGIN  
    ...
    ELSE IF (@LocationType = 'ST')   -- Delivery / Empty Pickup
    BEGIN
        INSERT INTO #Temp
        SELECT * FROM #Customer
        UNION ALL
    ...
    ELSE IF (@LocationType = 'RT')   -- Return
    BEGIN
        INSERT INTO #Temp
        SELECT * FROM #Port
    END
