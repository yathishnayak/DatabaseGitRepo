...
CREATE PROC [dbo].[SELL_NAC_Bobtail_MoveFinalDataOutput]
(
	@FileProcessKey		int,
	@Status				bit = 0 output,
...
BEGIN
	...
	Begin
		...
		return
	END
	if((select count(1) from SELL_NAC_Bobtail_FileUploadData where FileProcessKey = @FileProcessKey) = 0)
	Begin
		SEt @Status = 0
		SEt @Reason = 'Record not found in Upload Data'
		return
	end
	if((select count(1) from SELL_NAC_Bobtail_FileUploadData where FileProcessKey = @FileProcessKey and isnull(Remarks,'') <> '') > 0)
	Begin
		SEt @Status = 0
	...
	insert into SELL_NAC_Bobtail_FinalDataOutput(FileProcessKey, RecordSL, CustID, CustName, RateType, Segment, MarketLocation, Terminal, 
		OrderType, City, State, Zip, LocationName, IsLocationExists, Consignee, TruckType, BobtailRate, BobtailFormat, EffectiveDate, 
		EffectiveDateFrom,ExpiryDate)
	select FileProcessKey, RecordSL, CustID, CustName, RateType, Segment, MarketLocation, Terminal, 
		OrderType, City, State, Zip, LocationName, IsLocationExists, Consignee, TruckType, BobtailRate, BobtailFormat, EffectiveDate, 
		EffectiveDateFrom,ExpiryDate
	from SELL_NAC_Bobtail_FileUploadData 
