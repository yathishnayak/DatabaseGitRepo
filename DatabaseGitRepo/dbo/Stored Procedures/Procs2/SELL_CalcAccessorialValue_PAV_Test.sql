...
Create PROC [dbo].[SELL_CalcAccessorialValue_PAV_Test]  -- 1765 // 3281
...
BEGIN
	...
	Select ROW_NUMBER() over (partition by Lineitem order by   City Desc, State DESC, Terminal DESC, 
		Marketkey Desc, CustName Desc,LocationName Desc, (CASE 
        ...
        WHEN TRY_CONVERT(datetime, EffectiveDate, 103) IS NOT NULL 
            THEN CONVERT(varchar(10), TRY_CONVERT(datetime, EffectiveDate, 103), 101) END) Desc, outputdataKey Desc) Rownum,
	...
	where (B.CustName = @CustName OR B.CustName is null ) and MarketKey = isnull(@MarketKey,0) and 
		...
		( TRIM(LocationName )= isnull(TRIM(@Location),'') OR LocationName is null)
		and (CASE 
        WHEN ISDATE(EffectiveDate) = 1 
            THEN CONVERT(varchar(10), CAST(EffectiveDate AS datetime), 101)
        WHEN TRY_CONVERT(datetime, EffectiveDate, 103) IS NOT NULL 
            THEN CONVERT(varchar(10), TRY_CONVERT(datetime, EffectiveDate, 103), 101)end) <= convert(Date, getdate())
        ...
        WHEN TRY_CONVERT(datetime, B.ExpiryDate, 103) IS NOT NULL 
            THEN CONVERT(varchar(10), TRY_CONVERT(datetime, B.ExpiryDate, 103), 101) end) >= convert(Date, Getdate())
