/* --19907, 25335 -- Acc Rate : SMB- 47507, ENT- 47502
	DECLARE @InvoiceKey int =0, @InvoiceNo Varchar(50) = '160939',   @JsonOutput nvarchar(max) ='',@Status	bit = 0 , @Reason	varchar(500) = '' ,
		@IsSpotOn bit = 0, @CustomerSegment varchar(5) = 'NAC', @Debug bit = 1
	EXEC [Sell_OutputByInvoice_Ruthu_20260218] @InvoiceKey, @InvoiceNo, @IsSpotOn, @CustomerSegment, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT, @Debug
	SELECT @JsonOutput, @Status, @Reason

	107675

	DECLARE @InvoiceKey int =0, @InvoiceNo Varchar(50) = '151530',   @JsonOutput nvarchar(max) ='',@Status	bit = 0 , @Reason	varchar(500) = '' ,
		@IsSpotOn bit = 0, @CustomerSegment varchar(5) = 'NAC', @Debug bit = 1
	EXEC [Sell_OutputByInvoice_Ruthu_20260218] @InvoiceKey, @InvoiceNo, @IsSpotOn, @CustomerSegment, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT, @Debug
	SELECT @JsonOutput, @Status, @Reason
	...
	SELECT @JsonOutput, @Status, @Reason
*/
CREATE PROCEDURE [dbo].[Sell_OutputByInvoice_Ruthu_20260218]
(
	@InvoiceKey			int	= 0,
...
BEGIN
	...
	Begin
		select @InvoiceKey = InvoiceKey From InvoiceHeader WITH(NOLOCK) where InvoiceNo = @InvoiceNo
	End
	--print '@InvoiceKey'
	--print @InvoiceKey
	IF @Debug = 1
	...
	into #AccRercs
	from SELL_NAC_Accessorial_FinalDataOutput WITH(NOLOCK) where 1=0
	...
	SELECT TOP 1
    @MarketKey  = ISNULL(OH.MarketLocationKey, C.MarketLocationKey),
	...
	INNER JOIN OrderHeader OH WITH (NOLOCK)
		ON IH.OrderKey = OH.OrderKey
	INNER JOIN Customer C WITH (NOLOCK)
		ON IH.CustKey = C.CustKey
	INNER JOIN OrderType OT WITH (NOLOCK)
		ON OH.OrderTypeKey = OT.OrderTypeKey
	WHERE IH.InvoiceKey = @InvoiceKey

	SELECT @Market = ML.MarketLocation
	...
	IF ISNULL(@MarketKey,0) = 0
		BEGIN
			...
			WHERE MarketLocationKey = 2
		END

	-- Sumanth added this to get adresskey
	Declare @OrderDetailKey INT, @AddrKey INT

	-- Set @OrderDetailKey = (Select TOP 1 OrderDetailKey from InvoiceDetail WITH(NOLOCK) WHERE InvoiceKey = @InvoiceKey)
	SELECT TOP (1) 
    @OrderDetailKey = OrderDetailKey
	FROM InvoiceDetail WITH (NOLOCK)
	WHERE InvoiceKey = @InvoiceKey
	ORDER BY OrderDetailKey

	Print '@OrderDetailKey'
	Print @OrderDetailKey
	Print '-------------'
	--Set @AddrKey = (SELECT ODS.StopAddrKey FROM orderdetail od WITH(NOLOCK)
	--	INNER JOIN OrderDetailStops ODS WITH(NOLOCK) ON OD.OrderDetailKey=ODS.OrderDetailKey AND ODS.StopTypeKey=3 AND 
	--	ISNULL(ODS.IsDryrunCustomer,0)=0AND ISNULL(ODS.IsDryrunPort,0)=0
	--	INNER JOIN [Address] a WITH(NOLOCK) ON A.AddrKey=ODS.StopAddrKey
	--	WHERE OD.orderdetailkey=@OrderDetailKey)

	SELECT TOP (1)
    @AddrKey = ODS.StopAddrKey
	FROM OrderDetail OD WITH (NOLOCK)
	INNER JOIN OrderDetailStops ODS WITH (NOLOCK)
		ON OD.OrderDetailKey = ODS.OrderDetailKey
		AND ODS.StopTypeKey = 3
		AND (ODS.IsDryrunCustomer = 0 OR ODS.IsDryrunCustomer IS NULL)
		AND (ODS.IsDryrunPort = 0 OR ODS.IsDryrunPort IS NULL)
	WHERE OD.OrderDetailKey = @OrderDetailKey

	--/// TERMINAL
	Select @TerminalKey = case when @MarketKey = 2 then 6 else -- 4 end
	--- Sumanth edited this after updating records to the ShippingPortTerminals table
	(Select PriceGroupingKey from ShippingPortTerminals SPT WITH(NOLOCK)
	Where SPT.MarketLocationKey = @MarketKey and SPT.AddrKey = @AddrKey) end
	---
	...
	SELECT * INTO #BaseInfo_Auto_ReturnBaseInfo from BaseInfo_WRK where 1= 0
	
	--Insertion to temp table from procedure
	Insert Into #BaseInfo_Auto_ReturnBaseInfo 
	EXEC Auto_ReturnBaseInfo @InvoiceKey 

	if(@Debug = 1)
	Begin
		Select '#BaseInfo_Auto_ReturnBaseInfo',* from #BaseInfo_Auto_ReturnBaseInfo
	End

	--if((Select count(1) from #BaseInfo_Auto_ReturnBaseInfo where LegTypeName = 'Dray Base') > 0)
	--Begin
	--	set @IsDraybase = 1
	--End
	...
	IF EXISTS (
    SELECT 1 
    FROM #BaseInfo_Auto_ReturnBaseInfo 
    WHERE LegTypeName = 'Dray Base'
	)
	BEGIN
		SET @IsDraybase = 1
	END
	...
	-- select @IsBobtail = case when  count(1) > 0 then 1 else 0 end from #BaseInfo_Auto_ReturnBaseInfo where IsBobtail = 1
	IF EXISTS (
    SELECT 1 
    FROM #BaseInfo_Auto_ReturnBaseInfo 
    WHERE IsBobtail = 1
	...
	select @IsDryRun = case when  count(1) > 0 then 1 else 0 end from #BaseInfo_Auto_ReturnBaseInfo where IsDryRun  = 1

	if(@IsDraybase = 0)
	Begin
		
		select @IsDraybase = case when count(1) >0 then 1 else 0 end from #ItemInfo where DriverNonDriverCostDesc = 'Drayage'
	End

	select distinct OrderdetailKey, ContainerNo into #ContainerBase from  #BaseInfo_Auto_ReturnBaseInfo
	if(@Debug = 1)
	Begin
	...
	BEGIN
		...
		Begin
			...
			Begin
				select @City = A.City, 
					@State = A.State,
					@ZipCode = A.ZipCode,
					@LocationName = LTRIM(RTRIM(A.AddrName))
				from InvoiceHeader IH WITH (NOLOCK)
				inner join OrderHeader OH WITH (NOLOCK) on IH.OrderKey = OH.OrderKey
		...
		select top 1 @ZoneKey = ZoneKey from ZoneCityMap WITH (NOLOCK) where MarketKey = @MarketKey and TerminalKey = @TerminalKey and City = @City and State = @State
		if(@Debug = 1)
		Begin
		...
		--from #BaseInfo_Auto_ReturnBaseInfo where ORDERDETAILKEY = @_OrderdetailKey AND ContainerNo = @_ContainerNo AND TruckType is not null and 
		--Case when @OrderType = 'Import' then ToLocation else FromLocation end in ('Consignee','Customer','Shipper')
		select @DriverTypeKey = TruckTypeKey From TruckType WITH (NOLOCK) where TruckType = @DriverType

		print '@TruckType'
		print @DriverType
		if(isnull(@DriverType,'') = '')
		Begin
			select  @DriverType = TruckType, @DriverTypeKey = TruckTypeKey from TruckType WITH (NOLOCK) where TruckType = 'Broker Carrier'
			print @DriverType
		end

		Select top 1	@YardPortType = Yardtype
		From #BaseInfo_Auto_ReturnBaseInfo Where ORDERDETAILKEY = @_OrderdetailKey AND ContainerNo = @_ContainerNo 
		...
		--Begin
			...
			where (City = @city OR City is null) and 
				...
				ORDER BY LocationName desc, convert(datetime, EffectiveDate) DESC, city DESC, State DESC, Zip Desc--, LocationName DESC
			...
			select @RecCountDryrun = count(1) from COST_CostDataOutput WITH (NOLOCK) where City = @CityDryRun and State = @StateDryRun and 
		...
		Begin
			...
			from SELL_NAC_Bobtail_FinalDataOutput A WITH (NOLOCK)
			inner join SELL_NAC_Bobtail_FileProcessInfo F WITH (NOLOCK) on A.FileProcessKey = F.FileProcessKey
			inner join [user] U WITH (NOLOCK) on F.UserKey = U.UserKey
			where (City = @city OR City is null) and 
					(State = @State OR State is null) and 
					(LTRIM(RTRIM(LocationName)) = @LocationName OR LocationName is null) and  
					MarketKey = @MarketKey and A.Custkey = @CustKey and
					(TerminalKey = @TerminalKey OR TerminalKey is null)  
		...
		if(@Debug = 1)
		Begin
			Select @Isbobtail as Isbobtail, @IsDraybase as IsDraybase
			select '#AccRercs', * from #AccRercs
		end
		/*Century And Steam setting InternalCost = 0 #3255 Begins*/
		DECLARE @IsCenturyULS		bit = 0, @IsSteam		bit = 0
		...
		Begin
			-- Update InternalCost in #ItemInfo
			UPDATE A
			SET A.InternalCost = 0
			FROM #ItemInfo A
			INNER JOIN #BaseInfo_Auto_ReturnBaseInfo B ON A.InvoiceKey = B.InvoiceKey
			INNER JOIN #AccRercs X ON A.ItemKey = X.ItemKey
			INNER JOIN Item I ON A.ItemKey = I.ItemKey
			INNER JOIN Item M ON I.MasterItemKey = M.ItemKey
			WHERE (A.ItemKey IN (68, 121, 294) AND B.YardId IN (14, 15, 17, 19)) OR (A.itemkey in (100) and B.ChassisCategoryKey IN (2, 3));
			
			-- Update rate in #AccRercs
			UPDATE X
			SET X.rate = 0, X.BvsNB = 'NB'
			FROM #AccRercs X
			INNER JOIN #ItemInfo A ON A.ItemKey = X.ItemKey
			INNER JOIN #BaseInfo_Auto_ReturnBaseInfo B ON A.InvoiceKey = B.InvoiceKey
			INNER JOIN Item I ON A.ItemKey = I.ItemKey
			INNER JOIN Item M ON I.MasterItemKey = M.ItemKey
			WHERE (A.ItemKey IN (68, 121, 294) AND B.YardId IN (14, 15, 17, 19)) OR (A.itemkey in (100) AND B.ChassisCategoryKey IN (2, 3));
			...
			--INNER JOIN Item I ON A.ItemKey = I.ItemKey
			--INNER JOIN Item M ON I.MasterItemKey = M.ItemKey
			--WHERE A.ItemKey IN (68, 121, 294)

			print 'Test'
		End
		/*Century And Steam setting InternalCost = 0 #3255 Ends*/

		/* PROCESS TO IMPLEMENT BOBTAIL FEE FROM ACCESSORIAL DATA   */
		IF (@IsBobtail = 1 and isnull(( select count(1) from #Bobtail ),0)  = 0)
		Begin
			declare @IsBobtailItemExists	int = 0
			select @IsBobtailItemExists = count(1) from #AccRercs where CostGroup = 'BobTail'
			if(@IsBobtailItemExists > 1)
			Begin
				insert into #Bobtail (ContainerNo, BobtailFormat, BobtailRate, BobtailCalc,
				EffectiveDate, EffectiveDateFrom, FileName, DateUploaded, UploadedBy ,OutputDataKey )
				select @_ContainerNo, 'Flat Fee' , Rate, Rate, EffectiveDate, EffectiveDateFrom, FileName,
				DateUploaded, UploadedBy, 0
				from #AccRercs where CostGroup = 'BobTail'
			End
		End
		/* END OF PROCESS TO IMPLEMENT BOBTAIL FEE FROM ACCESSORIAL DATA   */

		if(@Debug = 1)
		Begin
			select '#AccRercs - After Bobtail <- Accessorial', * from #AccRercs
		end
		...
		if( @isBobtail = 1 and (select count(1) from #Bobtail ) = 0)
		Begin
	...
	if(@Debug = 1)
	Begin
		select '#AccRercs-Final', * from #AccRercs
		select '#ItemInfo-Final', * from #ItemInfo
	...
	--Drop table #AccesorialItemKeys
	--DROP TABLE #Summary
	drop table #BaseInfo_Auto_ReturnBaseInfo
	drop table #ItemInfo
	--drop table #Terminal
