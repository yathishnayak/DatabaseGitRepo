...
CREATe proc [dbo].[TMS_Integration_ReverseMapping_Robinson_Prev_16Jul2024] -- TMS_Integration_ReverseMapping_Robinson 4
(
	@DataKey	int
)
as
Begin
	...
	DECLARE @ContainerList INT = (SELECT ContainerKey FROM Integration_JCB.dbo.Robinson_ContainerList WHERE DataKey = @DataKey )
	...
	inner join #RobinsonCustKeys CK on OH.CustKey = CK.CustKey
	...
	delete from TMS_Integration_Routes where Siteid = 'Robinson' and  TMS_RouteKey in (
	...
	from TKT_RouteData RD WITH (NOLOCK) 
	inner join TKT_SyncData SD  WITH (NOLOCK) on SD.TMS_OrderDetailKey = RD.OrderDetailKey
	inner join Integration_JCB.dbo.Robinson_StopList SL  WITH (NOLOCK) on SL.ContainerKey = SD.TKT_ContainerKey and RD.StopType = SL.stopNumber
	...
	SELECT 'TMS_Integration_Routes', * FROM TMS_Integration_Routes WHERE SiteID = 'Robinson'
	Print '20'
	update  SL set TMS_LegKey = RD.legKey, TMS_RouteKey = RD.RouteKey, TMS_SourceAddrKey = RT.SourceAddrKey,
		TMS_DestinationAddrKey = RT.DestinationAddrKey
	--select *
	...
	SELECT 'TMS_Integration_Routes 11', * FROM TMS_Integration_Routes WHERE SiteID = 'Robinson'
	EXEC TMS_Integration_Robinson_PortToConsignee @DataKey
	EXEC TMS_Integration_Robinson_ConsigneetoPort @DataKey
	SELECT 'TMS_Integration_Routes 22', * FROM TMS_Integration_Routes WHERE SiteID = 'Robinson'
