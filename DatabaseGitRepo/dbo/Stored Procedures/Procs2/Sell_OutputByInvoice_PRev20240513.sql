...
/* --19907, 25335 
	DECLARE @InvoiceKey int =0, @InvoiceNo Varchar(50) = '47937',   @JsonOutput nvarchar(max) ='',@Status	bit = 0 , @Reason	varchar(500) = '' ,
		@IsSpotOn bit = 0, @CustomerSegment varchar(5) = 'NAC'
	EXEC [Sell_OutputByInvoice] @InvoiceKey, @InvoiceNo, @IsSpotOn, @CustomerSegment, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT
	SELECT @JsonOutput, @Status, @Reason
*/

CREATE PRoc [dbo].[Sell_OutputByInvoice_PRev20240513]
(
	@InvoiceKey			int	= 0,
...
BEGIN
	...
	RecordSL, LineItem, MarketLocation, Terminal, ItemKey, Rate, BvsNB, FreeTime, MinCnt, MaxCnt, EffectiveDate, EffectiveDateFrom
	into #AccRercs
	from SELL_NAC_Accessorial_FinalDataOutput where 1=0
	alter table #AccRercs add TotalCost Decimal(18,3) , FileName varchar(200), DateUploaded Datetime, UploadedBy varchar(100)
	...
	--// MARKET
	select @MarketKey = isnull(replace(OH.MarketLocationKey,0,null), C.MarketLocationKey), @Market = ml.MarketLocation,
	...
	LEft  join MarketLocation ML WITH (NOLOCK) ON isnull(replace(OH.MarketLocationKey,0,null), C.MarketLocationKey) = ml.MarketLocationKey
	...
	BEGIN
		...
		IF OBJECT_ID('tempdb.dbo.#AccesorialItemKeys', 'U') IS NOT NULL
		Begin
			DROP TABLE #AccesorialItemKeys; 
		End
		
		Create table #AccesorialItemKeys
		(
			value		varchar(500)
		)

		insert  into #AccesorialItemKeys (value)
		select distinct Description as value from #ItemInfo 
		...
		if(isnull(@IsSpotOn,0) = 0)
		Begin
			SEt @CustomerSegment = 'NAC'
		End
		...
		IF OBJECT_ID('tempdb.dbo.#Bobtail', 'U') IS NOT NULL
		Begin
			DROP TABLE #Bobtail; 
		End
		...
		if(@Debug = 1)
		Begin
			Select @MarketKey as marketKey, @InvoiceKey  as InvoiceKey, @_ContainerNo as ContainerNo,
		...
		exec SELL_CalcAccessorialValue @MarketKey = @marketKey, @InvoiceKey  = @InvoiceKey, @ContainerNo = @_ContainerNo,
		...
		Begin
			set @AccessorialReason = 'Record not found in NAC Accessorial for the Container: ' + @_ContainerNo 
				+ ', Cust Name: ' + @CustName
				+ ', Market: ' + @Market 
				+ ', City : ' + @city
				+ ', State : ' + @State 
	...
	Select @JsonOutput = (
		Select  @Market as Market, @MarketKey as MarketKey,
			...
			Accessorials = (Select 
				...
				FreeTime, MinCnt, MaxCnt, EffectiveDate, EffectiveDateFrom, CostGroup,  FileName, DateUploaded, UploadedBy
				from #AccRercs
				For JSON PATH),
			DrayBase = (Select 
