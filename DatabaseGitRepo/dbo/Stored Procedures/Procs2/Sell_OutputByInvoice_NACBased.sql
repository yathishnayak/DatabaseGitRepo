...
/* --19907, 25335 -- Acc Rate : SMB- 47507, ENT- 47502
	DECLARE @InvoiceKey int =0, @InvoiceNo Varchar(50) = '59837',   @JsonOutput nvarchar(max) ='',@Status	bit = 0 , @Reason	varchar(500) = '' ,
		@IsSpotOn bit = 0, @CustomerSegment varchar(5) = 'NAC'
	EXEC [Sell_OutputByInvoice_NACBased] @InvoiceKey, @InvoiceNo, @IsSpotOn, @CustomerSegment, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT
	SELECT @JsonOutput, @Status, @Reason
*/
CREATE PRoc [dbo].[Sell_OutputByInvoice_NACBased]
(
	@InvoiceKey			int	= 0,
...
BEGIN
	...
	--Added for getting invoicedate
	SELECT @InvoiceDate=ISNULL(InvoiceDate,GETDATE()) FROM InvoiceHeader IH WITH (NOLOCK) WHERE InvoiceKey=@InvoiceKey
