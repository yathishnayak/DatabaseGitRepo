...
create PROCEDURE [dbo].[Scheduler_GetList_shiva2]    -- [Scheduler_GetLis_shiva2t] @statusKey = 0
...
BEGIN
	...
	Declare @RecCount	int, @RowNum int
		...
		SELECT
			...
			ML.MarketLocation, OH.Consignee, SL.LineName AS SteamShipLine
		into #Temp
		FROM  dbo.OrderDetail OD					WITH (NOLOCK)		
			...
			LEft join vContainerType CT				WITH (NOLOCK) on CT.OrderDetailKey = OD.OrderDetailKey and Ct.ContainerTypeKey = 6 -- 'Transload'
			...
			LEFT JOIN vContainerType HZ				WITH (NOLOCK) ON HZ.OrderDetailKey = OD.OrderDetailKey AND CT.ContainerTypeKey = 1 -- 'Hazard'
		...
		WHERE  1=1 AND 
			...
			AND (  isnull(@SearchText ,'') = '' OR
			 ...
			 AND isnull(@isShowAll,0) = 1 OR (
				@LoggedUserKey = CR.LinkedUserKey OR @LoggedUserKey = CM.LinkedUserKey OR @LoggedUserKey = SP.LinkedUserKey
				)
