...
CREATE Proc [dbo].[TMS_Integration_ReverseMapping_CNB] -- [TMS_Integration_ReverseMapping_CNB] 4250
(
	@DataKey	int
)
as
Begin
	Declare @SiteId varchar(20) = 'CNB',
			@CustKey	int = 2867
	Exec dbo.TMS_INTEGRATION_InsertRTStops_CNB
	EXEC TMS_INTEGRATION_InsertAFATStops_CNB
	...
	from Integration_JCB.dbo.CNB_Header KH  WITH (NOLOCK)
	inner join OrderHeader OH  WITH (NOLOCK) on KH.TMS_OrderKey = OH.orderkey
	inner join Integration_JCB.dbo.CNB_ContainerList CL  WITH (NOLOCK) on Kh.DataKey = CL.DataKey
	...
	SElect	IH.DataKey as TKT_DataKey, 
			...
			@SiteId as SiteID
	Into #Temp_CNB
	from Integration_JCB.dbo.CNB_Header IH WITH (NOLOCK) 
	Inner Join (Select datakey, ContainerKey, equipmentNumber from  Integration_JCB.dbo.CNB_ContainerList WITH (NOLOCK) ) IC on IH.DataKey = IC.DataKey
	...
	inner join #Temp_CNB ND on SD.TKT_DataKey = ND.TKT_DataKey and SD.SiteID = @SiteId
	...
	inner join Integration_JCB.dbo.CNB_StopList SL on TC.ContainerKey = SL.ContainerKey and RD.LocationType = SL.facilityCode
	...
	--select *
	from Integration_JCB.dbo.CNB_ContainerList C
	...
	inner join Integration_JCB.dbo.CNB_Header H  WITH (NOLOCK) on C.DataKey  = H.DataKey
	...
	inner join Integration_JCB.dbo.CNB_ContainerList FC  WITH (NOLOCK) on SL.ContainerKey = FC.ContainerKey
	...
	select 'CNB_Header', * from Integration_JCB.dbo.CNB_Header where DataKey = @DataKey
	select 'CNB_ContainerList',* from Integration_JCB.dbo.CNB_ContainerList where datakey = @DataKey

	select 'CNB_StopList', b.* from Integration_JCB.dbo.CNB_ContainerList A
	inner join Integration_JCB.dbo.CNB_StopList B on A.ContainerKey = B.ContainerKey
