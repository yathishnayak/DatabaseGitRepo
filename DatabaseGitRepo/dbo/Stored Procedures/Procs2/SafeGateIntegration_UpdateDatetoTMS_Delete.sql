...
CREATE PRocedure	[dbo].[SafeGateIntegration_UpdateDatetoTMS_Delete]
AS
BEGIN
	...
	*/
	...
	SELECT			ROW_NUMBER() OVER (PARTITION BY TM.ContainerNo ORDER BY TM.ContainerNo, TM.createdDate DESC ) SL, TM.ActivityId, RT.RouteKey, TM.ContainerNo, TM.createdDate,ContainerDesc,SafeGateChassisNo,SafegareDriverID, Yardname AS SafegateYardName,  TM.DestinationYard,TM.SourceYard,  RT.ChassisKey , RT.DriverKey
					...
					,CASE WHEN DestinationYard <> ''  AND ISNULL(RT.DriverKey,0) <> '' AND RS.Status = 2  THEN TM.CreatedDate ELSE NULL END AS ActualArrival
					,CASE WHEN SourceYard <> ''   AND ISNULL(RT.DriverKey,0) <> '' AND RS.Status = 4  THEN TM.CreatedDate ELSE NULL END AS ActualDeparture
	INTO			#UPDATEDATA
	FROM			#TMP TM
