...
SET @OrderKey = 181918
EXEC MelroseIntegrate_GetScheduleAndActuals_Delete @OrderKey , @OutputID OUTPUT
SELECT @OutputID

*/


CREATE PRoc [dbo].[MelroseIntegrate_GetScheduleAndActuals_Delete] -- MelroseIntegrate_GetScheduleAndActuals 191930 
(
	@OrderKey		int,
...
BEGIN
	...
	SELECT DISTINCT * INTO #StopData FROM (
	SELECT		Sl.facilityCode,ScheduledDateTime  EventDate , 'S' ScheduleActual
	...
	WHERE		TMS_OrderKey = @OrderKey
	UNION ALL
	SELECT		Sl.facilityCode,  ActualDateTime EventDate , 'A' ScheduleActual
	FROM		Integration_JCB.dbo.Flexpro_Header H WITH (NOLOCK)
	INNER JOIN	Integration_JCB.dbo.Flexpro_ContainerList CL WITH (NOLOCK) On H.DataKey = Cl.DataKey
	INNER JOIn	Integration_JCB.dbo.Flexpro_StopList SL WITH (NOLOCK) On Cl.ContainerKey = SL.ContainerKey
	WHERE		TMS_OrderKey = @OrderKey ) A

	UPDATE		PD
	SET			EventDate = SD.EventDate 
	FROM		#ProcessData PD
	INNER JOIN	#StopData  SD ON PD.LocationType = SD.facilityCode AND PD.ScheduleActual = SD.ScheduleActual
	...
	INNER JOIN	MelroseIntegrate.dbo.Integration_Data ID WITH (NOLOCK) ON PD.OrderKey = ID.OrderKey 
				...
				AND ID.IsSuccess = 1 AND CAST(PD.EventDate  AT TIME ZONE 'Pacific Standard Time' AT TIME ZONE 'UTC' AS DATETIME)   = ID.EventDate
