...
--exec [dbo].[Scheduler_GetList_Work] @statusKey = 1, @LoggedUserKey = 29, @isShowAll = 1
-- [dbo].[Scheduler_GetList] @IsExport = 1  '2021-05-05','2021-05-08'
-- [Scheduler_GetList]  @CSRKey =1
-- [Scheduler_GetList] @SorField = 0
-- [Scheduler_GetList] @statusKey = 1, @PageNo = 2, @PageSize = 10, @CreatedUSer = '', @IsAscending = 0, 
-- [Scheduler_GetList] @statusKey = 1, @PageNo = 1, @PageSize = 10, @CreatedUSer = '', @IsAscending = 0,@CSRManagerKey = 6
-- [Scheduler_GetList] @statusKey = 2, @CSRManagerKey = 9
-- @SearchText = '', @SorField = 'OrderNo'
--exec Scheduler_GetList null,null,null,null,null,1,0,0,1,10,null,1,'','',1
CREATE PROCEDURE [dbo].[Scheduler_GetList_Work]   
...
BEGIN
	...
	Declare @RecCount	int, @RowNum int
		...
		WHERE  1=1 AND OD.OrderDetailKey = 75507 AND
		...
		Select @RecCount = COUNT(1) from #Temp
		...
		into #Temp2
		from #Temp A
		LEft join CSR M with (nolock) on A.CSManagerKey = M.CSRManagerKey
