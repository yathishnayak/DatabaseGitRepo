...
CREATE Proc [dbo].[TMS_Integration_ReverseMapping_Century_New] -- [TMS_Integration_ReverseMapping_Century_New] 6241 
(
	@DataKey	int
)
as
Begin
	Declare @SiteId varchar(20) = 'DHL',
			@CustKey	int = 3170
	...
	Exec dbo.TMS_INTEGRATION_InsertRTStops_DHL
	...
	from Integration_JCB.dbo.DHL_Header KH  WITH (NOLOCK)
	inner join OrderHeader OH  WITH (NOLOCK) on KH.TMS_OrderKey = OH.orderkey
	inner join Integration_JCB.dbo.DHL_ContainerList CL  WITH (NOLOCK) on Kh.DataKey = CL.DataKey
	...
	SElect	IH.DataKey as TKT_DataKey, 
			...
			@SiteId as SiteID
	Into #Temp_DHL
	from Integration_JCB.dbo.DHL_Header IH WITH (NOLOCK) 
	Inner Join (Select datakey, ContainerKey, equipmentNumber from  Integration_JCB.dbo.DHL_ContainerList WITH (NOLOCK) ) IC on IH.DataKey = IC.DataKey
	...
	and OH2.OrderKey is not null and IH.isAccepted =1 and OH2.CustKey IN (SELECT CustKey FROM #CustKeys) and IH.DataKey = @DataKey

	select '#Temp_DHL',* from #Temp_DHL
	...
	inner join #Temp_DHL ND on SD.TKT_DataKey = ND.TKT_DataKey and SD.SiteID = @SiteId
	...
	inner join OrderHeader OH  WITH (NOLOCK) on SD.TMS_OrderKey = OH.OrderKey and OH.custkey IN (SELECT CustKey FROM #CustKeys)
	...
	inner join OrderHeader OH  WITH (NOLOCK) on OD.OrderKey = OH.OrderKey and OH.custkey IN (SELECT CustKey FROM #CustKeys)
	...
	inner join Integration_JCB.dbo.DHL_StopList SL on TC.ContainerKey = SL.ContainerKey and RD.LocationType = SL.facilityCode
	...
	where OH.custkey IN (SELECT CustKey FROM #CustKeys) and isnull(H.TMS_OrderKey ,0) = 0 and isnull(ProcessStatus ,'') = ''
	...
	--select *
	from Integration_JCB.dbo.DHL_ContainerList C
	...
	inner join Integration_JCB.dbo.DHL_Header H  WITH (NOLOCK) on C.DataKey  = H.DataKey
	...
	INNER JOIN		Integration_JCB.dbo.DHL_ContainerList FC  WITH (NOLOCK) on SL.ContainerKey = FC.ContainerKey
	...
	select 'TKT_SyncData', * from TKT_SyncData where tkt_datakey = @DataKey AND SiteID = 'DHL'
	select 'TMS_Integration_Header', * from TMS_Integration_Header where datakey = @DataKey AND SiteID = 'DHL'
	select 'TMS_Integration_Container', * from TMS_Integration_Container where datakey = @DataKey AND SiteID = 'DHL'
	select 'TMS_Integration_Routes', * from TMS_Integration_Routes where datakey = @DataKey AND SiteID = 'DHL'

	select 'DHL_Header', * from Integration_JCB.dbo.DHL_Header where DataKey = @DataKey
	select 'DHL_ContainerList',* from Integration_JCB.dbo.DHL_ContainerList where datakey = @DataKey

	select 'DHL_StopList', b.* from Integration_JCB.dbo.DHL_ContainerList A
	inner join Integration_JCB.dbo.DHL_StopList B on A.ContainerKey = B.ContainerKey
