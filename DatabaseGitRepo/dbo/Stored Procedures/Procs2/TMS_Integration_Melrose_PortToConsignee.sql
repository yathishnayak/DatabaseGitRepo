CREATE Proc [dbo].[TMS_Integration_Melrose_PortToConsignee] -- TMS_Integration_Melrose_PortToConsignee 8
(
	 @DataKey	int = 16084
)
as
BEGIN
	...
	inner join Leg L on RT.LegKey = L.LegKey and L.FromLocation = 'Port' and L.ToLocation in ('Consignee','Customer','Shipper')
	where SiteID = 'Melrose' and TH.DataKey =  @DataKey
	...
	inner join Integration_JCB.dbo.Melrose_StopList SL on TRC.ContainerKey = SL.ContainerKey -- and TRT.StopKey = SL.StopKey
	where A.FromLocation = 'PORT' and SL.facilityCode IN('SF') --and (a.LegKey <> TRT.TMS_LegKey OR a.RouteKey <> TRT.TMS_RouteKey)
		and A.DataKey =  @DataKey
	...
	inner join Integration_JCB.dbo.Melrose_StopList SL on TRT.ContainerKey = SL.ContainerKey  and TRT.StopKey = SL.StopKey
	where A.FromLocation = 'PORT' and SL.facilityCode IN('SF') and (a.LegKey <> SL.TMS_LegKey OR a.RouteKey <> SL.TMS_RouteKey)
	and A.DataKey = @DataKey
	...
	inner join TMS_Integration_Container TRC on A.DataKey = TRC.DataKey
	inner join Integration_JCB.dbo.Melrose_StopList SL on TRC.ContainerKey = SL.ContainerKey -- and TRT.StopKey = SL.StopKey
	where A.ToLocation in ('Consignee','Customer','Shipper') and SL.facilityCode IN('ST') --and (a.LegKey <> TRT.TMS_LegKey OR a.RouteKey <> TRT.TMS_RouteKey)
		and A.DataKey =  @DataKey
	...
	inner join TMS_Integration_Routes TRT on A.DataKey = TRT.DataKey 
	inner join Integration_JCB.dbo.Melrose_StopList SL on TRT.ContainerKey = SL.ContainerKey  and TRT.StopKey = SL.StopKey
	where A.ToLocation in ('Consignee','Customer','Shipper') and SL.facilityCode IN('ST') --and (a.LegKey <> SL.TMS_LegKey OR a.RouteKey <> SL.TMS_RouteKey)
	and A.DataKey = @DataKey
