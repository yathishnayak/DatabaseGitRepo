...
DECLARE 
	@UserKey INT = 1144,
	@JSONString NVARCHAR(MAX)= '{"RouteKeyList" : "904872,", "DriverNote" : "Test123", "InternalNote" : "Test456"}',
	@Status	BIT = 0, 
	@IsDebug BIT = 0, 
	@Reason	VARCHAR(100)=''
	EXec [Insert_Voucher_V3] @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT, @IsDebug
	Select @Status AS Status, @Reason AS Reason
*/
CREATE PROCEDURE [dbo].[Insert_Voucher_V3] -- [Insert_Voucher] '483:489:',0,'','', @Out
(
	@UserKey		INT = 1144,
...
BEGIN
	...
	WITH
	(
	RouteKeyList			VARCHAR(300)		'$.RouteKeyList'	,
	...
	SELECT * FROM Fn_SplitParam (@RouteKeyList);
	...
	SET @VoucherStatus= ( SELECT StatusKey FROM dbo.VoucherStatus WITH(NOLOCK) WHERE [Description]='Pending' )  -- @VoucherStatus = 1
	...
	BEGIN	
		...
		PRINT 'Voucher Number is ' PRINT @VoucherNo
		...
		BEGIN TRY
			...
			PRINT 'Voucher Key is' PRINT @VoucherKey
