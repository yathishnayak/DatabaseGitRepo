...
Declare 
	...
	@IsDebug		BIT = 1,
	@JSONSTRING		NVARCHAR(Max) = '{"InvoiceKey" : 38513, "IsSpotOn" : true, "CustomerSegment" : "NAC"}'
EXEC [Sell_OutputByInvoice_V2] @Userkey, @JSONSTRING, @JsonOutput OUTPUT, @Status OUTPUT, @Reason Output, @IsDebug
SELECT @JsonOutput AS JsonOutput, @Status AS Status, @Reason AS Reason 
**/
CREATE PROCEDURE [dbo].[Sell_OutputByInvoice_V2]
(
 -- Consider only when the IsSpotOn = 1
	@UserKey			INT = 1144,
...
BEGIN
	...
	(
		...
		IsSpotOn					BIT						'$.IsSpotOn',		
		CustomerSegment				VARCHAR(5)				'$.CustomerSegment'
	)
	...
	Select @JsonOutput = (
		Select  @Market as MarketLocation, @MarketKey as MarketLocationKey,
