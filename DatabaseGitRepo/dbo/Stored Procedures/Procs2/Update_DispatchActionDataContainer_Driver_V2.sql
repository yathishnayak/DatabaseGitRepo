...
Declare 
	...
	EXEC [Update_DispatchActionDataContainer_Driver_V2] @Userkey, @JSONSTRING, @Status OUTPUT, @Reason Output, @IsDebug
    SELECT @Status, @Reason
*/


CREATE  PROCEDURE [dbo].[Update_DispatchActionDataContainer_Driver_V2]
(
	@UserKey		INT = 0,
...
BEGIN  
 ...
 IF (@IsDebug = 1)
	...
	BEGIN
		UPDATE dbo.[Routes]
		SET [Status]= ( SELECT [Status] FROM dbo.RouteStatus WITH(NOLOCK) WHERE [Description]='InProgress' AND IsActive=1 )
