...
CREATE Proc [dbo].[TMS_Integration_CNB_UpdateMissingRoute] -- TMS_Integration_CNB_UpdateMissingRoute 12286
(
	@DataKey		int = 0
)
as
Begin
	...
	where SiteID = 'CNB' and DataKey = @DataKey
	...
	Begin
		...
		inner join Integration_JCB.dbo.CNB_StopList SL  WITH (NOLOCK) on SL.ContainerKey = SD.TKT_ContainerKey and RD.StopType <> SL.stopNumber
		...
		where  SD.SiteID = 'CNB' and SD.TKT_DataKey = @datakey
		End Try
		Begin Catch
			print ERROR_MESSAGE()
