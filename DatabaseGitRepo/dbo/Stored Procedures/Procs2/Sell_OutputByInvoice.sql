...
/* --19907, 25335 -- Acc Rate : SMB- 47507, ENT- 47502
	DECLARE @InvoiceKey int =0, @InvoiceNo Varchar(50) = '223335',   @JsonOutput nvarchar(max) ='',@Status	bit = 0 , @Reason	varchar(500) = '' ,
	...
	DECLARE @InvoiceKey int =0, @InvoiceNo Varchar(50) = '203857',   @JsonOutput nvarchar(max) ='',@Status	bit = 0 , @Reason	varchar(500) = '' ,
		@IsSpotOn bit = 0, @CustomerSegment varchar(5) = 'NAC', @Debug bit = 1
	EXEC [Sell_OutputByInvoice] @InvoiceKey, @InvoiceNo, @IsSpotOn, @CustomerSegment, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT, @Debug
	SELECT @JsonOutput, @Status, @Reason
*/
CREATE PRoc [dbo].[Sell_OutputByInvoice]
(
	@InvoiceKey			int	= 0,
...
BEGIN
	...
	if(@Debug = 1)
	Begin
		Select '#BaseInfo_Auto_ReturnBaseInfo',* from #BaseInfo_Auto_ReturnBaseInfo order by ORderDetailKey, Containerno
	End
	...
	if(@Debug = 1)
	Begin
		SELECT '#ItemInfo',* FROM #ItemInfo order by Container, itemid
	End
	...
	BEGIN
		...
		Begin
			...
			where ORDERDETAILKEY = @_OrderdetailKey AND ContainerNo = @_ContainerNo AND  City is not null
				and ToLocation in ('Shipper','Customer', 'Consignee')
			order by legno desc--routekey DESC
		...
		--Begin
			insert into #DrayBase(ContainerNo, DrayBase_Value, Margin_Percent,Margin_Value,DrayBase_Rate, FSF_Percent,FSF_Value,Draybase_Total, 
								NetRevenue, EffectiveDate, EffectiveDateFrom, FileName, DateUploaded, UploadedBy, OutputDataKey, LocationName)
			exec SELL_GetDraybaseData 
					@ContainerNo=@_ContainerNo,@city =@City ,@State=@state,@ZipCode=@Zipcode,@LocationName=@LocationName,@MarketKey=@MarketKey, 
					@TerminalKey=@TerminalKey,@CustKey=@CustKey,@InvoiceDate=@InvoiceDate
   ...
   --         THEN CONVERT(varchar(10), TRY_CONVERT(datetime, EffectiveDate, 103), 101) END)) DESC, city DESC, State DESC, Zip Desc
		...
		Begin
			...
			where (City = @city OR ISNULL(City ,'')='') and 
					(State = @State OR ISNULL(State,'')='') and 
					(LTRIM(RTRIM(LocationName)) = @LocationName OR ISNULL(LocationName,'') ='') and  
					MarketKey = @MarketKey and A.Custkey = @CustKey and
					(TerminalKey = @TerminalKey OR ISNULL(TerminalKey,0)=0)  
					and (CASE 
							WHEN ISDATE(EffectiveDate) = 1 
					...
					(CASE 
						...
						WHEN TRY_CONVERT(datetime, ExpiryDate, 103) IS NOT NULL 
							THEN CONVERT(varchar(10), TRY_CONVERT(datetime, ExpiryDate, 103), 101) end) >= Convert(date, @InvoiceDate)
					ORDER BY city DESC, State DESC, LocationName DESC, 
		...
		insert into #AccRercs 
			( RecordSL, LineItem, MarketLocation, Terminal, ItemKey, Rate, BvsNB, FreeTime, MinCnt, MaxCnt, 
				EffectiveDate, EffectiveDateFrom, CostGroup, FileName, DateUploaded, UploadedBy, ContainerNo)
		...
		Update #AccRercs set CustSegment ='NAC'
	...
	Select @JsonOutput = (
		Select  @Market as Market, @MarketKey as MarketKey,
				...
				@City as city, @State as State, @ZipCode as ZipCode, @LocationName as LocationNAme,
