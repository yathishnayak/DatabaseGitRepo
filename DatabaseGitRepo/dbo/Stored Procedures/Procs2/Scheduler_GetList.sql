...
CREATE PROCEDURE [dbo].[Scheduler_GetList]   
...
BEGIN
	...
	Begin
		SET @PickupDateFrom		='01/01/2020'
		SET @PickupDateTo		='01/12/2050'
		SET @DeleveryDateFrom	='01/01/2020'
		SET @DeleveryDateTo		='01/12/2050'
	End
	...
	Declare @RecCount	int, @RowNum int
		...
		SELECT
			...
			SL.ScacCode AS SCAC, GICF.Discharged_dt AS Dischargedate, GICF.HoldStatus,'' LiveDrop,PDC.Code AS DelayReasonCode,OD.PUDelayedCodeKey,
			GICF.Available_for_pickup AS AvailableforPickup,GICF.Available_dt AS AvailableforPickupDate,CAST(0 AS BIT) IsEditDelayReasonCode
		into #Temp
		...
		WHERE  1=1 AND 
			...
			AND (  isnull(@SearchText ,'') = '' OR
			 ...
			 AND (ISNULL(@Pickuplocationkey,0)=0 OR ISNULL(CSR.AddrKey,SR.AddrKey)=@Pickuplocationkey)
			 AND (ISNULL(@Deliverylocationkey,0)=0 OR ISNULL(CDT.AddrKey,DT.AddrKey)=@Deliverylocationkey)
		...
		select 
			...
			@RecCount as RecCount, ISNULL(MarketLocationKey,0)MarketLocationKey , ISNULL(MarketLocation,'')MarketLocation, SteamShipLine,SenderInfo,
			Consignee,
			LinkedContainerNo,
			IsLinked,
			SCAC,
