...
/*
Declare @ItemKey int = 234, @InvoiceKey int = 100676,@ContainerNo varchar(50) = 'TCNU1876328',@JsonOutput	nvarchar(max) = '' 
Exec SELL_GetInvoiceItemSellPrice_shiva20240613 @itemkey, @InvoiceKey, @ContainerNo, @JsonOutput Output
Select @InvoiceKey, @ItemKey, @JsonOutput
-- 84, 18, 209, 277, 207, 116, 103, 243
*/
--select * from invoiceheader where invoiceno = '54154'
CREATE PRoc [dbo].[SELL_GetInvoiceItemSellPrice_shiva20240613]  
(
	@ItemKey				int = 0,
	@InvoiceKey				int = 0,
...
BEGIN
	...
	select RT.Routekey, L.LegID, L.FromLocation, L.ToLocation , 
		...
		isnull(RT.IsBobtail,0) as IsBobtail
	INTO #BaseInfo
	...
	Begin
		select @IsBobtail = case when count(1) > 0 then 1 else 0 end from ITEM I
		Inner join Item M on I.MasterItemKey = M.ItemKey
			where I.ItemKey = @ItemKey OR M.Itemkey = @Itemkey OR M.Description like '%bobtail%'
	End
	...
	if(@IsDebug = 1)
	Begin
		select 'Variables', @City as City, @State as State, @MarketKey as MArket ,
			@custKey as CustKey, @Terminal as Terminal, 
	...
	Begin
		...
		select Rownum, OutputDataKey, RecordSL, MarketKey, Terminal, City, State, Zip, LocationName,
			...
			Rate, BvsNB, FreeTime, MinCnt, MaxCnt
		from (
		Select ROW_NUMBER() over (partition by Lineitem order by EffectiveDate Desc, City Desc, State DESC, Terminal DESC, Marketkey Desc, 
		...
		--Order by EffectiveDate Desc, OutputDataKey  Desc
		) A --where Rownum = 1
	...
	if(@CostGroup in ('Drayage','FSF', 'Bobtail'))
	Begin
		...
		Begin
			...
			select TOP 1 @ContainerNo as ContainerNo, BobtailFormat, BobtailRate, 
				convert(numeric(18,2),0.00) as BobtailCalc,
			...
			End
			declare @DrayBaseValue decimal(18,2)
			select @DrayBaseValue = Draybase_Total from #DrayBase
