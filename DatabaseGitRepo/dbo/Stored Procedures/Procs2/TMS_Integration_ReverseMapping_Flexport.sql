...
CREATE Proc [dbo].[TMS_Integration_ReverseMapping_Flexport] -- [TMS_Integration_ReverseMapping_Flexport] 116729
(
	@DataKey	int
)
as
Begin
	...
	EXEC DBO.TMS_INTEGRATION_InsertEPStops_Flexpro
	EXEC DBO.TMS_INTEGRATION_InsertERStops_Flexpro
	Exec dbo.TMS_INTEGRATION_InsertRTStops
	...
	Left Join Routes RT  WITH (NOLOCK) on TR.TMS_RouteKey = RT.routekey and TR.TMS_LegKey = RT.LegKey
	...
	inner join TMS_Integration_Header TH  WITH (NOLOCK) on RD.OrderKey = TH.TMS_OrderKey 
	inner join TMS_Integration_Container TC  WITH (NOLOCK) on TH.DataKey = TC.DataKey and TH.SiteID = TC.SiteID
	inner join Integration_JCB.dbo.Flexpro_StopList SL  WITH (NOLOCK) on TC.ContainerKey = SL.ContainerKey and RD.LocationType = SL.facilityCode
	LEft join TMS_Integration_Routes TRT  WITH (NOLOCK) on Tc.DataKey = TRT.DataKey and TC.ContainerKey = TRT.ContainerKey 
	...
	inner join TMS_Integration_Container TC  WITH (NOLOCK) on TH.DataKey = TC.DataKey and TH.SiteID = TC.SiteID
	inner join Integration_JCB.dbo.Flexpro_StopList SL  WITH (NOLOCK) on TC.ContainerKey = SL.ContainerKey and RD.LocationType = SL.facilityCode
	inner join TMS_Integration_Routes TRT  WITH (NOLOCK) on Tc.DataKey = TRT.DataKey and TC.ContainerKey = TRT.ContainerKey 
		and TC.SiteID = TRT.SiteID and SL.StopKey = TRT.StopKey
	...
	where isnull(H.TMS_OrderKey ,0) > 0-- and isnull(H.ProcessStatus ,'') = '' 
		and ISNULL(C.TMSOrderDetailKey,0) <> ISNULL(OD.OrderDetailKey,0) and A.SiteID = @SiteId and A.TKT_DataKey = @DataKey

	print '10'
	-- To handle the Leg Changes
	update SL set TMS_RouteKey = null, TMS_LegKey = null, IsScheduleSent = 0, ScheduledDateTime = null,
