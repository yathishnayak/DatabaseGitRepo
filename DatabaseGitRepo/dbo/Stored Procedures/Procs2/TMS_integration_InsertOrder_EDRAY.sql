...
Exec [TMS_integration_InsertOrder] @siteid, @DataKey, @Json, @UserKey, @IsSaved output, @Reason output, @OrderKey output, @OrderNo Output, @OrderDate output
Select @IsSaved, @Reason, @OrderKey,@OrderNo , @OrderDate
*/
CREATE PROCEDURE [dbo].[TMS_integration_InsertOrder_EDRAY]
(
	@SiteId				varchar(50),
	@DataKey			int,
	@Json				nvarchar(max),
...
BEGIN
	...
	Begin Try
		...
		insert into #Header ( DataKey, FileProcessKey, TMS_OrderKey, TMS_CustKey, TMS_SourceAddrKey, TMS_DestinationAddrKey, 
			...
			shipmentReferenceNumber, billOfLadingNumber, workOrderNumber, workOrderDate, ETADate, Consignee )
		...
		Begin
			...
			INSERT INTO dbo.OrderHeader(OrderNo, OrderDate,Csrkey, CustKey,   
						...
						BillToAddrKey,ETADate,BaseRateAmount, IntegrationWONo)
			Select
						...
						,@BillToAddrKey,ETADate,@BaseRateAmount, workOrderNumber
			From #Header
		...
		BEGIN
			...
			SELECT  @Orderkey , @ContainerId,@Containerno , @TMS_ContainerSizeKey ,
				null,@sealNumberList,@grossWeight, case when @weightUOM = 'KG' then 2 else 1 end ,
		...
		where stopType in ('Pickup', 'Live Load')
		...
		BEGIN
			...
			select @RTSourceAddrKey = TMS_SourceAddrKey, @PickStopKey = StopKey 
				from #stops where ContainerKey = @ContainerKey and stopType in ( 'Pickup', 'Live Load')

			select @RTDeliveryAddrKey = TMS_DestinationAddrKey, @DelStopKey = StopKey 
				from #stops where ContainerKey = @ContainerKey and stopType in ( 'Delivery', 'Live Unload')
			...
			select 
				 ...
				 @CreateUserKey,GETDATE(),NULL, GETDATE()
