...
exec Gnosis_GetDynamicFiltersList @UserKey, @JSONString, @Status output, @Reason output
select @Status, @Reason
*/
CREATE  PROCEDURE Gnosis_GetDynamicFiltersList_V3
(
	@UserKey		int,
...
BEGIN
	...
	from Gnosis_Integration_Container_FINAL
		...
		SElect distinct  CD.Container_type as ContainerSize,
			...
			HoldStatus = Case when CTF = 'true' OR TMF = 'true' OR Line = 'true' OR Other = 'true' OR Customs = 'true' 
			OR Freight = 'true' OR ClosedArea = 'true' then 1
			else 0 end,
			OH.SalesPersonKey,
	...
	LEFT JOIN		Address AD WITH (NOLOCK) on OH.DestinationAddrKey = AD.AddrKey
		where 
			...
			(ISNULL(@HoldStatus,'') = '' OR 
				( @HoldStatus = case when CTF='true' OR TMF='true' OR Line='true' OR Other='true' OR Customs='true' 
				OR Freight = 'true' OR ClosedArea = 'true'  then 'YES'
				else 'NO' end)) AND
	...
	select 
		...
		CustomerList = (
			select distinct Customer as CustName, C.CustKey
			from #TempData CC
