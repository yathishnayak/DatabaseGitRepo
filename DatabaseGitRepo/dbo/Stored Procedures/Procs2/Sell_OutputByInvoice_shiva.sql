...
/* --19907, 25335 -- Acc Rate : SMB- 47507, ENT- 47502
	DECLARE @InvoiceKey int =0, @InvoiceNo Varchar(50) = '80933',   @JsonOutput nvarchar(max) ='',@Status	bit = 0 , @Reason	varchar(500) = '' ,
		@IsSpotOn bit = 0, @CustomerSegment varchar(5) = 'NAC', @Debug bit = 1
	EXEC [Sell_OutputByInvoice_shiva] @InvoiceKey, @InvoiceNo, @IsSpotOn, @CustomerSegment, @JsonOutput OUTPUT, @Status OUTPUT, @Reason OUTPUT, @Debug
	SELECT @JsonOutput, @Status, @Reason
*/
CREATE PRoc [dbo].[Sell_OutputByInvoice_shiva]
(
	@InvoiceKey			int	= 0,
...
BEGIN
	...
	select OT.OrderType, IH.Invoicekey, IH.InvoiceNo, IH.InvoiceDate,OD.OrderDetailKey,
		...
		ltrim(rtrim(A.City)) as City, ltrim(rtrim(A.State)) as State, A.ZipCode, D.DriverKey, D.driverID, TT.TruckType, ltrim(rtrim(A.AddrName)) as LocationName,
		...
		RT.LegNo, ltrim(rtrim(A.AddrName)) as AddrName, ltrim(rtrim(Address1)) as Address1
	INTO #BaseInfo
	...
	LEft join Address A  WITH (NOLOCK) on  A.AddrKey = case 
	...
	BEGIN
		...
		if(@OrderType = 'Export')
		Begin
			select Top 1 @city = ltrim(rtrim(City)) , @State = ltrim(rtrim(State)), @LocationName =ltrim(rtrim( LocationName)), 
				@ZipCode = ZipCode, @DriverType = ltrim(rtrim(TruckType))
			from #BaseInfo
			...
			select Top 1 @CityDryRun = ltrim(rtrim(City)) , @StateDryRun =ltrim(rtrim( State))
			from #BaseInfo
			where ORDERDETAILKEY = @_OrderdetailKey AND ContainerNo = @_ContainerNo AND  City is not null
		...
		Begin
			if(@OrderType = 'Import')
			Begin
				select @City = ltrim(rtrim(A.City)), 
					@State = ltrim(rtrim(A.State)),
					@ZipCode = ltrim(rtrim(A.ZipCode)),
		...
		select @zoneName = ZoneName from cost_Zones  WITH (NOLOCK) where ZoneKey = @ZoneKey
		print @ZoneKey
		print @zoneName
		...
		--Begin
			...
			where (ltrim(rtrim(City)) = @city OR City is null) and 
					(ltrim(rtrim(State)) = @State OR State is null) and 
		...
		BEGIN
			...
			Select @IsDraybaseItemExists = case when count(1) >0 then 1 else 0 end from #DrayBase
			...
			from #BaseInfo where FromLocation in ('Customer','Consignee','Shipper') and ToLocation in ('Customer','Consignee','Shipper')
			...
			from #BaseInfo Where FromLocation = 'Port' and 
			...
			from #BaseInfo where LegNo = @ConsToConsLegNo - 1
