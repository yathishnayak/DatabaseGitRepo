...
DECLARE 
	...
	@Status BIT=0,
	@Reason VARCHAR(100)=''
EXec Scheduler_GetListV2 @UserKey,@JSONString,@Status OUTPUT,@Reason OUTPUT
Select @Status, @Reason
**/
CREATE PROCEDURE [dbo].[Scheduler_GetListV2_revert]   
	@UserKey		int,
...
BEGIN
	...
	--SELECT OD.* INTO #OrderDetail FROM OrderDetail OD WITH (NOLOCK) 
	--LEFT OUTER JOIN #OrderDetailKeys ODK ON OD.OrderDetailKey = ODk.OrderDetailKey
	--WHERE (ISNULL(@SearchText,'') = '' and OD.status in (1,2,3,6,7,9,12,14)) OR 
	--			( (ISNULL(@SearchText,'') <> '' and od.OrderDetailKey = ODK.OrderDetailKey));

WITH  FilterOrderDetail  AS (
SELECT OD.*   FROM OrderDetail OD WITH (NOLOCK) 
	LEFT OUTER JOIN #OrderDetailKeys ODK ON OD.OrderDetailKey = ODk.OrderDetailKey
	WHERE (ISNULL(@SearchText,'') = '' and OD.status in (1,2,3,6,7,9,12,14)) OR 
				( (ISNULL(@SearchText,'') <> '' and od.OrderDetailKey = ODK.OrderDetailKey))
) , FilterAddress AS (
SELECT Addrkey, AddrName, Address1, City, Zipcode, State, Country FROM ADDRESS WITH (NOLOCK)
), FiltervContainerType AS (
SELECT TypeID, OrderDetailKey,ContainerTypeKey FROM vContainerType WHERE TypeID IN ('Transload','Hazard')
), FilterOrderHeader AS (
SELECT OH.* FROM OrderHeader OH
INNER JOIN  FilterOrderDetail OD ON OH.Orderkey = OD.OrderKey
)
		...
		into #TempAll
		FROM  FilterOrderDetail OD					WITH (NOLOCK)		
			INNER JOIN FilterOrderHeader OH			WITH (NOLOCK)	ON OH.OrderKey=OD.OrderKey
			...
			LEft join FiltervContainerType CT		on CT.OrderDetailKey = OD.OrderDetailKey and Ct.TypeID = 'Transload'
			...
			LEFT JOIN FiltervContainerType HZ		ON HZ.OrderDetailKey = OD.OrderDetailKey AND CT.TypeID = 'Hazard'
