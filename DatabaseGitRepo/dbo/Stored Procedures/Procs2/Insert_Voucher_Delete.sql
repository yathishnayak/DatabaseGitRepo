...
declare @p5 bit
set @p5=0
exec [dbo].[Insert_Voucher_Delete] @RouteKeyStr='507479:507492:',
	@UserKey=0,@DriverNote='',@InternalNote='',@OutPut=@p5 output
select @p5
*/

CREATE PROCEDURE [dbo].[Insert_Voucher_Delete] -- [Insert_Voucher_Delete] '507479:507492:',0,'','', @Out
...
BEGIN
	...
	BEGIN	
		...
		select @DriverKey as DriverKey, @VoucherNo As CoucherNo
		...
		BEGIN TRY
			...
			WHERE  RT.DriverKey = @DriverKey and IT.ItemType in ('Expense') 
				--and  isnull(C.name, '') <> 'Warehouse'
				select * from #VoucherLine
