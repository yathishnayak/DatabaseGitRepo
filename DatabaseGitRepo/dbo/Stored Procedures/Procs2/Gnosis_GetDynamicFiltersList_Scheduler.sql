...
exec Gnosis_GetDynamicFiltersList_Scheduler @UserKey, @JSONString, @Status output, @Reason output
select @Status, @Reason
*/
CREATE PROCEDURE [dbo].[Gnosis_GetDynamicFiltersList_Scheduler]   
(
	@UserKey		int,
...
BEGIN
	...
	from Gnosis_Integration_Container_FINAL  WITH (NOLOCK)
		...
		SElect distinct  CD.Container_type as ContainerSize,
			cd.ContainerStatus,
			C.OrderCSR CSRName, CM.CsrName as  CSMName,
		...
		Left join		CSR CM  WITH (NOLOCK) on ltrim(rtrim(C.OrderCSR)) = ltrim(rtrim(CM.CsrName))
