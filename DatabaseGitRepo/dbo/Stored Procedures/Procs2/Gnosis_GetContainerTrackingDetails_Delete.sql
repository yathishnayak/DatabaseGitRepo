...
CREATE Proc [dbo].[Gnosis_GetContainerTrackingDetails_Delete]
AS

--DROP TABLE			#BOLNotTracked
SELECT				BillOfLading , OD.ContainerNo,  MAX(OD.OrderDetailKey)OrderDetailKey
INTO				#BOLNotTracked
FROM				(SELECT * FROM OrderDetail WITH (NOLOCK)  WHERE ContainerNo = 'HASU4018513') OD  
