...
CREATE proc [dbo].[SELL_NAC_Accessorial_GetReport]
(
	@CustomerKey		int = 0,
	@MarketKey			int = 0,
...
BEGIN
	...
	SELECT FileProcessKey, RecordSL, isnull(A.CustID, C.CustID) as CustID,isnull(a.CustName,C.CustName) as CustName, 
		...
		LocationName, IsLocationExists, Rate, FreeTime, BvsNB, MinCnt, MaxCnt, ContainerSize, 
	...
	Where (isnull(@CustomerKey,0) = 0 OR A.CustKey = @CustomerKey) AND
		  ...
		  --and A.IsArchived = 0 and A.ExpiryDate >= Convert(date, Getdate())
