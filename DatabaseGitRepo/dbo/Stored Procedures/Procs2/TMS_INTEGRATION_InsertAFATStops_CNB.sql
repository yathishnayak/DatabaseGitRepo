...
CREATE PROC [dbo].[TMS_INTEGRATION_InsertAFATStops_CNB] -- TMS_INTEGRATION_InsertAFATStops_CNB 
AS

BEGIN
	SELECT			CL.ContainerKey,SD.AddrName,
					SD.address1, SD.Address2, SD.City, SD.State, SD.Country, SD.ZipCode, CL.equipmentNumber, CL.equipmentTypeCode , RTN.LocationType
	INTO			#TMP
	...
	WHERE			rtn.LocationType IN ('AF','AT') and sl.stopkey is null -- and FH.DataKey = 13142  
	...
	INSERT INTO		Integration_JCB.DBO.CNB_StopList (ContainerKey, stopType, stopName, stopNumber, facilityCode, stopReferenceNumber,
					address1, Address2, city, state, country, postalCode, equipmentNumber, equipmentTypeCode,IsScheduleSent,EDIEventCode)
	SELECT			ContainerKey,'Added Stops - Before', AddrName, 3, LocationType, 3,
					address1,Address2, City,State, Country, ZipCode, equipmentNumber, equipmentTypeCode,0, 'AS'
	FROM			#TMP
	WHERE			LocationType = 'AF'

	INSERT INTO		Integration_JCB.DBO.CNB_StopList (ContainerKey, stopType, stopName, stopNumber, facilityCode, stopReferenceNumber,
					address1, Address2, city, state, country, postalCode, equipmentNumber, equipmentTypeCode,IsScheduleSent,EDIEventCode)
	SELECT			ContainerKey,'Added Stops - After', AddrName, 3, LocationType, 3,
					address1,Address2, City,State, Country, ZipCode, equipmentNumber, equipmentTypeCode,0, 'AS'
	FROM			#TMP
	WHERE			LocationType = 'AT'
	...
	WHILE(@i <= @n)
		BEGIN
			...
			-- SELECT	@ContainerKey
			EXEC	TMS_INTEGRATION_UpdateStopNo_CNB @ContainerKey
