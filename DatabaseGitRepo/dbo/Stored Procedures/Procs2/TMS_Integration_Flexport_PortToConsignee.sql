...
CREATE proc [TMS_Integration_Flexport_PortToConsignee] -- TMS_Integration_Flexpro_PortToConsignee 16084
(
	 @DataKey	int = 16084
)
as
BEGIN
	...
	where SiteID = 'Flexport' and TH.DataKey =  @DataKey
	...
	inner join Integration_JCB.dbo.Flexpro_StopList SL on TRC.ContainerKey = SL.ContainerKey -- and TRT.StopKey = SL.StopKey
	where A.FromLocation = 'PORT' and SL.facilityCode = 'SF' --and (a.LegKey <> TRT.TMS_LegKey OR a.RouteKey <> TRT.TMS_RouteKey)
	...
	inner join Integration_JCB.dbo.Flexpro_StopList SL on TRT.ContainerKey = SL.ContainerKey  and TRT.StopKey = SL.StopKey
	where A.FromLocation = 'PORT' and SL.facilityCode = 'SF' and (a.LegKey <> SL.TMS_LegKey OR a.RouteKey <> SL.TMS_RouteKey)
	and A.DataKey = @DataKey
	...
	inner join TMS_Integration_Container TRC on A.DataKey = TRC.DataKey
	inner join Integration_JCB.dbo.Flexpro_StopList SL on TRC.ContainerKey = SL.ContainerKey -- and TRT.StopKey = SL.StopKey
	where A.ToLocation in ('Consignee','Customer','Shipper') and SL.facilityCode = 'ST' --and (a.LegKey <> TRT.TMS_LegKey OR a.RouteKey <> TRT.TMS_RouteKey)
		and A.DataKey =  @DataKey
	...
	inner join TMS_Integration_Routes TRT on A.DataKey = TRT.DataKey 
	inner join Integration_JCB.dbo.Flexpro_StopList SL on TRT.ContainerKey = SL.ContainerKey  and TRT.StopKey = SL.StopKey
	where A.ToLocation in ('Consignee','Customer','Shipper') and SL.facilityCode = 'ST' --and (a.LegKey <> SL.TMS_LegKey OR a.RouteKey <> SL.TMS_RouteKey)
	and A.DataKey = @DataKey
