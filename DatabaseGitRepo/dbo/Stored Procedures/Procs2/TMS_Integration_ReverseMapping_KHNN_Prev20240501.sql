...
Create Proc [dbo].[TMS_Integration_ReverseMapping_KHNN_Prev20240501]
(
	@DataKey	int
)
as
Begin
	...
	delete from TMS_Integration_Routes where Siteid = @SiteID and  TMS_RouteKey in (
	...
	from TKT_RouteData RD WITH (NOLOCK) 
	inner join TKT_SyncData SD  WITH (NOLOCK) on SD.TMS_OrderDetailKey = RD.OrderDetailKey
	inner join Integration_JCB.dbo.KHNN_StopList SL  WITH (NOLOCK) on SL.ContainerKey = SD.TKT_ContainerKey and RD.StopType = SL.stopNumber
