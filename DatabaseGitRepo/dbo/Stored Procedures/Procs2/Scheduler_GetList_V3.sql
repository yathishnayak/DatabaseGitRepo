...
INSERT INTO SqlExecutionTimeLog
	...
	VALUes (@UserKey,'Scheduler_GetList_V3','Procedure Entered','',GETDATE())
	SET NOCOUNT ON;
	SET FMTONLY OFF;
	--SET ARITHABORT ON;
...
SET ANSI_NULLS ON;
	...
	DECLARE 
		...
		@CustKeys				varchar(1000),
		@CSRKeys				varchar(1000),
		@CSMKeys				varchar(1000),
		@ContainerStatusKeys	varchar(1000),
		@HoldStatus				varchar(20)	,
		@HoldTypes				varchar(50)	,
		@TerminalNames			varchar(1000),
		@TerminalCodes			varchar(1000),
		@VesselIMOs				varchar(1000),
		@SalesPersonKeys		varchar(1000),
		@MarketKeys             varchar(1000),
		@PickupAvailable		bit		,	
		@PickUpFrom            datetime  ,
        @PickUpTo              datetime  ,
		@CSRName				varchar(50),
		...
		@IsMissingUpdates BIT,
		@IsUnapprovedCharges BIT,
	...
	Select @ContainerNo = ContainerNo, @CustKeys = isnull(CustKeys,''),
		...
		@Deliverylocationkeys=ISNULL(Deliverylocationkeys,''), @Tracking=ISNULL(Tracking,''), @RangeName=ISNULL(RangeName,''),
		@IsMissingUpdates =isnull(IsMissingUpdates,0),@IsUnapprovedCharges= isnull(IsUnapprovedCharges,0),
	...
	WITH (
		...
		ContainerStatusKeys		varchar(1000)	'$.ContainerStatusKeys',
		HoldStatus				varchar(1000)	'$.HoldStatus',
		HoldTypes				varchar(50)		'$.HoldTypes',
		TerminalNames			varchar(1000)	'$.TerminalNames',
		TerminalCodes			varchar(1000)	'$.TerminalCodes',
		VesselIMOs				varchar(1000)   '$.VesselIMOs',
		PickupAvailable			bit				'$.PickupAvailable',
		CustKeys				varchar(1000)	'$.CustKeys',
		CSMKeys					varchar(1000)	'$.CSMKeys',
		SalesPersonKeys			varchar(1000)	'$.SalesPersonKeys',
		MarketKeys				varchar(1000)   '$.MarketKeys',     
		PickUpFrom				datetime        '$.PickUpFrom', 
		PickUpTo				datetime        '$.PickUpTo', 
		PageNo					int				'$.PageNo',
		PageSize				int				'$.PageSize',
		SearchText				NVARCHAR(3000)	'$.SearchText',
		...
		RangeName               VARCHAR(50)		'$.RangeName',
		IsMissingUpdates		BIT				'$.IsMissingUpdates',
		IsUnapprovedCharges		BIT				'$.IsUnapprovedCharges',
	...
	SELECT 
		@StartDate = StartDate, 
		@EndDate = EndDate
	FROM v_DateRanges where RangeName=@RangeName;
	...
	END
		...
		SELECT
			...
			ODS.ActualPickupDate AS ActualPickupDate ,		
			ODSD.ActualDeliveryDate AS ActualDeliveryDate,
			IsPastSchedulePickup=CASE WHEN datediff(day, ODS.SchedulePickupDate, GETDATE()) > 0 
				THEN CAST(1 AS BIT) ELSE CAST(0 AS BIT) END ,		
			CASE WHEN datediff(day, ODSD.ScheduleDeliveryDate, GETDATE()) > 0  
			...
			ODSR.ScheduleDeliveryDate AS ReturnScheduleDeliveryDate, 
			ODSR.ActualDeliveryDate AS ReturnActualDeliveryDate,
			CASE WHEN datediff(day, ODSR.ScheduleDeliveryDate, GETDATE()) > 0 
					THEN CAST(1 AS BIT) ELSE CAST(0 AS BIT) END IsPastReturnScheduleDelivery,
			IsMissingUpdate = CASE 
				 WHEN (
			(DATEDIFF(day, ODS.SchedulePickupDate, GETDATE()) > 0 AND ODS.ActualPickupDate IS NULL) AND 
			 (DATEDIFF(day, ODSD.ScheduleDeliveryDate, GETDATE()) > 0 AND ODSD.ActualDeliveryDate IS NULL)
			...
			isnull(NULLIF(OD.BillOfLadding,''),OH.BillOfLading) AS BillOfLading,
			...
			CASE 
				...
				--WHEN OD.status=17 THEN 'Delivered'
				END AS NextAction,
			OH.custKey as CustKey,BR.BrokerName,OH.OrderSource,OD.[Weight],WU.WeightUnit,OH.VesselName,OD.SealNo,OD.CutOffDate 
			...
			convert(bit, case when isnull(B.OrderDetailKey,0) = 0 then 0 else 1 end) as IntDataExists ,
			...
			--ISNULL(RT.ToLocation,'' ) AS TOLocation,
			--ISNULL(RT.ACtuaLArrival,'') AS ACtualArrival
		into #TempAll
		FROM  dbo.#OrderDetailKeys OD1					WITH (NOLOCK)	
			...
			LEFT JOIN OrderDetailStops  ODS			WITH (NOLOCK) ON ODS.OrderDetailKey=OD.OrderDetailKey AND ODS.StopTypeKey=1 
													AND ISNULL(ODS.IsDryrunPort,0)=0--added stops table
			...
			LEFT JOIN  OrderDetailStops ODSR		WITH (NOLOCK) ON ODSR.OrderDetailKey=OD.OrderDetailKey AND ODSR.StopTypeKey=5 AND ISNULL(ODSR.IsDryRunPort,0)=0
			OUTER APPLY (
						SELECT TOP 1 *
		...
		Status=CASE WHEN StatusKey = 3 THEN 'Schedule InProgress'
				WHEN StatusKey = 1 THEN 'Open'
			--	WHEN StatusKey = 17 THEn 'Delivered'
				ELSE Status END,
		StatusKey = StatusKey 
		...
		AND ( ISNULL(@IsMissingUpdates,0) = 0 OR IsMissingUpdate = 1 )
		AND ( ISNULL(@IsUnapprovedCharges,0) = 0 OR IsChargesApproved = @IsUnapprovedCharges )
		AND	(ISNULL(@RangeName,'') ='' OR   (@RangeName = 'Past'   AND CAST(PickupDate AS DATE) < @StartDate)
				OR (@RangeName = 'Future' AND CAST(PickupDate  AS DATE)> @StartDate)
				OR (@RangeName NOT IN ('Past', 'Future') AND CAST(PickupDate  AS DATE) BETWEEN @StartDate AND @EndDate)))
		...
		Begin
			...
			--select @SearchText as SearchText
			Select @RangeName AS RangeName
			SELECT top 1000 '#TempAll = 123',  * from #TempAll
			where
			(ISNULL(@RangeName,'') ='' OR   (@RangeName = 'Past'   AND CAST(PickupDate AS DATE) < @StartDate)
				OR (@RangeName = 'Future' AND CAST(PickupDate  AS DATE)> @StartDate)
				OR (@RangeName NOT IN ('Past', 'Future') AND CAST(PickupDate  AS DATE) BETWEEN @StartDate AND @EndDate))
		end
		...
		Select ODS.Status as Statuskey, ODS.Description , isnull(cnt,0) as ContainerCount, ODs.Orderby
		into #DashBoard
		from OrderDetailStatus ODS WITH (NOLOCK)	
		Left join #Status T on ODS.Status = T.StatusKey
		where  ODS.status in (1,2,3,6,7,9,12,14,17) and IsScheduler = 1
		...
		*/
		...
		insert into #DashBoard (Statuskey,Description, ContainerCount, OrderBy)
		select 0, 'Total Containers', sum(Containercount), 15 from #DashBoard
		...
		where IsDataSelected = 1 and ((ISNULL(@SearchText,'') <>'' AND @StatusKey = 0) OR 
			(StatusKey in (1,2,3,6,7,9,12,14,17) and 
		...
		DropDowns = ( SELECT
			...
			OrderTypeList = (SElect distinct OrderType as OrderTypeDescription from #TempAll where IsSelectedStatusKey = 1 and isnull(OrderType,'')<>'' Order by OrderType For JSON PATH ),
			...
			ContainerStatusList=(SELECT distinct ContainerStatusKey,ContainerStatus FROM ContainerStatus where isnull(ContainerStatus,'')<>''  FOR JSON PATH),
			TruckStatusList=(SELECT distinct TruckStatusKey,TruckStatus FROM TruckStatus where  isnull(TruckStatus,'')<>'' FOR JSON PATH),
			DateRangeList=(SELECT distinct RangeName,StartDate,EndDate from v_DateRanges FOR JSON PATH)
			FOR JSON PATH
		),
		Dashboard = (
