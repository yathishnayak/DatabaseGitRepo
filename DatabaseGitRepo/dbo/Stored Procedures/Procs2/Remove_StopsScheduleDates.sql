...
exec Remove_StopsScheduleDates @UserKey, @JSONString,@IsDebug, @Status output, @Reason output  
select @Status, @Reason  
*/  
  
Create PROCEDURE [dbo].[Remove_StopsScheduleDates]  
(  
 @UserKey INT = 486,  
...
BEGIN  
 ...
 BEGIN  
  UPDATE OrderDetailStops  
		SET	SchedulePickupDate = NULL  ,
		SchedulePickupDateTo=null
  WHERE OrderDetailKey = @OrderDetailKey AND OrderDetailStopKey = @OrderDetailStopKey  
  
  INSERT INTO AuditLogDetail(DateCreated,CreateUser,RefType,RefId,RefKey,Stage,CommentType,Comments)  
  SELECT GETDATE(),@UserName,'Container',@ContainerNo,@OrderDetailKey,null,'Text','Schedule pickup date removed by '+@UserName  
 END  
  
 IF(@PickupOrDeliveryDate = 'D')  
 BEGIN  
  UPDATE OrderDetailStops  
		SET	ScheduleDeliveryDate = NULL  ,
		ScheduleDeliveryDateTo=null 
  WHERE OrderDetailKey = @OrderDetailKey AND OrderDetailStopKey = @OrderDetailStopKey  
  
  INSERT INTO AuditLogDetail(DateCreated,CreateUser,RefType,RefId,RefKey,Stage,CommentType,Comments)  
  SELECT GETDATE(),@UserName,'Container',@ContainerNo,@OrderDetailKey,null,'Text','Schedule delivery date removed by '+@UserName  
 END  
 SET @Status=1;  
 SET @Reason='Success';  
END  
